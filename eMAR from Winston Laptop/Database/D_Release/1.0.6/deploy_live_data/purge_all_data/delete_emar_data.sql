...
if not exists (
    ...
    begin
        raiserror ('DATABASE: [$(target_database)] Does not exist, there is nothing to delete', 10, 127);
    end;
...
use [$(target_database)];
set nocount on;
set quoted_identifier on; -- needed to delete fdb tables with computed columns
...
insert into [#table_order] values(2,6,'dbo','medications_high_risk',0);
insert into [#table_order] values(2,7,'dbo','patient_allergies',0);
insert into [#table_order] values(2,8,'dbo','patient_home_medications',0);
insert into [#table_order] values(2,9,'dbo','patient_indicators',0);
...
insert into [#table_order] values(2,14,'dbo','print_history',0);
insert into [#table_order] values(2,15,'dbo','prompt_choices',0);
insert into [#table_order] values(2,16,'dbo','site_formulary',0);
insert into [#table_order] values(2,17,'dbo','site_formulary_match',0);
insert into [#table_order] values(2,18,'dbo','templates',0);
insert into [#table_order] values(2,19,'dbo','user_patients',0);
insert into [#table_order] values(2,20,'dbo','user_quick_list_items',0);
insert into [#table_order] values(2,21,'dbo','user_settings',0);
...
insert into [#table_order] values(5,5,'dbo','pharmacy_notifications',0);
insert into [#table_order] values(6,1,'dbo','order_event_details',0);
insert into [#table_order] values(6,2,'dbo','pharmacy_notification_administrations',0);
insert into [#table_order] values(6,3,'dbo','pharmacy_notification_orders',0);
...
from [#table_order] as [tbl]
    inner join [cte_identity] as [id]
        on [tbl].[schema_name] collate database_default = [id].[schema]
            and [tbl].[table_name] collate database_default = [id].[table];

declare
    csr cursor local fast_forward
        for
        select [tbl].[load_level], [tbl].[load_sequence], [tbl].[schema_name], [tbl].[table_name], [tbl].[has_identity] from [#table_order] as [tbl] order by [load_level] desc, [load_sequence];
