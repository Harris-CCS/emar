...
set @template = N'
IF NOT EXISTS (SELECT name FROM sysindexes WHERE name=''IX__emar_update_queue__complete_datetime'') 
  BEGIN
    CREATE NONCLUSTERED INDEX [IX__emar_update_queue__complete_datetime] ON [dbo].[emar_update_queue]
    (
	  [complete_datetime] DESC
    )
  END;
';
...
set @template = N'
IF NOT EXISTS (SELECT name FROM sysindexes WHERE name=''IX__emar_update_queue__inprocess_datetime'') 
  BEGIN
    CREATE NONCLUSTERED INDEX [IX__emar_update_queue__inprocess_datetime] ON [dbo].[emar_update_queue]
    (
	  [inprocess_datetime] DESC
    )
  END;
';
