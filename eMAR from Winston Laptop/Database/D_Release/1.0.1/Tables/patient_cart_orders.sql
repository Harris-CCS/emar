...
alter table [dbo].[patient_cart_orders]
add constraint [fk__patient_cart_orders__medications] foreign key([medication_id]) references [dbo].[medications]([id]);
go

alter table [dbo].[patient_cart_orders]
add constraint [fk__patient_cart_orders__duration_units] foreign key([duration_unit_id]) references [dbo].[duration_units]([id]);
go

alter table [dbo].[patient_cart_orders]
add constraint [fk__patient_cart_orders__patient_problems] foreign key([patient_problem_id]) references [dbo].[patient_problems]([id]);
go

alter table [dbo].[patient_cart_orders]
add constraint [fk__patient_cart_orders__antimicrobial_indications] foreign key([antimicrobial_indication_id]) references [dbo].[antimicrobial_indications]([id]);
go
...
execute [sys].[sp_addextendedproperty] 
    @name = N'MS_Description'
  , @value = N'Person identifier that owns this cart record (Foreign Key to users table)'
  , @level0type = N'SCHEMA'
  , @level0name = N'dbo'
  , @level1type = N'TABLE'
  , @level1name = N'patient_cart_orders'
...
execute [sys].[sp_addextendedproperty] 
  ...
  , @level1name = N'patient_cart_orders'
  , @level2type = N'COLUMN'
  , @level2name = 'antimicrobial_indication_text';
go
