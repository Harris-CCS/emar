...
--        end;
		...
		-- update @schedule_offset if there is a @patient_order_id
		if @patient_order_id is not null
			begin
				select @schedule_offset = [far].[time_offset_minutes]
				from [dbo].[patient_orders] as [po]
				     inner join [dbo].[frequency_schedules] as [fs] on [fs].[id] = [po].[frequency_schedule_id]
					 inner join [dbo].[frequency_types] as [ft] on [ft].[id] = [fs].[frequency_type_id]
					 inner join [dbo].[future_administrations_reschedule] as [far] on [far].[patient_order_id] = [po].[id]
				where [po].[id] = @patient_order_id and [ft].[name] in (N'Daily', N'Weekly');
			end;
...
--				set @date_period_end = cast(@schedule_period_end as date);
		...
		-- update @schedule_period_end to include @schedule_offset if necessary
		-- this variable is used below to set the sched_stop_datetime value for daily and weekly frequency types
		if @schedule_offset is not null and @schedule_offset != 0
			begin
				set @schedule_period_end = DATEADD(minute, @schedule_offset, @schedule_period_end);
			end;
        ...
        (
            ...
            select dense_rank() over(
                 ...
                 , case when @schedule_offset is null or @schedule_offset = 0
				       then cast(cast([cal].[the_date] as datetime) + cast([fjd].[frequency_time] as datetime) as datetime2(7))
					   else DATEADD(minute, @schedule_offset, cast(cast([cal].[the_date] as datetime) + cast([fjd].[frequency_time] as datetime) as datetime2(7)))
				   end                                                                                                 as [sched_datetime]
            ...
            select       dense_rank() over(
                       ...
                       , [fs].[point_in_time] as                                                                                [point_in_time]
			           , case when @schedule_offset is null or @schedule_offset = 0
			  	           then cast(cast([cal].[the_date] as datetime) + cast([fjt].[frequency_time] as datetime) as datetime2(7))
					       else DATEADD(minute, @schedule_offset, cast(cast([cal].[the_date] as datetime) + cast([fjt].[frequency_time] as datetime) as datetime2(7)))
				         end                                                                                                 as [sched_datetime]
                       , cast(@schedule_period_end as datetime2(7)) as                                                          [sched_stop_datetime]
                       , [fs].[frequency_type_recurring]
            from           [dbo].[frequency_schedules] as [fs]
