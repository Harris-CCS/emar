select
    @sql_agent_job_name                  = 'Complete Long Running IDS Entries'
...
if @sql_agent_job_id is null
    begin
        exec [dbo].[sp_add_job] 
          ...
          , @description = ''This is an hourly job that will mark any long-running IDS entries as complete and log that we did so in EMAR.''
          , @start_step_id = 1
        ...
        execute [dbo].[sp_add_jobstep] 
          ...
          , @subsystem = N''TSQL''
          , @command = N''
exec complete_hung_ids_entries          
          ''
