print 'create view [ibex].[dbo].[emar_patients_retrieve_view];';

set @template = N'
create or alter view [dbo].[emar_patients_retrieve_view]

as

select
    [source].[ibex]                              as [external_id]
  , [source].[site]                              as [external_site_id]
  ...
  , case
        when isdate([source].[dob]) = 1 then cast([source].[dob] as date)
        else null
  ...
  , case
        when [source].[naalert] = ''Y'' then 1
        else 0
    end                                          as [name_alert]
  , case
        when [source].[withdraw] = ''Y'' then 1
        else 0
  ...
  , convert(int, 1)                              as [is_active]
  , ltrim(rtrim([source].[exittype]))            as [disposition_type_code]
  , ltrim(rtrim([source].[exitcode]))            as [disposition_code]
  -- 20220623 BRM: Added emar_pat to the View
  , ISNULL(source.emar_pat, ''N'')				 as [emar_pat]
from [dbo].[pat] as [source]
    inner join [dbo].[org] as [sites]
        on [sites].[site] = [source].[site]
    outer apply (
             select
                 [css].[cs_site] as [site]
