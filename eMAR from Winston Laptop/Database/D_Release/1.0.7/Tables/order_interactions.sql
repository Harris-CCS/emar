...
create unique index [ix__order_interactions__medication_interaction_id__drug_num] on [dbo].[order_interactions]
    ([medication_interaction_id], [drug_num]);
go

CREATE NONCLUSTERED INDEX [ix__order_interactions__patient_order_id] ON [dbo].[order_interactions]
    ([patient_order_id])
...
alter table [dbo].[order_interactions]
with check add constraint [fk__order_interactions__patient_home_medications] foreign key([patient_home_medication_id]) references [dbo].[patient_home_medications]([id]) on delete cascade;
go
