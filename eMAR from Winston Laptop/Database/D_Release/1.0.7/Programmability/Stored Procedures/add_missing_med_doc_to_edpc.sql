...
DECLARE @eventStartDate DATE = '2021-06-01';
...
SELECT a.* FROM (
	SELECT DISTINCT
		LEFT(ev.external_id, CHARINDEX('|', ev.external_id)-1) AS [site],
		SUBSTRING(ev.external_id, CHARINDEX('|', ev.external_id) + 1, LEN(ev.external_id) - CHARINDEX('|', ev.external_id)) AS [ibex],
	...
	FROM
		...
		left join emar..external_ids ev ON ev.internal_id = p.id AND ev.vendor = 'pulsecheck' AND ev.entity = 'patients'
	...
	WHERE 
			oe.action_id IN (7,8)
		AND SUBSTRING(ev.external_id, CHARINDEX('|', ev.external_id) + 1, LEN(ev.external_id) - CHARINDEX('|', ev.external_id)) IN (
			select ibex from ibex..hst
		)
		AND oe.order_administration_id IS NOT NULL
		AND oe.event_datetime >= @eventStartDate
		AND med.losecs NOT LIKE '%&%'
) a
	 JOIN ibex..hst pat ON pat.ibex = a.ibex AND pat.site = a.site AND ISNULL(pat.emar_pat, 'N') = 'Y'
...
WHERE
	...
	AND (
		...
		(LEN(ISNULL(@ibexYearMonthDayStart, '')) = 8 AND a.ibex BETWEEN @rangeStart AND @rangeEnd) 
		OR
		-- Process all ibex numbers passed in
		(LEN(ISNULL(@ibexList, '')) > 0 AND a.ibex IN (SELECT ibex FROM @ibexNumbers))
	)
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			DECLARE @str1 VARCHAR(8000) = dbo.stripChartMarkupFromString(@entry, 1);
			DECLARE @str2 VARCHAR(8000) = dbo.stripChartMarkupFromString(@comparisonData, 1);
			IF (@str1 = @str2)
			BEGIN
				SET @isDuplicated = 1;
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					END
					SET @str1 = dbo.stripChartMarkupFromString(@entry, 1);
					SET @str2 = dbo.stripChartMarkupFromString(@followupCompare, 1);
					IF (@str1 = @str2)
					BEGIN
						SET @isDuplicated = 1;
						SET @duplicateLine = @previousAdminData;
