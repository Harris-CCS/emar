select
    @sql_agent_job_name                  = 'EMAR_Clear_Notifications'
...
if @sql_agent_job_id is null
    begin
        exec [dbo].[sp_add_job] 
          ...
          , @description = ''This is an hourly job that will clear any unnecessary notifications for follow ups on meds in EMAR.''
          , @start_step_id = 1
        ...
        execute [dbo].[sp_add_jobstep] 
          ...
          , @step_name = N''Step 1''
          , @subsystem = N''TSQL''
          , @command = N''update notifications set acknowledged_datetime=sent_datetime where id in 
(select distinct notifications.id
from patients
join patient_orders on patient_orders.patient_id=patients.id 
join notifications on patient_orders.id=notifications.patient_order_id
where patients.is_active=0 
and acknowledged_datetime is null 
and category_code= ''''FU'''')''
