...
CREATE NONCLUSTERED INDEX [ix__order_events__action_id] ON [dbo].[order_events]
    ([action_id])
    INCLUDE ([order_administration_id],[event_datetime]);
GO

CREATE NONCLUSTERED INDEX [ix__order_events__order_administration_id] ON [dbo].[order_events]
    ([order_administration_id],[action_id])
    INCLUDE ([event_datetime]);
GO

CREATE NONCLUSTERED INDEX [ix__order_events__patient_order_id] ON [dbo].[order_events]
    ([patient_order_id]);
GO
...
alter table [dbo].[order_events]
add constraint [fk__order_events__users] foreign key([add_user_id]) references [dbo].[users]([id]);
go

alter table [dbo].[order_events]
add constraint [fk__order_events__templates] foreign key([template_id]) references [dbo].[templates]([id]);
go
...
execute [sys].[sp_addextendedproperty] 
    @name = N'MS_Description'
  , @value = N'Template identifier that created this record (Foreign Key to templates table)'
  , @level0type = N'SCHEMA'
  , @level0name = N'dbo'
  , @level1type = N'TABLE'
  , @level1name = N'order_events'
