print 'create function [dbo].[emar_patient_medications_retrieve_fn];';
...
*/
set @template = N'
CREATE OR ALTER FUNCTION [dbo].[emar_patient_medications_retrieve_fn] (@Ibex varchar(20))
RETURNS @ret TABLE (
	patient_id varchar(20)	-- ibex varchar(14)  --  only
	,internal_key varchar(50)
	,class varchar(12)
	,category varchar(12)
	...
	,medication_drug_id varchar(9)
	,is_active bit
	,comment varchar(255)
	,last_taken_note varchar(20)
	,dose varchar(20)
	,route varchar(20)
	,schedule varchar(40)
	,unit varchar(20)	
...
BEGIN
	...
	, cte_hie as (
		...
		JOIN   [dbo].[emar_hie_meds_medication_id_cache] c
				ON [source].[num] = c.[num]
	...
	INSERT	@ret_work
			(id, internal_key, patient_id, class, category, internal_drug_id, ndc, drug_id, [name], alternate_name, medication_drug_id, 
			is_active, comment, last_taken_note, dose, route, schedule, unit, reaction, [source], parent_drug_id, 
			parent_drug_name, add_user_id, add_datetime, change_user_id, change_datetime, action_status, information_source, 
			person_number, account_number, medication_id, [match])
	select result.id
		...
		, internal_key = CASE
				...
				WHEN LTRIM(ISNULL(medication_drug_id, '''')) NOT IN (''0'', '''')
					THEN ''AlgDrugId:'' + medication_drug_id + CASE WHEN LTRIM(ISNULL(parent_drug_id, '''')) = '''' THEN '''' ELSE '':'' + parent_drug_id END
	...
	-- Filter the result list to only include current/active current medications.
	-- Winston Murdock, 06/30/2022. PC-27381
	WHERE result.action_status = ''C''
	AND result.is_active = 1
	-- 20220818 BRM:  Need to make sure that at least one of the WHEN''s from the internal_key returns a value
	-- If not, the record is of no use, so filter it out...
	AND (medication_id != 0 
		OR LTRIM(ISNULL(medication_drug_id, '''')) NOT IN (''0'', '''') 
		OR LTRIM(ISNULL(ndc, '''')) != ''''
		OR ISNULL(LTRIM(internal_drug_id), ''ft'') NOT IN (''ft'', '''')
		OR ISNULL(LTRIM(result.name), '''') != '''');

	-- Guarantee we only return one record for each ibex/internal_key combination
	...
	INSERT	@ret
			(patient_id, internal_key, class, category, internal_drug_id, ndc, drug_id, name, alternate_name, medication_drug_id, 
			is_active, comment, last_taken_note, dose, route, schedule, unit, reaction, [source], parent_drug_id, parent_drug_name, 
			add_user_id, add_datetime, change_user_id, change_datetime, action_status, information_source, person_number, 
			account_number, medication_id, match)
	SELECT	patient_id,
			internal_key,
			class,
			category,
			...
			person_number,
			account_number,
			CASE medication_id WHEN 0 THEN NULL ELSE medication_id END as medication_id,
			match
	FROM	@ret_work
	WHERE	dup_num = 1
...
execute [sys].[sp_addextendedproperty] 
  ...
  , @value = N'Function to retrieve a table of patient medication data'
  , @level0type = N'SCHEMA'
  , @level0name = N'dbo'
  , @level1type = N'FUNCTION'
  , @level1name = N'emar_patient_medications_retrieve_fn';
*/