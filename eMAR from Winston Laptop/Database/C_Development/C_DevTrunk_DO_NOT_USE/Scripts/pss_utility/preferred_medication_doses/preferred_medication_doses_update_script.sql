...
select
    *
from dbo.[preferred_medication_doses_export_view] [pfsev]
where site_name = 'Pulsecheck Hospital'
      and [drug_id] in ('156072')
order by [drug_id];
...
/*
select * from [dbo].[preferred_medication_doses_import_template] as [source] where [source].[drug_id] in ('156072') order by [source].[drug_id];
update source set [dose02] = '305',unit_name02='mL' from [dbo].[preferred_medication_doses_import_template] as [source] where [source].[drug_id] = '156072';
--update source set [unit_name01] = 'PO'   from [dbo].[preferred_medication_doses_import_template] as [source] where [source].[drug_id] = '156072';
--update source set [unit_name01] = 'Oral' from [dbo].[preferred_medication_doses_import_template] as [source] where [source].[drug_id] = '156072';
--update source set [unit_name01] = 'PO'   from [dbo].[preferred_medication_doses_import_template] as [source] where [source].[drug_id] = '156072';
select * from [dbo].[preferred_medication_doses_import_template] as [source] where [source].[drug_id] in ('156072') order by [source].[drug_id];
*/
select
