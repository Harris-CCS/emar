if '$(load_data)' in('sample', 'live')
    begin
        ...
        with src
             as (select *
                 from   (values
                     ('Ear', 1, 'Ear Give Template',null,null)) as [t]([name], [is_active], [title], [save_button_text], [cancel_button_text]))
             insert into [dbo].[templates]([name], [is_active], [title], [save_button_text], [cancel_button_text])
             select [name], [is_active], [title], [save_button_text], [cancel_button_text]
             from   [src];
