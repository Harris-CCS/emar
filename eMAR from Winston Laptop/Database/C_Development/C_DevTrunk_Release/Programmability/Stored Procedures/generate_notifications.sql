/*
	...
	when the patient's med orders meet any of the following criteria:
		...
		* IV Continuous orders ending (notification_categories.code = 'IV1')
			- half an hour before the duration ends 
		* IV Continuous orders ended (notification_categories.code = 'IV2')
...
begin
	...
	-- Find ordered meds with a frequency type of continuous, which are ending 30 minutes or less from now, and
	-- have not yet been discontinued. Meds that have not yet ended get the "ending" category, else they
	-- get the "ended" category.
	INSERT INTO
		@action_orders
	SELECT
		po.id AS [patient_order_id],
		...
		po.medication_route_id,
		oa.id AS [order_administration_id],
		po.end_datetime AS [action_datetime],
		CASE 
			WHEN DATEDIFF(MINUTE, @Now, po.end_datetime) BETWEEN 1 AND 30 THEN 'IV1' 
			ELSE 'IV2'
		END AS [category_code]
	FROM
				patient_orders po
		JOIN @action_patients p ON po.patient_id = p.patient_id
		JOIN frequency_schedules fs ON po.frequency_schedule_id = fs.id
		JOIN frequency_types ft ON fs.frequency_type_id = ft.id
		JOIN order_administrations oa ON po.id = oa.patient_order_id
		LEFT JOIN order_events oe ON po.id = oe.patient_order_id AND oe.action_id = 7	-- Follow up action ID
		LEFT JOIN order_event_details oed ON oe.id = oed.order_event_id AND oed.prompt_text LIKE '%~~(infusionDiscontinued)' AND (oed.prompt_text LIKE '%Date%' or oed.prompt_text LIKE '%Time%')
	WHERE
			po.order_status NOT IN ('Pending','Cancelled','Deleted','Completed')
		AND oa.administration_datetime <= @Now
		AND DATEDIFF(MINUTE, @Now, po.end_datetime) <= 30
		AND ft.[name] = 'Continuous'
		AND LEN(LTRIM(RTRIM(ISNULL(oed.entered_text,'')))) = 0

	-- Delete any existing "Ending" continuous order notifications that match to new "Ended" orders and have not been acknowledged yet,
	-- so we don't end up with an "Ending" and "Ended" notification for the same order.
	DELETE
		n
	FROM
			 notifications n
		JOIN 
				@action_orders ao ON n.patient_order_id = ao.patient_order_id
			AND n.order_administration_id = ao.order_administration_id
			AND ao.category_code = 'IV2'
			AND n.acknowledged_datetime IS NULL
	WHERE
		n.category_code = 'IV1'
	...
	-- Delete notifications that were generated more than 2 days ago.
	DELETE FROM notifications WHERE generated_datetime < DATEADD(dd, -2, SYSDATETIMEOFFSET());
