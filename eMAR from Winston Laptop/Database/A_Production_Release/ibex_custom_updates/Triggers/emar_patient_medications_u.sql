print 'create trigger [ibex].[dbo].[hie_alg].[emar_patient_medications_u];';

set @template = N'
create or alter trigger [dbo].[emar_patient_medications_u] on [dbo].[hie_meds] after update as
begin
    ...
    from     (select [patients].[ibex]
              ...
              where ([i].[person] > char(1) and [i].[acctnum] > char(1)) and [patients].[emar_pat] = ''Y''
                and (
                    ...
                    isnull([i].[site], 0)         <> isnull([d].[site], 0)

	             -- Copied these lines from emar_patient_allergies2_u.
	             -- We were getting an error converting an empty string to an int.
	             -- Allergies also does a similar isnull check for usr and usrchg.
	             -- Winston Murdock, 01/06/2023.  PC-26489
	             --or isnull([i].[class]       ,char(1))   <> isnull([d].[class]       ,char(1))
                 --or isnull([i].[cat]         ,char(1))   <> isnull([d].[cat]         ,char(1))
	             or isnull([i].[class]       ,0)       <> isnull([d].[class]       ,0)
                 or isnull([i].[cat]         ,0)       <> isnull([d].[cat]         ,0)
                 ...
                 or isnull([i].[dose], char(1))         <> isnull([d].[dose], char(1))
                 or isnull([i].[unit], char(1))         <> isnull([d].[unit], char(1))
                 or isnull([i].[route], char(1))        <> isnull([d].[route], char(1))
...
print 'create trigger [ibex].[dbo].[alg].[emar_patient_medications2_u];';

set @template = N'
create or alter trigger [dbo].[emar_patient_medications2_u] on [dbo].[alg] after update as
begin
