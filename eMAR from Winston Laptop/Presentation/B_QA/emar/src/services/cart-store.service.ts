  ...
  readonly cartOrders$ = this.cart$.pipe(
    map(carts => (carts && carts.orders) ? carts.orders.map((ord) => ({
      ...
      displayDoseUnit: ord.doseUnit ? ord.doseUnit.printName : '',
			displayDuration: (ord.duration && ord.durationUnit && ord.durationUnit.name) ? ' for ' + ord.duration + ' ' + ord.durationUnit.name : ''		
  ...
  async postCartOrderByListOrderId(order: CartOrder, listOrderId: number, patientId: number, userId: number, listType?: string) {
    ...
    try {
      ...
      if (listType === 'quicklist') {
        cartOrder = await this.medOrderService.postCartOrderByListOrderId(listOrderId, patientId, userId, order).toPromise()
      } else if (listType === 'deptPreferredWithTab') {
        cartOrder = await this.medOrderService.postCartOrderByDeptPreferredListOrderId(listOrderId, patientId, userId, order).toPromise()
      } else if (listType === 'groups') {
        cartOrder = await this.medOrderService.postCartOrderByGroupsListOrderId(listOrderId, patientId, userId, order).toPromise()
      }
