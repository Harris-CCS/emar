...
export class HeaderPatientComponent implements OnInit {
  ...
  ngOnInit(): void {
    setTimeout(() => this.refreshPatientData(), 10000)
  }
  ...
  async refreshPatientData() {
    ...
    await this.patientStoreService.fetchPatient(this.patientStoreService.patientId)
    
    this.currentVitalsArray = null
    this.vitalsTableStructure = null
  ...
  getPatientFullName(): string {
    ...
    const fullName: string = `${this.patient.lastName}, ${this.patient.firstName}`
...
*/
  ...
  getIndicatorClassName(indicator: any): string {
    /*
      ...
      4 = Panic High
      ******
      Special Indicators for BP (because it is a combined systolic/diastolic evaluation):
      5 = Abnormal Low/High
      6 = Panic Low/High
    */
    switch (indicator) {
      ...
      for (const alg of this.patient.patientAllergies) {
        if (alg.isActive) {
          this.allergiesTableStructure.appendTableBodyCell(true, {
            isHeaderCell: true,
            data: `${alg.internalDrugId === 'ft' ? 'Free Text: ' : ''}${alg.name} ${alg.alternateName || ''}`,
            dataType: 'string',
            className: (alg.actionStatus.toUpperCase() === 'C' ? 'align-center' : 'align-center italics') + `${alg.internalDrugId === 'ft' ? ' freetext' : ''}`,
          });
          ...
          this.allergiesTableStructure.appendTableBodyCell(false, {
            isHeaderCell: false,
            data: `${alg.internalDrugId === 'ft' ? alg.comment + ' Please review this free text entry' : alg.comment}`,
            dataType: 'string',
            className: (alg.actionStatus.toUpperCase() === 'C' ? 'align-center' : 'align-center italics') + `${alg.internalDrugId === 'ft' ? ' freetext' : ''}`,
          });
        }
    ...
    if (!this.homeMedsTableStructure) {
      ...
      for (const med of this.patient.homeMedications) {
        if (med.isActive && med.actionStatus.toUpperCase() === 'C') {
          if (!this.hasHomeMeds) {
            this.hasHomeMeds = true;
          }
          this.homeMedsTableStructure.appendTableBodyCell(true, {
            isHeaderCell: true,
            data: `${med.internalDrugId === 'ft' ? 'Free Text: ' : ''}${med.name} ${med.alternateName || ''}`,
            dataType: 'string',
            className: `align-center ${med.internalDrugId === 'ft' ? 'freetext' : ''}`,
          });
          const dose = med.dose == null? '': med.dose;
          this.homeMedsTableStructure.appendTableBodyCell(false, {
            isHeaderCell: false,
            data: `${dose} ${med.medicationUnit && med.medicationUnit.unitName
          ...
          this.homeMedsTableStructure.appendTableBodyCell(false, {
            isHeaderCell: false,
            data: `${med.internalDrugId === 'ft' ? med.comment + ' Please review this free text entry' : med.comment || ' '}`,
            dataType: 'string',
            className: `align-center ${med.internalDrugId === 'ft' ? 'freetext' : ''}`,
          });
        }
