...
export class DateTimeModalComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        // the modal gets open
        this.modalSubscribe = this.modalService.modalOpening.subscribe(({ data }) => {
            ...
            if (data.dateTime !== undefined && regExpDateTime.test(data.dateTime)) {
                ...
                this.disableButtons();
            } else if ((data.dateTime !== undefined && regExpIso.test(data.dateTime)) || data.dateTime === ''){
                let mo = data.dateTime === '' ? moment(): moment(data.dateTime);
