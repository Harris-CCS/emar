...
export class CalendarFormComponent implements OnInit, OnDestroy {
  ...
  nbDisplayedWeeks: number; // number of weeks that will be displayed
  ...
  ngOnInit() {
    /// one only one day of administrations on the first line
    this.nbDisplayedWeeks = Math.ceil((this.siteStoreService.schedule_future_items - 1) / 7) + 1;
  ...
  group(events: AdministrationEvent[]): CalendarDay[][] {
    ...
    let iDay: number;
    let iWeek: number = null;
    let current: NgbDate;
    let date: NgbDate; /// date of the event
    let arr: string[];
    let firstWeek: number = null;
    ...
    for (let ii = 0; ii < events.length; ++ii) {
      ...
      let n = moment(event.eventDateTimeUTC).week();
      if (iWeek == null) {
        iWeek = 0;
        firstWeek = n;
      } else if (iWeek + firstWeek != n) {
        iWeek = n - firstWeek;
      }
      if (typeof weeks[iWeek] == 'undefined') { // build empty week line of weeks array
        weeks[iWeek] = [];
        current = (iDay == 0)? date: this.calendarService.getPrev(date, 'd', iDay);
        for (let d = 0; d < 7; d++) {
          weeks[iWeek][d] = {
            name: DAYLABELS[d],
            day: ('0' + current.day.toString()).slice(-2),
            month: ('0' + current.month.toString()).slice(-2),
            events: [],
          };
          current = this.calendarService.getNext(current, 'd', 1);
        }
        if (iWeek >= this.nbDisplayedWeeks) {
    ...
    /// build at least NBDISPLAYEDWEEKS weeks
    if (typeof weeks[0] !== 'undefined') {
      for (iWeek = iWeek+1; iWeek < this.nbDisplayedWeeks; ++iWeek) {
        weeks[iWeek] = [];
        for (let d = 0; d < 7; d++) {
          weeks[iWeek][d] = {
            name: DAYLABELS[d],
