...
export class FrequencyFormComponent implements OnInit, OnDestroy {
  ...
  prnOptions: Array<PRNIndication>;
  ...
  subscriptionPRNIndications: Subscription;
  ...
  this.subscriptionPRNIndications = this.composerSchedulerService.getPRNIndicationsFromAPI(this.userSiteId)
    .subscribe((response) => {
  ...
  async setFrequencyDefaultValues() {
    if (this.medComponentId === 0) {
      ...
      let needUpdate:boolean = false; // to avoid to call multiple time the API call
      ...
      if (initialOrderData.duration === 0 || initialOrderData.duration) {
        ...
        this.changeSelectedDuration(this.selectedDuration, false);
        needUpdate = true;
      }
      // Duration Unit
      if (initialOrderData.durationUnitId) {
        ...
        if (durationUnit) {
          ...
          this.onDurationUnit(this.selectedDurationUnit, false);
          needUpdate = true;
        }
      }
      // Start Date and Time
      ...
      if (initialOrderData.cartOrderAdministrations && initialOrderData.cartOrderAdministrations.length > 0) {
        ...
        if (initialOrderData.frequencySchedule && initialOrderData.frequencySchedule.id) {
          ...
          this.availableAdministrations = await this.composerSchedulerService.getFrequencyAdministrationsFromAPI
            (this.userId, this.userSiteId, initialOrderData.frequencySchedule.id, beginDateTimeFormatted, endDateTimeFormatted, this.selectedDuration, this.selectedDurationUnit?.id).toPromise();
      ...
      if (initialComposerData.action === 'add' || needUpdate) {
        this.updateAdministrations(
          initialOrderData.frequencySchedule,
          this.frequencyForm.get('startTimeUTC').value,
          this.frequencyForm.get('endTimeUTC').value,
  ...
  ngOnDestroy() {
    ...
    this.subscriptionPRNIndications.unsubscribe();
  }
  ...
  async updateAdministrations(frequency: Frequency, beginDateTime?: string, endDateTime?: string, location?: string) {
    if (frequency &&
      ...
      this.availableAdministrations = await this.composerSchedulerService.getFrequencyAdministrationsFromAPI
        (this.userId, this.userSiteId, frequency.id, beginDateTimeFormatted, endDateTimeFormatted, this.selectedDuration, this.selectedDurationUnit?.id).toPromise();
  ...
  changeSelectedDuration(duration: number, update:boolean = true) {
    ...
    if (update) {
      this.updateAdministrations(this.selectedFrequencyData,
        this.frequencyForm.get('startTimeUTC').value,
        this.frequencyForm.get('endTimeUTC').value,
  ...
  onDurationUnit(unit: DurationUnit, update:boolean = true) {
  ...
  async updateScheduledAdministrationsDuration(duration: number, durationUnit: DurationUnit, fromSchAdmin?: boolean) {
    return;
...
// const beginDateTimeFormatted = this.frequencyForm.get('startTime').value ? `${this.datePipe.transform(this.frequencyForm.get('startTime').value, 'dateTimeSeconds')}${this.siteUTCOffset}` : '';
// const endDateTimeFormatted = this.frequencyForm.get('endTime').value ? `${this.datePipe.transform(this.frequencyForm.get('endTime').value, 'dateTimeSeconds')}${this.siteUTCOffset}` : ''; 
// const freq = this.frequencyForm.get('frequency').value;
//     this.scheduledAdministrations = await this.composerSchedulerService.getFrequencyAdministrationsFromAPI
 //    (this.userId, this.userSiteId, freq, beginDateTimeFormatted, endDateTimeFormatted, this.selectedDuration, this.selectedDurationUnit?.id).toPromise();
*/
      this.frequencyForm.patchValue({ scheduledAdministrations: this.scheduledAdministrations });
  ...
  onChangeEndTime(event) {
    if (event.target.value == '' || this.validDateTime(event.target.value)) {
