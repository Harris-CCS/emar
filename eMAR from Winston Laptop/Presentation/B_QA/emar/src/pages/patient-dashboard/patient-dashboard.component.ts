import { Component, OnInit, ViewChild, ElementRef, ComponentFactoryResolver, OnDestroy, ViewChildren, QueryList } from '@angular/core';
...
import { AdministrationMedicines } from '../../app/interfaces/administration-medicines';
import { SelectorMatcher } from '@angular/compiler';
import { NgbPopover } from '@ng-bootstrap/ng-bootstrap';
import { OrderAdministrationEvent } from 'src/shared/pipes/orderAdministrationEvent';
interface Interval {
  id: number;
  event: string;
  isHour: boolean;
...
export class PatientDashboardComponent implements OnInit, OnDestroy {
    ...
    modalMedicationSelected: string;
    modalMedicationSelectedSchedule: string;
    itemOrderSelected: number;
    administeredItems: AdministrationMedicines[] = [];
    returnedAdministoredLinkDateTime: string;
    // @ViewChild ('popAdm') popAdm: NgbPopover;
    @ViewChildren('popAdm') popAdministrations: QueryList<NgbPopover>;
  ...
  ngOnInit(): void {
    console.log('~~~~~~~~~~MAR Patient ngOnInit at ', new Date().toUTCString())
    ...
    if (MOCKUP) {
      ...
      .subscribe( orders => {
        ...
        this.orders.forEach(order => {
          if ('pharmacyVerificationStatus' in order && order.pharmacyVerificationStatus !== 1) {
            const rxvIndex = order.availableActions.findIndex((action) => action.availableAction === 'PharmVerification')

            if (rxvIndex > 0) {
              // remove action if pharmacyVerificationStatus is completed (2) or not needed (0)
              order.availableActions.splice(rxvIndex, 1)
            }
  ...
  filterOrders(): void {
    ...
    this.orders.sort(this.compareOrdersStatus.bind(this));
    console.log('ORDERS AFTER SORT', this.orders);
  }

  // Compare 2 orders - NOT USED ANYMORE
  compareOrders(o1: Order, o2:Order): number {
    ...
    // Compare 2 orders on the status - then on nextActionTime (no nextActionTime null at teh bottom) - then on name
    // return -1 if o1 will be displayed above o2
    compareOrdersStatus(o1: Order, o2:Order): number {
      let m1: moment.Moment;
      let m2: moment.Moment;
      let status: string;
      const criteria: Array<string> = ['deleted','cancelled','discontinued','completed','ongoing','pending','onhold','pendingdiscontinue','pending',
    'scheduled'];
    if (this.sort.toLowerCase() == 'alphabetical') {
      let active1: Boolean = this.IsActiveOrder(o1);
      let active2: Boolean = this.IsActiveOrder(o2);
      if (active1 && !active2) return -1; // actives are always before inactives
      if (!active1 && active2) return 1; // inactives are always after actives
      // they have the same status – so we compare the alphabetical order
      return o1.medication.displayName.toLowerCase() < o2.medication.displayName.toLowerCase()? -1: 1;
  }

    else if (this.sort.toLowerCase() == 'administration time') {
        if (typeof o1.nextActionTime === 'undefined' || o1.nextActionTime == null) {
          m1 = this.getOrderOldestOverdue(o1, null);
        } else {
          m1 = moment(o1.nextActionTime);
        }
        if (typeof o2.nextActionTime === 'undefined' || o2.nextActionTime == null) {
          m2 = this.getOrderOldestOverdue(o2, null);
        } else {
          m2 = moment(o2.nextActionTime);
        }
      } else {
        m1 = (o1.beginDatetime !== null)? moment(o1.beginDatetime): moment(o1.addDatetime);
        m2 = (o2.beginDatetime !== null)? moment(o2.beginDatetime): moment(o2.addDatetime);
      }
      let ret = 0;

      for (status of criteria) {
        if (!ret) {
          if (o1.orderStatus.toLowerCase() == status) {
            if (o2.orderStatus.toLowerCase() == status) {
              // status are sort together on date or if same date on name
        
              if ((m1 === null && m2 === null) || (m1 !== null && m2 !== null && m1.isSame(m2))) {
                ret = o1.medication.displayName.toLowerCase() < o2.medication.displayName.toLowerCase()? -1: 1;
                // if (DEBUG_SORT) console.log('SORTNAME');
              } else if (m1 === null) {
                ret = 1;
                // if (DEBUG_SORT) console.log('SORT+1');
              } else if (m2 === null) { //  o2 after o1
                ret = -1;
                // if (DEBUG_SORT) console.log('SORT-1');
              } else {
                ret = m2.isBefore(m1)? 1: -1;
              }
            } else {
              ret = 1; // o2<o1 - status are below any other status
            }
          } else if (o2.orderStatus.toLowerCase() == status) {
          ret = -1; // o1<o2 - status are below any other status
          }


        }
      }
      if (DEBUG_SORT) console.log('SORT:'+o1.id+o1.medication.displayName.substring(0,20)+'('+(m1==null?m1:m1.format())+')-'+o1.orderStatus+'||'+o2.id+o2.medication.displayName.substring(0,20)+'('+(m2==null?m2:m2.format())+')-'+o2.orderStatus+'='+ret);
      return ret;
    }
    IsActiveOrder(o:Order) {
      return this.patientMedOrderStoreService.IsActiveOrder(o);
    }
...
showPrinterModal() {
  ...
  /* added import modal service
   added "showPrinterModal()" to this components html
   added route reference to this route coments html 
  the route is located in the UserPrinterInfoModalComponent:
  selector: 'app-user-printer-info-modal',
  */
  this.modalService.open('userPrinterInfo');
...
trackByFn(index, item) {
  ...
  rxVerificationStatus(order: Order) {
    return order.pharmacyVerificationStatus === 1 
      ? {color: this.rx_verification_needed, tooltip: 'Verification Needed'} 
      : {color: this.rx_verification_complete, tooltip: 'Verification Completed'} 
  }
  /* no more used
  showPatientAdministrationsModal(item: any, event: any) {
    // var whereNoticed = 0;
 
    try {
    if (event.currentTarget.offsetParent.id != null 
      && event.currentTarget.offsetParent.id.trim().length > 12) {
    this.itemOrderSelected = parseInt(event.currentTarget.offsetParent.id.trim().substring(12));
    var itemSelectedCount = 0;
    
     var TableRow= 0;  
     var ItemTime= "00:00";
     var ItemDate= "01/01/2021";  
     var ItemStatus= "?";
     var Given = " ";
     var FollowUp = " ";
     var PharmacyVerified = " ";
     var ItemLinkDateTime ="#";
     
try {
  this.administeredItems.splice(0,this.administeredItems.length);
} catch (error) {}

    while (itemSelectedCount < item.orders[this.itemOrderSelected].orderAdministrations.length) {
      TableRow = itemSelectedCount;
      if (item.orders[this.itemOrderSelected].orderAdministrations[itemSelectedCount].administrationScheduledDatetime.length > 16) {
        ItemLinkDateTime =
        item.orders[this.itemOrderSelected].orderAdministrations[itemSelectedCount].administrationScheduledDatetime;

        ItemTime=
        item.orders[this.itemOrderSelected].orderAdministrations[itemSelectedCount].administrationScheduledDatetime.substr(11,5);
        var _itemDateTime =
        item.orders[this.itemOrderSelected].orderAdministrations[itemSelectedCount].administrationScheduledDatetime.substr(0,10);
        ItemDate = _itemDateTime.substring(5,7) + "/" + _itemDateTime.substring(8,10) + "/" + _itemDateTime.substring(0,4);
        ItemStatus= 
        item.orders[this.itemOrderSelected].orderAdministrations[itemSelectedCount].administrationStatus;
        if (ItemStatus=="Late") {ItemStatus="Due";}
        var _xItemStatus = ItemStatus.toUpperCase().trim();
        if (_xItemStatus == "GIVEN") {Given = '\u2714';} else {Given = " ";}
        if (_xItemStatus == "FOLLOWUP") {FollowUp = '\u2714';} else {FollowUp = " ";}
        PharmacyVerified = item.orders[this.itemOrderSelected].pharmacyVerificationStatus;
        if (PharmacyVerified == "0") {PharmacyVerified = " "} else {PharmacyVerified = '\u2714';}
        
      }
      this.administeredItems.push(
        {"TableRow": TableRow,
         "ItemTime": ItemTime,
         "ItemDate": ItemDate,
         ...
         "ItemLinkDateTime": ItemLinkDateTime}
         );
      itemSelectedCount++;
    }

  }
//    this.administeredItems[numberOfItem].Time= 
// alert (TableRow);
// alert ( this.administeredItems[0].ItemTime);
  }

  catch (error  ) {
    this.itemOrderSelected = 0;
    }
var _itemDisplayName = " ";
if ( item.orders[this.itemOrderSelected].medication.displayName != null) {
  _itemDisplayName = item.orders[this.itemOrderSelected].medication.displayName;
}
var _itemDose = " ";
if ( item.orders[this.itemOrderSelected].dose != null) {
  _itemDose = item.orders[this.itemOrderSelected].dose;
}
var _itemUnitName = " ";
if (item.orders[this.itemOrderSelected].doseUnit != null
  && item.orders[this.itemOrderSelected].doseUnit.unitName != null) {
  _itemUnitName = item.orders[this.itemOrderSelected].doseUnit.printName;
}
var _itemRouteName = " ";
if (item.orders[this.itemOrderSelected].medicationRoute.routeName != null) {
  _itemRouteName = item.orders[this.itemOrderSelected].medicationRoute.routeName;
}
var _itemScheduleName = " ";
if (item.orders[this.itemOrderSelected].frequencySchedule.scheduleName != null) {
  _itemScheduleName =item.orders[this.itemOrderSelected].frequencySchedule.scheduleName;
}
 
    this.modalMedicationSelected  = _itemDisplayName  ;
    this.modalMedicationSelectedSchedule = "Dose: " +
    _itemDose + " " +  _itemUnitName + ", "  + _itemRouteName + ", " + 
        _itemScheduleName;     
        
     
    

    this.modalService.open('patientAdministrationInfo');
    
    //   action: 'add',);
  }
  */

  getPatientAdministrationsModalDateRequests(returnedAdministoredLinkDateTime: string) {
    // close all the administrations popups
    this.popAdministrations.forEach((popup) => popup.close());
    if (returnedAdministoredLinkDateTime != null) {
      this.moveTimes(0, true, returnedAdministoredLinkDateTime);
      this.setIntervals();
      this.refreshScreen();
    }
 }
  ...
  cosignNeeded(administration: OrderAdministration, order: Order) {
    if (!order.cosignNeeded) {
      return false;
    }
    return !administration.administrationEvents.find(event => event.action.actionId == 5);
  }
  estimatedEnd(order: Order): string {
    if ((order.endDatetime == '' || order.endDatetime == null) && order.durationUnit != null) { // last scheduled administration
      const administration = order.orderAdministrations.reduce(function (result:OrderAdministration, current) {
        if (result == null) return current;
          return moment(result.administrationScheduledDatetime).isBefore(moment(current.administrationScheduledDatetime))? current: result;
        }, null);
        return administration == null? '' : administration.administrationScheduledDatetime;
    } else {
      return order.endDatetime;
    }
  } 
}
