...
export class MyPatientsStoreService {
  ...
  constructor(
    ...
    this.userStoreService.user$.subscribe( async() => {
      ...
      if (userId && userSiteId) {
        this.fetchMyPatients(userId, userSiteId)
      }
    })
  ...
  async fetchMyPatients(userId: number, userSiteId: number) {

    console.log('MyPatientsStoreService: fetchMyPatients')
    this.myPatientsResp = await this.patientService.getMyPatients(userSiteId, userId).toPromise()
