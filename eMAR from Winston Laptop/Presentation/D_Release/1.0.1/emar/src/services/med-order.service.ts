...
export class MedOrderService {
  ...
  getMedListBySelectedTab(tab: string) {
    ...
    const userQuickListsByTabUrl = `${this.userQuickListsUrl}/tabs/${tab}?r=${Math.random()}`
  ...
  postCartOrderByDeptPreferredListOrderId(listOrderId: number, patientId: number, userId: number): Observable<any> {

    const headers = new HttpHeaders({ Accept: 'application/json', 'EMAR-User': `${userId}`, 'EMAR-Site': `${this.userStoreService.userSiteId}`, 'EMAR-Patient': `${this.patientStoreService.patientId}`})
    const url = `/api/patients/${patientId}/departmentPreferredLists/${listOrderId}/cartOrders`
  ...
  postCartOrderByGroupsListOrderId(listOrderId: number, patientId: number, userId: number): Observable<any> {
    ...
    const url = `/api/patients/${patientId}/groupsrememberedorderslists/${listOrderId}/cartOrders`
  ...
  brandNameSearch(term: string, source: string) {
    ...
    const url = `/api/BrandNameList/${term}/${source}`
