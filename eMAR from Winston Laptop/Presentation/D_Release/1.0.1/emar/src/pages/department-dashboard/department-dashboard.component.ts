import { Component, OnInit, AfterViewInit, ViewEncapsulation, ViewChild, ElementRef, ComponentFactoryResolver } from '@angular/core';
...
export class DepartmentDashboardComponent implements OnInit, AfterViewInit {
    ...
    nbPatients: TypeCount = { upcomingOrders: 0, myPatients: 0, all: 0};
  ...
  ngOnInit(): void {
      ...
      console.log('MAR DEPT: (ngOnInit)')
      
      // const mp = this.departmentStoreService.myPatientsResp
      // console.log('MAR DEPT: (ngOnInit) mmmmmmmmp: ', mp)
      this.myPatientsStoreService.myPatients$.subscribe(async (resp) => {
      this.fetching = true;
      this.fetchingMsg = '... Preparing My Patients patient list...'
      ...
      this.myPatientsStoreService.myPatientsOrders$.subscribe(async (resp) => {
      ...
      this.allPatientsStoreService.allPatients$.subscribe(async (resp) => {
        // console.log('MAR DEPT: (ngOnInit)(subscribe allPatient$) allPatients resp: ', resp)
        this.fetchingMsg = '... Preparing Upcoming Orders patient list...'
      ...
      this.allPatientsStoreService.allPatientsOrders$.subscribe(async (resp) => {
  ...
  consolidateOrderAdministrationsForPatient(patientId, patientOrders = []) {
    ...
    for (const order of patientOrders) {
      for (const admin of order.orderAdministrations) {
        // pcoa[admin.administrationScheduledDatetime] = (pcoa[admin.administrationScheduledDatetime] || 0) + 1
        pcoa[admin.administrationScheduledDatetime] = pcoa[admin.administrationScheduledDatetime] || {count: 0, leastStatusAdmin: {}}
        
        if (pcoa[admin.administrationScheduledDatetime].count === 0) {
          // pcoa[admin.administrationScheduledDatetime].count = pcoa[admin.administrationScheduledDatetime].count + 1
          pcoa[admin.administrationScheduledDatetime].leastStatusAdmin = admin

        } else {
          // compare the administration status for the "least"
          // Late / Pending / OnHold / Ongoing (given but not completed) / Missed (not going to give) / Given
          const currentLeastStatusAdmin = pcoa[admin.administrationScheduledDatetime].leastStatusAdmin

          if (currentLeastStatusAdmin.administrationStatus.toLowerCase() !== 'late' && admin.administrationStatus.toLowerCase() === 'late') {
            pcoa[admin.administrationScheduledDatetime].leastStatusAdmin = admin

          } else if (admin.administrationStatus.toLowerCase() === 'pending') {
            if (currentLeastStatusAdmin.administrationStatus.toLowerCase() !== 'late') {
              pcoa[admin.administrationScheduledDatetime].leastStatusAdmin = admin
            }

          } else if (admin.administrationStatus.toLowerCase() === 'onhold') {
            if (currentLeastStatusAdmin.administrationStatus.toLowerCase() !== 'late' 
                 && currentLeastStatusAdmin.administrationStatus.toLowerCase() !== 'pending') {
              pcoa[admin.administrationScheduledDatetime].leastStatusAdmin = admin
            }
            ...
            if (currentLeastStatusAdmin.administrationStatus.toLowerCase() !== 'late' 
                && currentLeastStatusAdmin.administrationStatus.toLowerCase() !== 'pending' 
                && currentLeastStatusAdmin.administrationStatus.toLowerCase() !== 'onhold') {
              pcoa[admin.administrationScheduledDatetime].leastStatusAdmin = admin
            }
            ...
            if (currentLeastStatusAdmin.administrationStatus.toLowerCase() !== 'late' 
                && currentLeastStatusAdmin.administrationStatus.toLowerCase() !== 'pending' 
                && currentLeastStatusAdmin.administrationStatus.toLowerCase() !== 'onhold'
                && currentLeastStatusAdmin.administrationStatus.toLowerCase() !== 'ongoing') {
              pcoa[admin.administrationScheduledDatetime].leastStatusAdmin = admin
            }
          }
        }
        ...
        pcoa_orderInfo[admin.administrationScheduledDatetime] = pcoa_orderInfo[admin.administrationScheduledDatetime] || []
        pcoa_orderInfo[admin.administrationScheduledDatetime].push(order)
      }
    }
  ...
  countPatientOrders(patientId: number): void {
    ...
    let nbOrders: TypeCount = {all:0, prn:0, stat:0, scheduled:0, continuous:0, timed:0, iv:0, ancilliary:0};
  ...
  onFilter(type: string): void {
    ...
    // this.prepare()  // no need to re-count/re-consolidate the administrations/orders 
    this.patientsList = {...this[this.filter]}
    this.patientOrders = { ...this[`${this.filter + 'PatientOrders'}`]}
