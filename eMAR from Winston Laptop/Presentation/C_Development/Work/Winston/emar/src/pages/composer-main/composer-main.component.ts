...
export class ComposerMainComponent implements OnInit, AfterViewInit, AfterContentChecked, OnDestroy {
  ...
  subscriptionStrengthClosing: Subscription = null;
  ...
  ngOnInit(): void {
    ...
    this.subscriptionStrengthClosing = this.modalService.modalClosing.subscribe( (modal: ModalComponent) => {
      if (modal.modalId == 'strengthModal')  {
        this.cancelOrder();
      }
    })
  ...
  ngOnDestroy() {
    ...
    if (this.subscriptionStrengthClosing !== null) this.subscriptionStrengthClosing.unsubscribe();
  ...
  async getRequiredAPIData() {
    ...
    let brandNameRequest;
    const sourceType = this.initialComposerData.sourceType?.toLowerCase() === 'all' ? 'all' : null;
    if (this.initialComposerData.source === 'med-search') {
      // remember we go to composer from search
      this.medOrderService.setCurrentTab('med-search','');
      // use the url api is giving
      brandNameRequest = this.composerSchedulerService.getSearchMedOptionsFromAPI(
        this.initialComposerData.med.brandName,
        this.initialComposerData.med.link.href,
        this.userSiteId)
      .toPromise();
    } else {
      // build the url
      ...
      // remember we go to composer from a tab
      this.medOrderService.setCurrentTab(rememberedListItemType, this.initialComposerData.med.groupName);

      brandNameRequest = (rememberedListItemType) ?
