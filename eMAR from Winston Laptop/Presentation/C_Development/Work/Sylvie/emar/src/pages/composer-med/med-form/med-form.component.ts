...
export class MedFormComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    if (!this.medOptions) {
      ...
      if (this.medOptions.availableFormStrength.length > 1 && !this.medComponentId) {
        this.modalService.open(
          'strengthModal',
          {strengths: this.medOptions.availableFormStrength},
          '<span class="bigger-bolder">Select Strength & Form</span>'
        )
      }
    ...
    this.composerSchedulerService.selectPopupStrengthEvent
    .subscribe( (i:number) => {
      this.changeSelectedStrength(i, true); // need to refresh to get preferrd frequencies for instance
    })
  }
