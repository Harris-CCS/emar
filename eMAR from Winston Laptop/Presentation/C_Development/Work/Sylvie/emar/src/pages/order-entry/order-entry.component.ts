...
export class OrderEntryComponent implements OnInit {
  ...
  dptSelected: boolean = false;
  gSelected: boolean = false;
  gSelectedGroup: string = '';

  hasDeptPreferredDefined: boolean = true;
  ...
  ngOnInit(): void {
    ...
    this.medOrderService.getCurrentTab().subscribe ( (tab) => {
      if (tab[0] == 'Groups' || tab[0] == 'GroupRememberedOrder') {
        this.qlSelected = false;
        this.gSelected = true;
        if (tab[1] != '') {
          this.gSelectedGroup = tab[1];
        }
      }
    ...
    this.timeZoneOffset = this.userStoreService.userSite.timeZoneOffset;
  }
  ...
  onSelectTab(tab: string) {
    if (tab === 'list-dept-preferred-orders') {
      ...
      this.dptSelected = false;
      this.gSelected = false;
    } else if (tab === 'list-dept-preferred-orders-with-tab') {
      this.qlSelected = false;
      this.dpSelected = false;
      this.dptSelected = true;
      this.gSelected = false;
    }  
  ...
  // Is order active depending on status
