...
export class SiteStoreService {
  ...
  setTimeout() {
    const delay = this.site.siteOptions.session_timeout * 60 * 1000;
    // const delay = 5000; // 5s
    if (delay) {
      const url = this.site.siteOptions.session_timeout_url.includes('http')? this.site.siteOptions.session_timeout_url: '/timeout';
  ...
  @HostListener('window:mousemove') active() {
