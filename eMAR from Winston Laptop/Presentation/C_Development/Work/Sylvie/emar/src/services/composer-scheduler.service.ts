...
export class ComposerSchedulerService {
  ...
  @Output() selectPopupStrengthEvent = new EventEmitter<number>();
  ...
  async saveOrderToUserQuickList(medComponentId: number, userId: number, siteId: number): Promise<boolean> {
    ...
    const rememberedListOrder: UserRememberedListOrder = {
      ...
      prnIndication: order.composerMedForm.value.frequency.frequencyData.prn ? order.composerMedForm.value.frequency.prnIndicationDescription : '',
  ...
  // HTTP
  // get data for composer from a med search result
  getSearchMedOptionsFromAPI(
    brandName: string,
    url: string,
    siteId: number
  ): Observable<any> {
    const headers = new HttpHeaders({ Accept: 'application/json', 'EMAR-Site': `${siteId}` , 'brandName': `${brandName}`});
    // CORS problem: let the proxy do its job in case api returns full url.
    const idx = url.indexOf('/api');
    if (idx > 0) url = url.substr(idx);
    if (url.indexOf('r=') < 0) url = url + '?r=' + Math.random();
    return this.http
    .get<any>(url, { headers })
    .pipe(
      catchError(this.handleError<any>('getSearchMedOptionsFromAPI'))
    );
  }

  // get data for composer for other than med search
  getBrandNameOptionsFromAPI(
    ...
    const optionsPredicate: string = (rememberedListItemType && rememberedListItemId) ?
      ...
      `/site/${siteId}${sourceType ? `/${sourceType}` : ''}?r=${Math.random()}`;
  ...
  getPRNIndicationsFromAPI(siteId: number): Observable<any> {
    const headers = new HttpHeaders({ Accept: 'application/json', 'EMAR-Site': siteId.toString() });
    const url = `api/PrnIndications/site/${siteId}`;
    return this.http
      .get<any>(url, { headers })
      .pipe(
        catchError(this.handleError<any>('getPRNIndicationsFromAPI'))
      );
  }
  ...
  getPatientOrderInteractionsFromAPI(
    ...
    const fullPatientOrderInteractionUrl = `${this.patientOrderInteractionBaseUrl}/${orderItemType}/${orderItemNumber}?r=${Math.random()}`;
    // console.log(
  ...
  getFrequencyAdministrationsFromAPI(
    ...
    if (typeof duration !== 'undefined' && duration != null && typeof durationUnitId !== 'undefined' && durationUnitId != null) {
      let d = 'duration=' + duration + '&durationUnitId=' + durationUnitId;
      paramString = paramString ? `${paramString}&${d}`: `?${d}`;
    }
    ...
    return this.http
      ...
      .pipe(tap(data => {
        // console.log('ADMINISTRATIONS',frequencyAdministrationsFullUrl, data)
      }))
      .pipe(catchError(this.handleError<any>('getFrequencyAdministrationsFromAPI'+frequencyAdministrationsFullUrl)));
  ...
  setBrandNameOptions(options: ComposerOptions): void {
    ...
    if (options !== undefined) options.availableFormStrength = newAvailableFormStrength 
  ...
  // when selecting a strength from the popup emit an event that is listened by med-form
  selectPopupStrength(i: number) {
    this.selectPopupStrengthEvent.emit(i);
  }
}
