...
export class ComposerMedModalComponent implements OnInit, AfterViewInit, AfterContentChecked {
  ...
  ngOnInit(): void {
    this.modalService.modalOpening.subscribe(({ data }) => {
      ...
      this.composerSchedulerService
        .getBrandNameOptionsFromAPI(this.userSiteId, this.initialData.med.medication.displayName)
        .subscribe((response) => {
          if (
            this.initialData &&
    ...
    this.modalService.modalClosing.subscribe(({ modalId }) => {
      // if (this.modalService.modalClosed.value === 'medComposer') {
      if (modalId === 'medComposer') {
