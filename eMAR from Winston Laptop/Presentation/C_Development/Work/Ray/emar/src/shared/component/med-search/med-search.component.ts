...
export class MedSearchComponent implements OnInit, OnDestroy {
  ...
  onSelect($event, input, sourceType) {
    ...
    //   med: $event.item,
    // });
    this.launchMedComposer($event.item.brandName, $event.item, sourceType);
    input.value = '';
    input.blur();
  }

  launchMedComposer(medBrandName: string, medData: object, sourceType): void {
    this.composerSchedulerService.setInitialComposerData({ action: 'add', source: 'med-search', med: medData, sourceType: sourceType });
