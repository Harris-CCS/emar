...
export class MedOrderService {
  ...
  // old version when getting list groups with all detail
  private groupListUrl = `/api/sites/${this.userStoreService.userSiteId}/groupsrememberedorderslists`
  private groupNameListUrl = `/api/sites/${this.userStoreService.userSiteId}/groupNames`;
  private groupDetailUrl = `/api/sites/${this.userStoreService.userSiteId}/groupdetails`;
  ...
  getGroupNameList(): Observable<string[]> {
    const headers = new HttpHeaders({ 
      Accept: 'application/json', 
      'EMAR-Site': `${this.userStoreService.userSiteId}`,
      'EMAR-User': this.userStoreService.userId?.toString(),
    ...
    return this.http
      .get<any>(`${this.groupNameListUrl}`, { headers })
      .pipe(
        tap(_ => console.log('med-order.service: getGroupNamesList HTTP Client - GET')),
        catchError(this.handleError<any>('getGroupNamesList', []))
      );
  }
  getGroupDetail(groupName:string): Observable<any> {
    const headers = new HttpHeaders({ 
      Accept: 'application/json', 
      'EMAR-Site': `${this.userStoreService.userSiteId}`,
      'EMAR-User': this.userStoreService.userId?.toString(),
    ...
    return this.http
      .get<any>(`${this.groupDetailUrl}?r=${Math.random()}`, { headers })
      .pipe(
        tap(
          res => console.log('med-order.service: getGroupDetail HTTP Client - GET',res)),
        map(o => o.groups[0]),
        map( (group) => ({
          ...group,
          displayGroupName: group.groupName,
          orders: group.orders.map( (x) => ({
          ...x,
        ...
        tap(
          res => console.log('med-order.service: getGroupDetail HTTP Client - GET',res)),
        catchError(this.handleError<any>('getGroupDetail', []))
      );
  }
  ...
  brandNameSearch(term: string, source: string) {
    ...
    const headers = new HttpHeaders({ 
      ...
      //  ignore for now as the dept quick list does not filter
      // 'EMAR-PatientDepartment': `${this.patientStoreService.patientDeptCode}`,
      'brandName': `${term}`
    })
    // ${encodeURIComponent(term)}
    const url = `/api/BrandNameList/${source}/site/${this.userStoreService.userSiteId}?r=${Math.random()}`
  ...
  setCurrentTab(tab:string, group:string) {
    this.currentTab = tab;
    this.currentGroup = group;
  }

  getCurrentTab(): Observable<string[]> {
    return of([this.currentTab,this.currentGroup]);
  }
