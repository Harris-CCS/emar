...
export class DepartmentDashboardComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor (
    ...
    // Retrieve dept page filter from session storage
    this.deptPageFilter = this.userStoreService.userDeptPageFilter
    ...
    // API POST DEPARTMENT_PAGE_FILTERING is not ready.  Use session storage to preserve the user setting for each session temporarily
    if (this.deptPageFilter === null) {

      this.deptPageFilter = this.userStoreService.DEPARTMENT_PAGE_FILTERING ? 
      this.userStoreService.DEPARTMENT_PAGE_FILTERING === 'M' ? 'myPatients' :
      this.userStoreService.DEPARTMENT_PAGE_FILTERING === 'V' ? 'rxVerificationNeeded' :  
      // this.userStoreService.DEPARTMENT_PAGE_FILTERING === 'U' ? 'upcomingOrders' :
      this.userStoreService.DEPARTMENT_PAGE_FILTERING === 'A' ? 'all' : 'upcomingOrders'
      : 'upcomingOrders'  // default
  ...
  ngOnInit(): void {
      ...
      this.myPatientsStoreService.myPatients$
        ...
        .subscribe(async (resp) => {
          ...
          if (this.myPatients?.patients.length && this.deptPageFilter === 'myPatients') {
            // this.countPatients()
            // this.nbPatients.myPatients = this.myPatients?.patients?.length;
            this.filter = 'myPatients'
            console.log('MAR DEPT: (ngOnInit)(subscribe NEW myPatients$): you have "My Patients"')
            console.log('MAR DEPT: (ngOnInit)(subscribe NEW myPatients$) going to prepare()')
            // this.nbPatients.myPatients = this.myPatients.patients.length;
            this.prepare()

            console.log('MAR DEPT: (ngOnInit)(subscribe NEW myPatients$): going to refresh()')
            this.refresh();
          }
      ...
      */
      ...
      this.pharmVerificationStoreService.pharmVerificationPatients$
        ...
        .subscribe(async (resp) => {
          ...
          if (this.rxVerificationNeeded?.patients.length && this.deptPageFilter === 'rxVerificationNeeded') {
...
*/
      ...
      this.allPatientsStoreService.allPatients$
        ...
        .subscribe(async (resp) => {
          ...
          if ( !(this.deptPageFilter === 'myPatients' && this.myPatients?.patients.length) && 
               !(this.deptPageFilter === 'rxVerificationNeeded' && this.rxVerificationNeeded?.patients.length) ) {

            if (this.deptPageFilter === 'all') {
              this.filter = 'all'
            } else {
              this.filter = 'upcomingOrders' // it is also the default
            }
 ...
 */       
  ...
  consolidateOrderAdministrationsForPatient(patientId, patientOrders = []) {
    ...
    for (const order of patientOrders) {
      for (const admin of order.orderAdministrations) {
        ...
        const time = (admin.administrationDatetime != '' && admin.administrationDatetime != null)? admin.administrationDatetime: admin.administrationScheduledDatetime;
        const adminTimeHour = moment(time).hour()
        const minuteGroup = Math.floor(moment(time).minutes() / this.minutesPerGroup) * this.minutesPerGroup
        const groupTime = moment(time).set('hour', adminTimeHour).set('minute', minuteGroup).set('second', 0).format()
        ...
        if (pcoa[groupTime].adminCount === 0) {
          ...
          // Late / Pending / OnHold / Ongoing (given but not completed) / Missed (not going to give) / Given without follow-up / Given with Follow-up
  ...
  onFilter(type: string): void {
    ...
    this.userService.setFilterSetting(this.filter, this.userSiteId, this.userId);
    // store in session
    this.userStoreService.userDeptPageFilter = this.filter;
  }
  ...
  // Test if all the administrations of this groupTime have a followUp event
  hasFollowUp(administration): Boolean {
    let admin:OrderAdministration;
    for (admin of administration.value.admins) {
      if (!admin.administrationEvents.find(event => event.action.actionId == 7)) {
        return false;
      }
    }
    return true;
