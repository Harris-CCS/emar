...
export class SignCartOrderModalComponent implements OnInit, OnDestroy {
  ...
  get hasValidationErrors() : boolean {
    return this.hasProviderErrors() || this.hasInteractionErrors();
  }
  hasInteractionErrors(): boolean {
    for (const ord of this.cartOrders) {
      for (const reaction of ord.allergyReactions) {
        if (!(reaction.id in this.selectedAllergyReactionReasonMap)) {
          // console.log("all interaction missing:", reaction, Object.keys(this.selectedAllergyReactionReasonMap))
          return true;
        }
      }
      for (const reaction of ord.orderInteractions) {
        if (!(reaction.medicationInteractionId in this.selectedDrugInteractionReasonMap)) {
          // console.log("med interaction missing", reaction, Object.keys(this.selectedDrugInteractionReasonMap))
          return true;
        }
      }
    }
    ...
    const interactionMap = preparation.drugInteractionOrders.reduce( (prev, ord) => {
      ...
      ord.orderInteractions.forEach( interaction => {
        const obj = prev[interaction.medicationInteractionId] || {
          ...
          interactionOrderId: interaction.drugInteraction.interactionOrderId,
