                ...
                using (var con = new SqlConnection(connection))
                {
                    ...
                    for (int t = 0; t < 100; t++)
                    {
                        try
                        {
                            ...
                            // Assign cmdParams a new List each time through using cloned parameters
                            // This prevents cmdParams and parameters from referencing the same memory (same pointer)
                            cmdParams = new List<SqlParameter>(parameters.Select(x => x?.Clone()))
                            {
                                losecsParam
                            };
                            var result = new DB.Insert()
