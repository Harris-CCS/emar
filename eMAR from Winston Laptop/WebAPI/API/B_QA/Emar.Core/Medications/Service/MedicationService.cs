...
namespace Emar.Core.Medications.Service
{
    public class MedicationService : IMedicationService
    {
        ...
        public List<MedicationInteractionReaction> GetPatientCartOrderInteractionsReactions(int userId, long patientId, int itemId)
        {
            //Calculate the interactions and reactions for a specific patient cart order.
            //If it's a combo med, then we'll need to go to the MedicationDetails children,
            //get the actual Medication for them, and then call CheckInteractionReactions for each one of them.
            ...
            //I found a workaround by changing the connection string to allow that.
            //But I'd rather not make a global change like that.
            //So I'll make a new repository method and call it for each medication detail.
            foreach (var md in cartOrder.Medication.MedicationDetails)
            {
                md.FdbBrandName = _cartOrderRepository.GetFdbBrandNameForCartOrder(md.Id, patient.SiteId);
            } //end foreach.
            ...
            //instance of that combo med.
            //Winston Murdock, 09/06/2022.  PC-27429
            orders = orders.Where(o => o.MedicationId != cartOrder.MedicationId);
            ...
            //Remove any cart orders that have the same medication id.
            //This way we don't check a combo med against another instance of itself.
            var cartOrdersMinusThisOne = cartOrders.Where(co => co.MedicationId != cartOrder.MedicationId);
            ...
            if (cartOrder.Medication.DrugId == "COMBO")
            {
                ...
                //Write a helper function to accomplish this.
                //Winston Murdock, 04/27/2022.
                interactionsReactions = CompressPatientCartOrderComboMedInteractionsToOneEntry(cartOrder, userId, interactionsReactions);
            }
            else
            {
                //Not a combo med.
                ...
                interactionsReactions = _orderService
                    .CheckInteractionsReactions(
                        ...
                        new List<MedicationModel>
                        {
                            OrderMapper.MapOrderItemToModel(EmarOrderType.PatientCartOrder, cartOrder, patientId, userId, codeShareSiteMedicationUnit)
                        },
                        patientId,
                        true,
                        null,
            ...
            return interactionsReactions;
        } //end GetPatientCartOrderInteractionsReactions

        public List<MedicationInteractionReaction> CompressPatientCartOrderComboMedInteractionsToOneEntry(PatientCartOrder cartOrder, int userId, List<MedicationInteractionReaction> currentList)
        {
            //When a patient cart order is a combo med, we need to compress the interactions/reactions
            //into one entry rather than one entry for each medication detail within the combo med.
            //Winston Murdock, 02/21/2023.  PC-27804
            ...
            MedicationInteractionReaction mir = new MedicationInteractionReaction
            {
                SiteId = cartOrder.Patient.SiteId,
                UserId = userId,
                SourceTable = "patient_cart_orders",
                SourceTableId = cartOrder.Id,
                Type = EmarOrderType.PatientCartOrder,
                BrandName = cartOrder.Medication.DisplayName,
                ActiveName = cartOrder.Medication.DisplayName,
                ActiveId = cartOrder.Id.ToString() //We don't have an FDB id here.  So possibly the group's ID will work?
            };
        ...
        public List<MedicationInteractionReaction> GetPatientOrderInteractionsReactions(int userId, long patientId, int itemId)
        {
            //Calculate the interactions and reactions for a specific patient cart order.
            //If it's a combo med, then we'll need to go to the MedicationDetails children,
            //get the actual Medication for them, and then call CheckInteractionReactions for each one of them.
            ...
            var order = _orderRepository.GetOrder(itemId);
            var patient = _patientRepository.GetPatient(order.PatientId, null, false);
            ...
            //But I'd rather not make a global change like that.
            //So I'll make a new repository method and call it for each medication detail.
            foreach (var md in order.Medication.MedicationDetails)
            {
                md.FdbBrandName = _cartOrderRepository.GetFdbBrandNameForCartOrder(md.Id, patient.SiteId);
            } //end foreach.
            ...
            //instance of that combo med.
            //Winston Murdock, 09/06/2022.  PC-27429
            orders = orders.Where(o => o.MedicationId != order.MedicationId);
            ...
            //Remove any cart orders that have the same medication id.
            //This way we don't check a combo med against another instance of itself.
            var cartOrdersMinusThisOne = cartOrders.Where(co => co.MedicationId != order.MedicationId);
            ...
            if (order.Medication.DrugId == "COMBO")
            {
                ...
                foreach (var medDetail in order.Medication.MedicationDetails)
                {
                    //Get the actual Medication for this one (not the combo med).
                    //It will match on drug_id.
                ...
                //Write a helper function to accomplish this.
                //Winston Murdock, 04/27/2022.
                interactionsReactions = CompressPatientOrderComboMedInteractionsToOneEntry(order, userId, interactionsReactions);
            }
            else
            {
                //Not a combo med.
                ...
                interactionsReactions = _orderService
                    .CheckInteractionsReactions(
                        ...
                        new List<MedicationModel>
                        {
                            OrderMapper.MapOrderItemToModel(EmarOrderType.PatientOrder, order, patientId, userId, codeShareSiteMedicationUnit)
                        },
                        patientId,
                        true,
                        null,
        ...
        public List<MedicationInteractionReaction> CompressPatientOrderComboMedInteractionsToOneEntry(PatientOrder order, int userId, List<MedicationInteractionReaction> currentList)
        {
            //When a patient cart order is a combo med, we need to compress the interactions/reactions
            //into one entry rather than one entry for each medication detail within the combo med.
            //Winston Murdock, 02/21/2023.  PC-27804
            ...
            MedicationInteractionReaction mir = new MedicationInteractionReaction
            {
                SiteId = order.Patient.SiteId,
                UserId = userId,
                SourceTable = "patient_orders",
                SourceTableId = order.Id,
                Type = EmarOrderType.PatientCartOrder,
                BrandName = order.Medication.DisplayName,
                ActiveName = order.Medication.DisplayName,
                ActiveId = order.Id.ToString() //We don't have an FDB id here.  So possibly the group's ID will work?
            };
        ...
        public List<PrnIndicationDto> GetPrnIndicationsBySiteId(int siteId)
        {
            //Might filter by site id at some point.
            //If we do, we'll probably code share the indications in the DB,
            //and I would need to change this method to grab the site id
            //that we're code sharing from and pass that down rather than
            //the site id that the user is logged in to.
            var prnIndications = _medicationRepository.GetPrnIndicationsBySiteId(siteId);

            //Sylvie confirms that the UI needs id and optionDescription fields.
            //I have changed this (and the interface) to return a list of DTO object.
            //Then I commented out the SiteId field in the DTO and mapper.
            //Lastly, I changed this method to call the mapper (and to sort by optionDescription).
            var ret = prnIndications.Select(x => MedicationMapper.MapPrnIndication(x)).OrderBy(x => x.OptionDescription).ToList();
