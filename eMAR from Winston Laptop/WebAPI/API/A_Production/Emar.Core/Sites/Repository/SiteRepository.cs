...
namespace Emar.Core.Sites.Repository
{
    public class SiteRepository : ISiteRepository
    {
        ...
        {
            return _context.Sites.Find(siteId).TimeZoneName.NowWithTimeZoneOffset();
        }

        public string GetTimeZoneOffsetByTimeZoneName(string timeZoneName)
        {
            try
            {
                var timeZoneList = _context.StringLists.FromSqlInterpolated($"exec get_time_zone_offset {timeZoneName}").ToList();
                string soffset;
