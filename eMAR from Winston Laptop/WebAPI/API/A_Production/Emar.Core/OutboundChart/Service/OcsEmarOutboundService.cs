...
namespace Emar.Core.OutboundChart.Service
{
    public class OcsEmarOutboundService : IOcsEmarOutboundService
    {
        ...
        public string SendChartLinesAsync(OcsChartParameters ocsChartParams)
        {
            ...
            //Use siteNow instead of now.
            //var now = DateTime.Now.ToString("yyyyMMddHHmmss"); // note: not part of CreateMeds

            //Get the timezone of the current site.
            //ocsChartParams.Site is the eMAR Site ID.
            string timeZoneName = _siteRepository.GetSiteTimeZone(ocsChartParams.site);

            //Get now in the current site's time zone.
            var siteNow = timeZoneName.NowWithTimeZoneOffset(); // note: not part of CreateMeds

            //format siteNow as PulseCheck expects it to be formatted.
            string siteNowFormatted = GetIbexFormatDateTimeFromDTO(siteNow.ToString("yyyyMMddHHmmss"), siteNow, true);
            ...
            using (var con = new SqlConnection(connection))
            {
                ...
                try
                {
                    ...
                    foreach (var medication in meds)
                    {
                        ...
                        var line = _emarOutboundChartRepository.ChartEntry(patient, medication, siteNowFormatted, ocsChartParams.PharmVerifStatus, ocsChartParams.PRNIndication);
                        emrLines.Add(line);
            ...
            if (ordersPlaced)
            {
                // Regarding MU, still need the corresponding 'ACESSED' log though

                //We formatted siteNow to the format PulseCheck needs up at the top of this method.
                //Use that formatted version of it here.
                MeaningfulUse.LogCreation(user, patient.Ibex, "MEDICATION SERVICE", activePatient, siteNowFormatted);
            ...
            // qlMedNums not currently being set - see ibex4q and its use of %FTIT
        ...
        private void CreateTrigger(ISite site, Patient patient, int userId, string action, List<Medication> meds, string opt = null)
        {
            try
            {
                ...
                {
                    // verify the query results before making further assignments
                    if (r.ContainsKey("field_num")  && !string.IsNullOrWhiteSpace(r["field_num"])
                     && r.ContainsKey("field_name") && !string.IsNullOrWhiteSpace(r["field_name"])
                     && r.ContainsKey("field_val")  && !string.IsNullOrWhiteSpace(r["field_val"]))
                    {
                ...
                foreach (var triggerId in triggerSettings.Keys)
                {
                    ...
                    if ((opt.Equals(Trigger.Constants.TRIGGER_MED_SVC_BOTH) || opt.Equals(triggerId)) && ((patAge == 0 && patAgeUnit == Patient.Constants.AGEUNIT_DAYS) || (minAge <= patAge && maxAge >= patAge)))
                    {
                        ...
                        // only proceed with trigger handling if a valid formulary object was created
                        // at the moment, HasFatalErrors() will always return false until Formulary.cs is updated
                        //     - see _add_fatal_error in Formulary.pm of PCED code
                        if (formulary != null && !formulary.HasFatalErrors())
                        {
                        ...
                        {
                            // formulary error occurred so log it
                            Utilities.LogError(patient.Ibex, site.Id, 0, 0, "CreateTrigger error with formulary for med with losecs " + meds[0].Losecs, action);
                        }
                    }
                }
            }
            ...
            {
                Utilities.LogError(patient.Ibex, site.Id, 0, 0, "CreateTrigger exception for med with losecs " + meds[0].Losecs, action);
                Utilities.LogException(ex);
        ...
        private void AddInteractionsAndReactions(Medication med, OcsChartParameters ocsChartParams)
        {
            ...
            foreach (var comp in med.Components)
            {
                ...
                var interactionDrugList = new List<string>();

                foreach (var interaction in orderInteractions)
                {
                    var view = interaction.DrugInteractionView;
                    // Since it is possible that the view may have duplicate rows, skip any found dups
                    // Assign the combo of drug1 and drug2 with a separating delimiter as a string, drugPair
                    var drugPair = view.InteractionDrug1 + '|' + view.InteractionDrug2;
                    if (interactionDrugList.Contains(drugPair))
                    {
                        // skip since a duplicate
                        continue;
                    }
                    else
                    {
                        // add to list since unique
                        interactionDrugList.Add(drugPair);
                    }
                    var rsel = new Dictionary<string, string>();
                    var sev = ((FdbInteractionSeverity)view.Severity).ToString();
                    rsel["dnum"] = !string.IsNullOrWhiteSpace(view.InteractionDrug2) ? view.InteractionDrug2 : null;
                    rsel["drug"] = view.InteractionOrderName;
                    rsel["type"] = "drug";
                    rsel["interaction"] = sev + " INTERACTION";
                    rsel["sevtxt"] = sev;
                    rsel["override_reason"] = _emarOutboundChartRepository.GetOverrideReason(interaction.MedicationInteraction.OverrideReasonId);
                    acc_inters.Add(rsel);
                }
                ...
                if (allergyReactions != null)
                {
                    ...
                    {
                        var rsel = new Dictionary<string, string>();
                        rsel["dnum"] = _emarOutboundChartRepository.GetAllergyDrugIdFromPatientAllergyId(reaction.PatientAllergyId);
                        ...
                        rsel["override_reason"] = _emarOutboundChartRepository.GetOverrideReason(reasonId);
                        acc_reacts.Add(rsel);
                    }
                }
        ...
        public string SendChartTemplateMarkup(List<OcsPromptParameters> orderedPromptList, int siteId, long patientOrderId, int userId, ActionEnum action, long adminId, bool newOrderAdmin)
        {
            try
            {
                ...
                // check if calculated proper values
                if (patId <= 0 || string.IsNullOrWhiteSpace(patIbex) || externalSiteId == 0 || externalUserId <= 0)
                {
                    var errMsg = string.Format("Invalid values in SendChartTemplateMarkup: patId={0}, patIbex={1}, externalSiteId={2}, externalUserId={3}"
                                                , patId, patIbex, externalSiteId, externalUserId);
                    Utilities.LogError(patIbex, externalSiteId, patientOrderId, adminId, errMsg, action.ToString());
                    // Should we return here since something went terribly wrong?
                }
                // determine if active patient by trying to retrieve patient from pat table
                ...
                if (action == ActionEnum.Give)
                {
                    ...
                    {
                        Utilities.LogError(patIbex, externalSiteId, patientOrderId, adminId, ret, action.ToString());
                    }
                }
                ...
                {
                    var ret = updatePatientMedicationIndicator(patId, patIbex, externalSiteId, action, adminId);
                    if (!string.IsNullOrWhiteSpace(ret))
                    {
                        Utilities.LogError(patIbex, externalSiteId, patientOrderId, adminId, ret, action.ToString());
                    }
                ...
                // check if valid losecs
                if (string.IsNullOrWhiteSpace(losecsFromOrder))
                {
                    var errMsg = "Could not get losecs from the patient order id in SendChartTemplateMarkup";
                    Utilities.LogError(patIbex, externalSiteId, patientOrderId, adminId, errMsg, action.ToString());
                    // Should we return here since something went terribly wrong?
                }
                var losecsDerived = GetLosecsFromAdministrationId(adminId) ?? losecsFromOrder;
                // check if valid derived losecs
                // need to add a check if the adminId != <base pat order adminId> && losecsDerived == losecsFromOrder
                if (!string.IsNullOrWhiteSpace(losecsFromOrder) && string.IsNullOrWhiteSpace(losecsDerived) && adminId != 0)
                {
                    var errMsg = "Could not get derived losecs from the order admin id in SendChartTemplateMarkup";
                    Utilities.LogError(patIbex, externalSiteId, patientOrderId, adminId, errMsg, action.ToString());
                    // Should we return here since something went terribly wrong?
                }
                ...
                if (emr == null || emr.Lines == null || emr.Lines.Count < 1)
                {
                ...
                if (newLine != null)
                {
                    ...
                    foreach (EMR.Line dup_line in emr.Lines)
                    {
                        ...
                        if (dup_line.NCT() == EMR.Constants.NCT_MED_SVC && dup_line.User() == newLine.User()
                         ...
                         && dup_line.Losecs().Equals(newLine.Losecs()))
                        {
                ...
                else
                {
                    ...
                    var debugInfo = " losecs: " + losecsFromOrder + "," + losecsDerived + " newOrderAdmin: " + (newOrderAdmin ? "true" : "false");
                    var errMsg = "newLine is null - nothing to document." + (!foundLosecs ? " Could not find" : "") + debugInfo;
                    Utilities.LogError(patIbex, externalSiteId, patientOrderId, adminId, errMsg, action.ToString());
                }
                ...
                if (notifyUsers)
                {
                    ...
                    if (internalMail != null)
                    {
                        foreach (int notifyId in notifyIds)
                        {
                            ...
                            if (ret == false)
                            {
                                Utilities.LogError(patIbex, externalSiteId, patientOrderId, adminId, "PulseMail send error", action.ToString());
                            }
                        }
                    }
                    else
                    {
                        Utilities.LogError(patIbex, externalSiteId, patientOrderId, adminId, "PulseMail constructor error", action.ToString());
                    }
                }
            ...
            {
                Utilities.LogError("No ibex", 0, patientOrderId, adminId, "SendChartTemplateMarkup error", action.ToString());
                Utilities.LogException(ex);
        ...
        public string addPatientVitals(OcsVitals ocsVitals, string patIbex, byte externalSiteId, int externalUserId, byte internalSiteId)
        {
            ...
            {
                Utilities.LogException(ex);
                String returnString = "DbUpdateException in addPatientVitals";
                String appendString = ex.InnerException != null ? " - " + ex.InnerException.Message : ".";
                return returnString + appendString;
            }
            catch (Exception ex)
            {
                Utilities.LogException(ex);
        ...
        /// <param name="currentAction">Action enumerator</param>
        ...
        public string updatePatientMedicationIndicator(long patientId, string ibex, int siteId, ActionEnum currentAction, long adminId)
        {
            try
            {
                ...
                foreach (var data in emarMedAdminData)
                {
                    ...
                    if (map.Equals(Medication.Constants.CANCEL_TITLE) || map.Equals(Medication.Constants.DELETE_TITLE))
                    {
                ...
                foreach (var emarlosecs in emar_losecs.Keys)
                {
                    ...
                    foreach (var entry in emar_medications[emarlosecs])
                    {
                        ...
                        if (map.Equals(Medication.Constants.HOLD_TITLE))
                        {
                            temp_med.UnholdUserId = null;
                            temp_med.UnholdDate = null;
                        }
                        else if (map.Equals(Medication.Constants.UNHOLD_TITLE))
                        {
                            temp_med.HoldUserId = null;
                            temp_med.HoldDate = null;
                        }
                ...
                foreach (var losecsItem in losecs)
                {
                    ...
                    foreach (var entry in emar_medications[losecsItem])
                    {
                        ...
                        if (map.Equals(Medication.Constants.HOLD_TITLE))
                        {
                            medsFromMed.UnholdUserId = null;
                            medsFromMed.UnholdDate = null;
                        }
                        else if (map.Equals(Medication.Constants.UNHOLD_TITLE))
                        {
                            medsFromMed.HoldUserId = null;
                            medsFromMed.HoldDate = null;
                        }
                ...
                int openPRNCount = 0;
                // set boolean flag when current action is an ack or a hold
                var ackOrHold = currentAction == ActionEnum.Acknowledge || currentAction == ActionEnum.Hold;
                // set boolean flag when the current action (a give?) exposes a prior ack or a hold and makes them the active ord30 datetime
                var earliestAckOrHold = false;
                // set the corresponding order administration ID - default to 0
                long earliestOrderAdminID = 0;
                // set the corresponding action enumeration - default to ack
                ActionEnum earliestOrderActionEnum = ActionEnum.Acknowledge;
                // set the corresponding color - default to blue (ack)
                string earliestOrderColor = Medication.ActionConstants.BLUE;
                // set the corresponding DateTimeOffset - default to null
                DateTimeOffset? earliestOrderDateTime = null;
                // attempt at signifying the first admin on a patient for purposes of ack or hold related handling
                var firstAdmin = false;
                // Keep track of the order id associated with the current value of earliestFutureAdmin
                long earliestFutureAdminOrderId = 0;
                // get the Patients object associated with the patient
                var patient = _ibexContext.Patients.FirstOrDefault(p => p.Ibex == ibex && p.Site == siteId);
                // get the medDueTime
                int medDueTime = 0;
                // define the DateTimeOffset object that represents the scheduled datetime of the ord30 datetime
                DateTimeOffset? scheduledOrd30DateTime = null;
                // define the initial ord30 value before modification
                string initialOrd30 = null;
                ...
                if (patient != null)
                {
                    try
                    {
                        ...
                        // as long as we're here, set scheduledOrd30DateTime
                        if (patient.Ord30DateTime != null)
                        {
                            scheduledOrd30DateTime = ((DateTimeOffset)patient.Ord30DateTime).AddMinutes(medDueTime);
                        }
                        // set initialOrd30 too
                        initialOrd30 = patient.Ord30;
                    }
                    catch (Exception ex)
                    {
                        Utilities.LogException(ex);
                ...
                foreach (var key in medications.Keys)
                {
                    ...
                    // the following section of code has the purpose of determining whether or not to skip the current med/admin
                    // in the foreach loop so as not to use it in the color calculation.
                    long ord30AdminId = 0;
                    Boolean medHasAckOrHold = med.AckUserId > 0 || med.HoldUserId > 0;
                    // get the administration ID from the current med object in the loop
                    var medAdminId = GetAdministrationIdFromLosecs(med.Losecs, patientOrder, med.GiveDate);
                    if (scheduledOrd30DateTime != null)
                    {
                        // get the admin ID represented by the current ord30 datetime
                        ord30AdminId = GetAdminIdFromOrd30DateTime(patientOrder, (DateTimeOffset)scheduledOrd30DateTime, currentAction, adminId);
                    }
                    // check if ord30 alt is empty
                    if (string.IsNullOrWhiteSpace(patient.Ord30Alternate))
                    {
                        if (currentAction != ActionEnum.Give)
                            firstAdmin = true;
                        // get the administration scheduled datetime for the med order administration ID
                        var medAdminDateTime = GetDateTimeFromOrderAdministrationId(patientOrder, medAdminId);
                        // check if the scheduled datetime from the current looped med is more than medDueTime in the future
                        if (medAdminDateTime > siteNow.AddMinutes(medDueTime))
                        {
                            // if there are multiple outstanding order actions, bypass newer ones
                            if (earliestOrderDateTime == null || medAdminDateTime <= earliestOrderDateTime)
                            {
                                // set earliestAckOrHold based upon whether or not the med order administration is earlier than the earliest future administration
                                earliestAckOrHold = earliestFutureAdmin == null || earliestFutureAdmin > medAdminDateTime ? medHasAckOrHold : earliestAckOrHold;
                                // determine if the admin ID from the current action is after ord30AdminId (time/schedule wise, not ID wise)
                                var afterOrd30DateTime = adminAfterOrd30DateTime(patientOrder, medAdminId, scheduledOrd30DateTime);
                                firstAdmin = afterOrd30DateTime ? false : true;
                                if (earliestAckOrHold && (!afterOrd30DateTime || (currentAction == ActionEnum.Give && earliestOrderDateTime != null)))
                                {
                                    // set the earliest variables accordingly
                                    earliestOrderAdminID = medAdminId;
                                    earliestOrderActionEnum = !string.IsNullOrWhiteSpace(med.AckDate) ? ActionEnum.Acknowledge : ActionEnum.Hold;
                                    earliestOrderColor = !string.IsNullOrWhiteSpace(med.AckDate) ? Medication.ActionConstants.BLUE : Medication.ActionConstants.YELLOW;
                                    earliestOrderDateTime = medAdminDateTime;
                                }
                                if (currentAction != ActionEnum.Give)
                                {
                                    // set earliestFutureAdmin here so that it can be used for the null check against non-PRN and non-held orders
                                    earliestFutureAdmin = earliestFutureAdmin == null || earliestFutureAdmin > medAdminDateTime ? medAdminDateTime : earliestFutureAdmin;
                                    earliestFutureAdminOrderId = patientOrder.Id;
                                    continue;
                                }
                            }
                        }
                    ...
                    // default the skip color calculation (ord30_alt) to false
                    var skipColorCalculation = false;
                    // validity check before calling adminAfterOrd30DateTime
                    // values must be non null as well as the admin id must be different or action is reschedule
                    if (patientOrder != null && medAdminId != 0 && scheduledOrd30DateTime != null
                        && ((ord30AdminId != 0 && ord30AdminId != medAdminId) || currentAction == ActionEnum.Reschedule))
                    {
                        // determine if the admin ID from the current action is after ord30AdminId (time/schedule wise, not ID wise)
                        var afterOrd30DateTime = adminAfterOrd30DateTime(patientOrder, medAdminId, scheduledOrd30DateTime);
                        // skip processing if true
                        if (afterOrd30DateTime)
                            skipColorCalculation = true;
                    }
                    ...
                    // If a modify, skip the 'order' portion
                    ...
                    // calculate the medStatus
                    var medStatus = new Dictionary<string, string>();
                    if (!skipColorCalculation)
                        medStatus = med.GetMedStatus();
                    else
                        medStatus["code"] = ""; // default to satisfy ContainsKey("code") check below
                    ...
                    if (medStatus.ContainsKey("code"))
                    {
                        ...
                        if (!isPrnOrder && !isHeldOrder)
                        {
                            ...
                            foreach (Data.Entities.OrderAdministration o in patientOrder.OrderAdministrations)
                            {
                                ...
                                //I'm also changing this to use siteNow instead of Now.
                                //Winston Murdock, 03/29/2022.  PC-27069
                                if (adminScheduledDateTime < siteNow.AddMinutes(-5) ||
                                    o.AdministrationDatetime != null ||
                                    //We do not want to exclude administrations that have been acknowledged from the
                                    //calculation of ord30_dt.
                                    //I have no idea why this was added.  But the checkin history shows that
                                    //EMAR-1140 asked us to add it.
                                    //Winston Murdock, 08/22/2022.  No Ticket.
                                ...
                                if (earliestFutureAdmin == null)
                                {
                                    ...
                                    if (adminScheduledDateTime != null)
                                    {
                                        ...
                                        earliestFutureAdmin = adminScheduledDateTime;
                                        earliestFutureAdminOrderId = patientOrder.Id;
                                        // set earliestAckOrHold based upon whether or not the order administration has an ack or hold 
                                        earliestAckOrHold = o.AcknowledgeDatetime != null || o.OnHold;
                                        if (earliestAckOrHold)
                                        {
                                            // set the earliest variables accordingly
                                            earliestOrderAdminID = o.Id;
                                            earliestOrderActionEnum = o.AcknowledgeDatetime != null ? ActionEnum.Acknowledge : ActionEnum.Hold;
                                            earliestOrderColor = o.AcknowledgeDatetime != null ? Medication.ActionConstants.BLUE : Medication.ActionConstants.YELLOW;
                                        }
                                    } //end if
                                }
                                else
                                {
                                    ...
                                    else if (string.IsNullOrWhiteSpace(patient.Ord30Alternate) && earliestAckOrHold && !skipColorCalculation)
                                    {
                                        // revisit this code
                                        if (currentAction != ActionEnum.Give || o.AdministrationScheduledDatetime < earliestFutureAdmin)
                                        {
                                            // set the earliest variables accordingly
                                            earliestOrderAdminID = o.Id;
                                            earliestOrderActionEnum = o.AcknowledgeDatetime != null ? ActionEnum.Acknowledge : ActionEnum.Hold;
                ...
                if (patient != null)
                {
                    ...
                    //We were not checking to see if there were any past due administrations when calculating ord30_dt.
                    //Thus, we were incorrectly setting ord30_dt when we had past due administrations.
                    //If we have any past administrations that are due, then earliestFutureAdmin to
                    //the time of the earliest past due administration.
                    //If there are not any, then leave it at the value we've already calculated.
                    //Winston Murdock, 07/22/2022.  PC-27276
                    var pastDueAdministrations = _orderRepository.GetAllPastDueAdministrationsForPatient(patientId);

                    //See if we have any past due administrations or not.
                    if (pastDueAdministrations.Any())
                    {
                        //We do have one or more past due administrations.
                        //Use the time of the earliest past due administration.
                        //Grab the timestamp of the earliest administration that is past due.
                        earliestFutureAdmin = pastDueAdministrations.Min(oa => oa.AdministrationScheduledDatetime);
                        // need to set earliestFutureAdminOrderId, earliestAckOrHold here?
                    } //end if
                    ...
                    if (earliestFutureAdmin != null)
                    {
                        ...
                        // 08/25/2022 PC-27483
                        // The Ord30Alternate does not change based upon the time - should be calculated by index
                        patient.Ord30Alternate = index;

                        // investigative code block follows
                        //if (patient.Ord30DateTime != null && earliestFutureAdmin != null && earliestFutureAdminOrderId != 0
                        //    && (patient.Ord30 == Medication.ActionConstants.BLUE || patient.Ord30 == Medication.ActionConstants.YELLOW))
                        //{
                        //    var patientOrder = patient_orders.ContainsKey(earliestFutureAdminOrderId) ? patient_orders[earliestFutureAdminOrderId] : null;
                        //    var nextAdmin = GetNextAdministrationForOrd30DateTime(patientOrder, earliestFutureAdmin);
                        //    if (nextAdmin != null)
                        //    {
                        //    }
                        //}

                        // if not ack or hold, set ord30 to red. May be adjusted downstream for hold or ack
                        patient.Ord30 = ackOrHold ? patient.Ord30 : Medication.ActionConstants.RED;
                    ...
                    else
                    {
                        ...
                        if (index != null && index.Equals(Medication.ActionConstants.RED))
                        {
                            ...
                            //Need to use the site's time zone, not the webserver's time zone.
                            //Winston Murdock, 07/13/2022.
                            //futureStatusTime = DateTimeOffset.Now;
                            futureStatusTime = siteNow;
                    ...
                    // ack and hold handling specific for setting ord30
                    // must be either a current ack or hold action or a give that exposes a prior ack or hold action
                    if ((ackOrHold || earliestAckOrHold) && earliestFutureAdminOrderId != 0)
                    {
                        // get the patient order associated with this order Id
                        var patientOrder = patient_orders.ContainsKey(earliestFutureAdminOrderId) ? patient_orders[earliestFutureAdminOrderId] : null;
                        // set temp variables to default values
                        var tempAdminID = adminId;
                        var tempAction = currentAction;
                        // check if earliestAckOrHold is true
                        if (earliestAckOrHold)
                        {
                            // swap out the adminId and action
                            tempAdminID = earliestOrderAdminID;
                            tempAction = earliestOrderActionEnum;
                        }
                        // get the administration scheduled datetime for the listed order administration ID
                        var currentAdminDateTime = GetDateTimeFromOrderAdministrationId(patientOrder, tempAdminID);

                        // re-calculate ord30
                        // currently, there are three separate checks upon which to set ord30
                        // check if the current admin datetime is after or equal to the scheduled ord30 datetime
                        if (currentAdminDateTime != null && scheduledOrd30DateTime <= currentAdminDateTime)
                        {
                            // if a 'swap' then use the color based upon the earlier action, otherwise use the original color
                            patient.Ord30 = earliestAckOrHold ? earliestOrderColor : initialOrd30;
                        }
                        // if first admin set ord30 based upon the current action
                        else if (firstAdmin)
                        {
                            patient.Ord30 = tempAction == ActionEnum.Acknowledge ? Medication.ActionConstants.BLUE : Medication.ActionConstants.YELLOW;
                        }
                        else
                        // not a first admin and current admin datetime is before the scheduled ord30 datetime
                        {
                            // get the next order admin id associated with the patient order and the earliestFutureAdmin DateTimeOffset
                            var nextAdmin = GetNextAdministrationForOrd30DateTime(patientOrder, earliestFutureAdmin);
                            // check if found next admin
                            if (nextAdmin != null) // && nextAdmin.Id == adminId?
                            {
                                // check if next admin action is not a hold or ack
                                if (!nextAdmin.OnHold && nextAdmin.AcknowledgeDatetime == null)
                                {
                                    // set color to earliestOrderColor if earliestAckOrHold
                                    patient.Ord30 = earliestAckOrHold ? earliestOrderColor : Medication.ActionConstants.RED;
                                }
                                else
                                {
                                    // set color based upon next action
                                    patient.Ord30 = nextAdmin.OnHold ? Medication.ActionConstants.YELLOW
                                                  : Medication.ActionConstants.BLUE;
                                }
                            }
                            else
                            {
                                // set ord30 color to ord30 alt if has a color, otherwise red
                                patient.Ord30 = !string.IsNullOrWhiteSpace(patient.Ord30Alternate) ? patient.Ord30Alternate : Medication.ActionConstants.RED;
                            }
                        }
                    }
            ...
            {
                Utilities.LogException(ex);
                String returnString = "DbUpdateException in updatePatientMedicationIndicator";
                String appendString = ex.InnerException != null ? " - " + ex.InnerException.Message : ".";
                return returnString + appendString;
            }
            catch (SqlException ex)
        ...
        public string addTrxEntryForFormulary(long patientOrderId, long patId, string patExtId, byte extSiteId, int userId, string enteredTime, int intSiteId)
        {
            try
            {
                ...
                //Get the timezone of the current site.
                string timeZoneName = _siteRepository.GetSiteTimeZone(intSiteId);
                ...
                foreach (var svc in serviceCodesList)
                {
                    ...
                    var Values = new Dictionary<string, object>
                    {
                        ...
                        { Transaction.Constants.Date, enteredTime ?? siteNowFormatted}
                    };
                    var t = new Transaction(extSiteId, patient, userId, Values, null);
        ...
        public string updateTrxEntryForDelete(string patExtId, byte extSiteId, string trxDate, string losecs)
        {
            try
            {
                var trxParams = new List<SqlParameter> {
                    ...
                    new SqlParameter("@losecs", SqlDbType.Int) { Value = int.Parse(losecs) }
                };
        ...
        public string updatePatientPharmacyVerification(long patientId, string extPatientId, byte externalSiteId)
        {
            ...
            {
                Utilities.LogException(ex);
                String returnString = "DbUpdateException in updatePatientPharmacyVerification";
                String appendString = ex.InnerException != null ? " - " + ex.InnerException.Message : ".";
                return returnString + appendString;
            }
            catch (Exception ex)
        ...
        public string GetIbexFormatDateTimeFromDTO(string inputDTO, DateTimeOffset siteNow, bool? bReturn14Chars = false)
        {
            //Former logic.
            ...
            //Some places need this returned as 12 chars (i.e. without seconds).
            //Anywhere writing into PulseCheck needs this to be returned as 14 chars.
            //Thus, I added an optional, boolean parameter for this.
            //When true, then we return 14 chars.
            //When missing or false, then we return 12 chars.
            //Any existing calls to this will continue to return 12 chars.
            //Anywhere that needs 14 chars can specify that via the parameter.
            //Winston Murdock, 02/17/2023.
            if (bReturn14Chars.HasValue)
            {
                if ((bool)bReturn14Chars)
                {
                    return offset.ToString("yyyyMMddHHmmss");
                }
                else
                {
        ...
        public long GetAdministrationIdFromLosecs(int losecs, Data.Entities.PatientOrder patientOrder, string giveDate)
        {
            if (losecs == 0 || patientOrder == null)
                return 0;

            var query = from m in _ibexContext.EmarMedicationAdministrations
                        where m.Losecs == losecs && m.PatientOrderId == patientOrder.Id
                        select m.OrderAdministrationsId;

            // if found the order admin ID from the emar_med_administrations table, then return it
            ...
            {
                // did not find the order admin in emar_med_administrations
                // must be a give action, so find the admin ID in the order_events table using the give date
                if (string.IsNullOrWhiteSpace(giveDate))
                    return 0;

                // find the order admin id based upon the action and give date
                foreach (Data.Entities.OrderEvent o in patientOrder.OrderEvents)
                {
                    if (o.Action.Name == "Give")
                    {
                        if (o.EventDateTime.ToString("yyyyMMddHHmmss") == giveDate)
                        {
                            return o.OrderAdministrationId != null ? (long)o.OrderAdministrationId : 0;
                        }
                    }
                }
                return 0;
        ...
        public Data.Entities.OrderAdministration GetNextAdministrationForOrd30DateTime(Data.Entities.PatientOrder patientOrder, DateTimeOffset? earliestFutureAdmin)
        {
            long earliestFutureAdminId = 0;
            // find the current order admin id based upon the earliestFutureAdmin
            foreach (Data.Entities.OrderAdministration o in patientOrder.OrderAdministrations)
            {
                if (o.AdministrationScheduledDatetime == earliestFutureAdmin)
                {
                    earliestFutureAdminId = o.Id;
                    break;
                }
            }

            if (earliestFutureAdminId > 0)
            {
                foreach (Data.Entities.OrderAdministration o in patientOrder.OrderAdministrations)
                {
                    // expectations are that the next order administration will have an order admin id one greater than current
                    if ((o.Id - 1) == earliestFutureAdminId)
                    {
                        return o;
                    }
                }
        ...
        public bool adminAfterOrd30DateTime(Data.Entities.PatientOrder patientOrder, long adminid, DateTimeOffset? ord30DateTime)
        {
            bool adminAfterOrd30DateTime = false;

            foreach (Data.Entities.OrderAdministration o in patientOrder.OrderAdministrations)
            {
                // find the current order admin id based upon the adminid
                if (o.Id == adminid)
                {
                    // set boolean to true if administration scheduled datetime is greater than the current ord30 datetime
                    if (o.AdministrationScheduledDatetime > ord30DateTime)
                    {
                        adminAfterOrd30DateTime = true;
                    }
                    break;
                }
            }

            return adminAfterOrd30DateTime;
        }

        public long GetAdminIdFromOrd30DateTime(Data.Entities.PatientOrder patientOrder, DateTimeOffset ord30DateTime, ActionEnum action, long adminId)
        {
            long earliestFutureAdminId = 0;
            // find the current order admin id based upon the earliestFutureAdmin
            foreach (Data.Entities.OrderAdministration o in patientOrder.OrderAdministrations)
            ...
            if (earliestFutureAdminId == 0 && action == ActionEnum.Reschedule )
            {
                earliestFutureAdminId = adminId;
            }

            return earliestFutureAdminId;
        }

        public DateTimeOffset? GetDateTimeFromOrderAdministrationId(Data.Entities.PatientOrder patientOrder, long adminId)
        {
            if (patientOrder == null || adminId == 0)
                return null;

            DateTimeOffset? scheduledDateTime = null;
            // find the scheduled datetime based upon the admin id
            foreach (Data.Entities.OrderAdministration o in patientOrder.OrderAdministrations)
            {
                if (o.Id == adminId)
                {
                    scheduledDateTime = o.AdministrationScheduledDatetime;
                    break;
                }
            }
