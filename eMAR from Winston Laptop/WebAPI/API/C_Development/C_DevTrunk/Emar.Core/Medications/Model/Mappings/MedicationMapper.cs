    ...
    public static class MedicationMapper
    {
        ...
        public static OrderInteractionDto MapOrderInteraction(OrderInteraction orderInteraction, string drugDbVendor, BaseLinkResource resource)
        {
            ...
            var orderInteractionDto = new OrderInteractionDto
            {
                ...
                //I have no idea why we don't copy over the DrugNum.
                //Given my filter above, we will only have drug one in the return JSON.
                //Winston Murdock, 03/16/2022.
        ...
        public static MedicationDetailDto MapMedicationDetail(MedicationDetail dbObj, int? codeShareSiteMedicationUnit)
        {
            ...
            var ret = new MedicationDetailDto
            {
                ...
                DoseUnit = dbObj.MedicationUnit != null
                           //Not sure why we would not want to map the unit when it's a combo med.
                           //We need the unit for the medication details that make up the combo med.
                           //Winston Murdock, 08/24/2021.  Unknown ticket number
        ...
        public static BrandNameSearchDto MapBrandName(BrandNameReturnDto dbObj, string schedulerDataRetrieveBase)
        {
            ...
            return new BrandNameSearchDto
            {
                ...
                Link = new HateOasLinkDto(
                    ...
                    //string.Format(schedulerDataRetrieveBase, dbObj.BrandName),
                ...
                IsGroupItem = dbObj.IsGroupItem
            };
        }

        public static BrandNameSearchDto MapGroupBrandName(BrandNameReturnDto dbObj, string groupLink)
        {
            //Map group items when doing a group search.
            //The only changes from the above method are pulling in the GroupItemId
            //and having the HateOAS link that goes to the scheduler options for this item.
            ...
            return new BrandNameSearchDto
            {
                ...
                Link = new HateOasLinkDto(
                    string.Format(groupLink, dbObj.GroupItemId),
                    "get_scheduler_options",
                    "GET"),

                IsGroupItem = dbObj.IsGroupItem,
                GroupItemId = dbObj.GroupItemId
            };
        }

        public static PrnIndicationDto MapPrnIndication(PrnIndication dbObj)
        {
            //Since we're just returning a list of strings right now, we probably don't need this mapper.
            ...
            return new PrnIndicationDto
            {
                Id = dbObj.Id,
                //SiteId = dbObj.SiteId,
                OptionDescription = dbObj.OptionDescription
            };
        } //end MapPrnIndication
    }
}