...
namespace Emar.Core.Orders.Model.Mappings
{
    public static class OrderMapper
    {
            ...
            OrderActionMapperHelper orderActionMapperHelper, List<CodeSharedId> codeShareSites, BaseLinkResource resource = null)
        {
            ...
            {
                //If point in time is true and PRN is false, then this is a timed order.
                //Winston Murdock, 04/29/2022.  PC-27080
            ...
            //If an order is a "PRN" order, then it should not be flagged as
            //"scheduled" regardless of the begin date time on the order.
            //Winston Murdock, 04/13/2022.  PC-27178
            if ((patientOrderDto.BeginDatetime > DateTimeOffset.Now) && !(patientOrderDto.Prn))
            {
                applicables.Add("scheduled");
            } //end if

            //Per Sylvie, an order cannot ever be timed unless it's not PRN.
            //Thus, we should only be checking for timed when PRN is false.
            //I've moved this check to the else of the "PRN" section above.
            //Winston Murdock, 04/29/2022.  PC-27080
            ...
            //Perhaps we should define "continous" orders based on the Frequency Type.
            //Rather than this calculation.
            //We would look at patientOrderDto.FrequencySchedule.FrequencyType.Name.
            //I have confirmed that the below calculation works fine on 57c.
            //But it seems like it would be cleaner to check the Frequency Type.
            //Winston Murdock, 04/18/2022.
            if
            (
        ...
        public static GroupListItemDto MapGroupListItem(GroupListItem dbObj, BaseLinkResource resource, List<CodeSharedId> codeShareSites)
        {
            ...
            var ret = new GroupListItemDto
            {
                ...
                Dose = dbObj.Dose,
                DoseUnit = dbObj.MedicationUnit != null
                           //I'm not sure why we explicitly don't map dose unit for combo meds.
                           //But we want to map it so that it can show on the groups list on Med Svc.
                           //Winston Murdock, 02/10/2023.  PC-27786
                           //&& dbObj.Medication.DrugId != "COMBO"
                           //////&& dbObj.Medication.SiteId != -1
                           && dbObj.MedicationUnit.SiteId == codeShareSites
                               .FirstOrDefault(c =>
                    ...
                    : null,
                MedicationRoute = dbObj.MedicationRoute != null
                                  //I'm not sure why we explicitly don't map route for combo meds.
                                  //But we want to map it so that it can show on the groups list on Med Svc.
                                  //Winston Murdock, 02/10/2023.  PC-27786
                                  //&& dbObj.Medication.DrugId != "COMBO"
                                  //////&& dbObj.Medication.SiteId != -1
                    ...
                    : null,
                FrequencySchedule = dbObj.FrequencySchedule != null
                                    //I'm not sure why we explicitly don't map frequency for combo meds.
                                    //But we want to map it so that it can show on the groups list on Med Svc.
                                    //Winston Murdock, 02/10/2023.  PC-27786
                                    //&& dbObj.Medication.DrugId != "COMBO"
                                    //////&& dbObj.Medication.SiteId != -1
        ...
        public static MedicationModel MapMedicationInComboMedOrderToModel(Medication item, long patientId, int userId, int? codeShareSiteMedicationUnit)
        {
            if (item == null)
            {
                return null;
        ...
        } //end MapMedicationInComboMedOrderToModel
        ...
        public static MedicationModel MapPatientOrderToModel(PatientOrder order, int? userId, int? siteId, int? codeShareSiteMedicationUnit)
        {
            ...
            return new MedicationModel
            {
                ...
                Medication = MedicationMapper.MapMedication(order.Medication, codeShareSiteMedicationUnit),
                //MedicationDrugId = null,
                MedicationDrugId = order.Medication.DrugId,
                MedicationRouteId = order.MedicationRouteId,
                MedicationUnitId = order.MedicationUnitId,
                OrderPhysicianUserId = null,
                OrderStatus = null,
        ...
        public static SchedulerOptionsDto MapSchedulerSetupData(
            ...
            string? pathwayToLoad = "")
        {
            //Added isGroupitem and pathwayToLoad
            //The only methods that call this mapper
            //are the scheduler options load for a medication name
            //and the scheduler options load for a, specific item (group, user quick list, repeat order, etc...).
            //Both of those calculate/set these appropriately before calling the mapper.
            //The mapper passes along what they give to it.
            ...
            var ret = new SchedulerOptionsDto
            {
                ...
                IsGroupItem = isGroupitem.HasValue ? isGroupitem.Value : false,
                PathwayToLoad = pathwayToLoad
            };
            ...
            if (medications.Any())
            {
                ...
                //Map the brand name of the first (should be only) MedicationDetail.
                //Map the dose form and strnegth of the first (should be only) FdbNdcInfo.
                //Winston Murdock, 10/25/2022.  PC-27618
                if (ret.AvailableFormStrength.Any())
                {
                    foreach (FormStrengthDto fsd in ret.AvailableFormStrength)
                    {
                        //Only do these if we have any fdbndcinfo childreno f ret.AvailableFormStrength
                        //We were hitting an error when calling .Any on the null list of FDbNdcInfos
                        //when we get here from a remembered list and not from the search.
                        //Winston Murdock, 10/31/2022.  PC-27618
                        if (ret.AvailableFormStrength.ToList()[0].FdbNdcInfos != null)
                        {
                            if (fsd.MedicationDetails.Any())
                            {
                                fsd.BrandName = fsd.MedicationDetails.ToList()[0].BrandName;
                            } //end if

                            if (ret.AvailableFormStrength.ToList()[0].FdbNdcInfos.Any())
                            {
                                fsd.DoseForm = fsd.FdbNdcInfos.ToList()[0].DoseForm;
                                fsd.Strength = fsd.FdbNdcInfos.ToList()[0].Strength;
                            } //end if
                        } //end if (fdbndcinfos is null?)
                    } //end for each
                } //end if
            } //end if
                ...
                .Select(p =>
                    p == null
                        ...
                        : new FdbNdcInfoDto
                        {
                            ...
                            Route = p.Route,
                            DrugCat = p.DrugCat,
                            //Not sure why weren't mapping the strength.
                            //Winston Murdock, 10/25/2022.  PC-27618
                            Strength = p.Strength
                        })
                .ToList();
