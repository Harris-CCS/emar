...
namespace Emar.Api
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            //Moved these out of the IDS if so that we always have them so we can do it without the IDS running.
            //One of the methods in PatientRepository that is only called by the IDS uses IbexInboundDataRepository.
            //Even though we won't call that method when we're not using the IDS, we need to have this reference available to us.
            //The negligible overhead from having these three things in scope when the IDS is not running is worth it
            //since this allows us to log issues in the IDS FilePatient method into the IDS logging table rather than
            //using some other logging method (like writing to the Event Viewer).
            //Winston Murdock, 08/22/2022.  PC-27447
