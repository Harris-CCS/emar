...
namespace Emar.Api.Controllers
{
    ...
    public class PatientsController : ControllerBase
    {
        ...
        /// </param>
        /// <param name="pharmacyVerificationStatus">
        /// *Optional.* \
        ...
        /// </param>
        /// <param name="upcomingOrdersOnly">
        /// *Optional.* \
        /// Whether we want to only include patients with "upcoming" orders or include all patients.
        /// Upcoming being defined as any order with scheduled administrations in the future.
        /// </param>
        /// <param name="onlySaveDefaults">
        /// *Optional.* \
        /// If this is true, then we're only saving defaults and not actually getting the patient list.
        /// Here are the querystring parameters that we look at to set the defaults.
        /// "includeMyPatientsOnly=false" = A
        /// "includeMyPatientsOnly=true" = M
        /// "pharmacyVerificationStatus=1 = V
        /// "upcomingOrdersOnly=true" = U
        /// </param>
        /// <param name="onlyGetDefaults">
        /// *Optional.* \
        /// If this is true, then we're only retrieving the user's defaults and not actually getting the patient list.
        /// A = all  patients ("includeMyPatientsOnly=false")
        /// M = only my patients ("includeMyPatientsOnly=true")
        /// V = phamracy verification status = 1 ("pharmacyVerificationStatus=1")
        /// U = upcoming orders only (don't need to pass this in as the UI is handling this filtering)
        /// After calling this to get the user's defaults, call this same endpoint with these settings
        /// (without specifying onlyGetDefaults or onlySaveDefaults) to actually get the list of patients.
        /// </param>
        ...
        public ActionResult<IEnumerable<PatientDto>> GetPatients(
            ...
            [FromQuery] string upcomingOrdersOnly,
            [FromQuery] string onlySaveDefaults,
            [FromQuery] string onlyGetDefaults
            ...
            )
        {
            ...
            var resourceParameters = new PatientsResourceParameters
            {
                ...
                UpcomingOrdersOnly = bool.TryParse(upcomingOrdersOnly, out bool incUpcomingOrders) && incUpcomingOrders,
                OnlySaveDefaults = bool.TryParse(onlySaveDefaults, out bool saveDefaults) && saveDefaults,
                OnlyGetDefaults = bool.TryParse(onlyGetDefaults, out bool getDefaults) && getDefaults
            };


            //If only get defaults or set defaults is specified, then don't get the list of patients.
            //Just get (or set) the defaults and return them.
            if (resourceParameters.OnlyGetDefaults)
            {
                //Get the defaults from the DB.
                string sUsereDefaultMarDeptFilters = "";
                sUsereDefaultMarDeptFilters = _userService.GetUserDefaultMarFilters(userId.Value);
                
                return Ok(sUsereDefaultMarDeptFilters);
            }
            else if (resourceParameters.OnlySaveDefaults)
            {
                //Set the defaults and return the new default string.
                string sUsereDefaultMarDeptFilters = "";
                sUsereDefaultMarDeptFilters = _userService.SetUserDefaultFilters(userId.Value, resourceParameters);

                return Ok(sUsereDefaultMarDeptFilters);
            }

            //Since we haven't returned here, we are actually getting the patients.
            //Go get them.
        ...
        /// <summary>
        /// Return all order administrations for a given patient and order.
        /// </summary>
        /// <param name="mediaType">
        /// Media type from Accept header.
        /// </param>
        ...
        /// 
        /// <returns>
        /// The specified patient, then the specified order undernath the patient.
        /// Then all order administrations for the specified order.
        /// </returns>
        /// <remarks>
        /// </remarks>
        [HttpGet("{patientId}/order/{orderId}", Name = nameof(GetAdminstrationsByPatientAndOrder))]
        [ProducesResponseType(200)] // (OK) - the resource is sent in the response
        [ProducesResponseType(400)] // (bad request) - indicates a bad request (e.g. wrong parameter)
        [ProducesResponseType(404)] // (not found) - the resource does not exits
        public ActionResult<int> GetAdminstrationsByPatientAndOrder
        (
            [FromHeader(Name = "Accept")] string mediaType,
            [FromRoute(Name = "patientId")] long patientId,
        ...
        {
            //Given a patient id and order id,
            //get the patient, the order, and then all order administrations.
            //This is intended for a pop-up on the UI that will show this information for completed orders.
            //Winston Murdock, 08/24/2021.  EMAR-1176
            ...
            //Call the service to get the info we need.
            var ret = _patientService.GetAdministrationsByPatientAndOrder(patientId, orderId, userId.Value);
