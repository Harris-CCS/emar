...
namespace Emar.Core.Devices.Service
{
    public partial class DeviceService : IDeviceService
    {
        ...
        public string PrintPdfFile(int extUserId, byte extSiteId, string extPatId, int extDevId, string root,
                                   string sourcepath, DateTimeOffset printDateTime, int userId, bool activePatient)
        {
            ...
            var chart = _deviceRepository.SendChartAdminEntry(extPatId, extSiteId, extUserId, data, activePatient);
        ...
        public string PrintIPBasedFile(int extUserId, byte extSiteId, string extPatId, int extDevId, string root,
                                       string sourcepath, DateTimeOffset printDateTime, int userId, bool activePatient)
        {
        ...
        public string MoveImageFile(int extUserId, byte extSiteId, string extPatId, int extDevId,
                                    string sourcepath, string targetPath, bool activePatient, string imageType)
        {
            //Not needed because Winston's stuff already took the base64 string and converted it to a byte array.
        ...
        public string MakePrintFile(Dictionary<string, string> printFileResponses, string sourcepath)
        {
            ...
            var patientId = Convert.ToInt32(printFileResponses["patient_id"]);
            var activePatient = _deviceRepository.GetPatientStatus(patientId);
            var extPatId = GetExtPatientId(patientId);
            ...
            switch (device.DeviceType)
            {
                case "I":
                    ...
                    var ipReturn = PrintIPBasedFile(extUserId, extSiteId, extPatId, extDevId, root, sourcepath, printDateTime, printUserId, activePatient);
                    break;
                case "D":
                    // PDF Printer
                    var pdfReturn = PrintPdfFile(extUserId, extSiteId, extPatId, extDevId, root, sourcepath, printDateTime, printUserId, activePatient);
                    break;
                case "W":
                    ...
                    var imgReturn = MoveImageFile(extUserId, extSiteId, extPatId, extDevId, sourcepath, targetPath, activePatient, "");
                    break;
                default:
                    break;
            }
