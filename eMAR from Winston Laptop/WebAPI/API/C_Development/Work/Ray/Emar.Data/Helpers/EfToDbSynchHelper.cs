...
namespace Emar.Data.Helpers
{
    public class EfToDbSynchHelper
    {
        ...
        private readonly EfDiscrepancyReport _report;
        private readonly string _contextName;


        public EfToDbSynchHelper(DbContext context, string contextName)
        {
            _context = context;
            _contextName = contextName;
            _report = new EfDiscrepancyReport(contextName);
        }

        public object CompareEfToDb(EfToDbSynchHelperParams parameters)
        {
            ...
            if (ProblemsExitInEfDefinitions(tables))
                return _report;

            CompareSurveyToDatabase(tables, parameters);

            if (!_report.Files.Any()) return tables;

            return _report;
        }

        public EfDiscrepancyReport AddTables(EfToDbSynchHelperParams parameters)
        {
            object compareReturn = CompareEfToDb(parameters);
            if (compareReturn.GetType() == typeof(EfDiscrepancyReport))
            ...
            var tables = new List<EfTableAttributes>(SurveyEfEntities().OrderBy(t => t.EntityName));
            var report = new EfDiscrepancyReport(_contextName);

            foreach (var table in parameters.TablesToAdd)
            {
                if (table != table.Trim())
                {
        ...
        private List<EfTableAttributes> SurveyEfEntities()
        {
            ...
            foreach (var propertyInfo in _context.GetType().GetProperties())
            {
                ...
                contextPropertyNames.Add(entityName ?? "NameMissing", propertyInfo.Name);
            }

            var tables = new List<EfTableAttributes>();
            foreach (IEntityType entity in _context.Model.GetEntityTypes().OrderBy(a => a.Name))
            {
                ...
                {
                    var col = new EfColumnAttributes(_report)
                    {
                        ClrName = iProp.Name,
            ...
            foreach (IEntityType entity in _context.Model.GetEntityTypes().OrderBy(a => a.Name))
            {
                var table = tables.FirstOrDefault(t => t.EntityFullName == entity.Name);
                Debug.Assert(table != null, nameof(table) + " != null");

                var fkData = entity.GetForeignKeys().ToList();
                var missingReverseKeys = fkData
                    .Where(fk => fk.PrincipalToDependent?.Name == null).ToList();

                var temp = new List<string> { "fred", "george", "John", "Paul" };

                var output = temp.Aggregate((i, p) => i + ", " + p);


                if (missingReverseKeys.Any())
                {
                    foreach (var fk in missingReverseKeys)
                    {
                        var tbl = tables
                            .FirstOrDefault(t => t.EntityFullName == fk.PrincipalEntityType.Name);
                        if (tbl != null)
                        {
                            // Think this iisn't needed
                            //DeclaringEntityProperties = fk.Properties.Select(property => property.Name).ToList(),
                            //fk.Properties.Select(i => i.Name).Aggregate((i, p) => i + ", " + p)

                            // This is what was null that caused the problem in the first place
                            //var principalEntityNavigationProperty = fk.PrincipalToDependent.Name,

                            var declaringEntityNavigationProperty = fk.DependentToPrincipal.Name;
                            var principalEntityTypeFullName = fk.PrincipalEntityType.Name;

                            var deleteBehavior = fk.DeleteBehavior;
                            var constraintName = fk.GetConstraintName();

                            var details =
                                $"declaringEntityNavigationProperty\t{declaringEntityNavigationProperty}{Environment.NewLine}" +
                                $"principalEntityTypeFullName\t{principalEntityTypeFullName}{Environment.NewLine}" +
                                $"deleteBehavior\t{deleteBehavior}{Environment.NewLine}" +
                                $"constraintName\t{constraintName}";

                            _report.RegisterProblem(tbl, ReportProblem.ForeignKeyMissingReverseDeclaration, details);
                        }
                    }
                }

                var foreignKeys = fkData
                    .Where(fk => fk.PrincipalToDependent?.Name != null)
                    .Select(fk => new EfForeignKeyAttributes(table, tables)
                    {
                        DeclaringEntityProperties = fk.Properties.Select(property => property.Name).ToList(),
        ...
        public class EfForeignKeyAttributes
        {
            ...
            // Derived Properties //
            internal bool AccountedFor { get; set; }
        ...
        public class EfColumnAttributes
        {
            ...
            public EfColumnAttributes(EfDiscrepancyReport report)
            {
                _report = report;
            }
                ...
                bool computed)
            {
                ...
                if (!string.IsNullOrWhiteSpace(SqlDataType) && SqlDataType != _dbDataType)
                    _report.RegisterProblem(this, ReportProblem.DbDataTypeNotMatchDefinedSqlType,
                ...
                if ((error = EntityColumnTypeNotMatchSql(out string probDetails)) == ReportProblem.None)
                {
                    ...
                    else if (Required == nullable && !(IsComputed ?? false))
                    {
                        error = nullable
                ...
                _report.RegisterProblem(this, error, probDetails);
            }
                ...
                switch (typeParts[0].Trim().ToLower())
                {
                    ...
                    case "bigint":
                        CheckRequiredAssumedType(typeParts[0].Trim().ToLower(), ClrDataType,
                            typeof(long), typeof(long?), out probDetails);
                        break;
                    case "int":
                        CheckRequiredAssumedType(typeParts[0].Trim().ToLower(), ClrDataType,
                            typeof(int), typeof(int?), out probDetails);
                        break;
                    case "smallint":
                        CheckRequiredAssumedType(typeParts[0].Trim().ToLower(), ClrDataType,
                            typeof(short), typeof(short?), out probDetails);
                        break;
                    case "tinyint":
                        CheckRequiredAssumedType(typeParts[0].Trim().ToLower(), ClrDataType,
                            typeof(byte), typeof(byte?), out probDetails);
                        break;
                    case "bit":
                        CheckRequiredAssumedType(typeParts[0].Trim().ToLower(), ClrDataType,
                            typeof(bool), typeof(bool?), out probDetails);
                        break;
                    case "decimal":
                    case "numeric":
                        CheckRequiredAssumedType(typeParts[0].Trim().ToLower(), ClrDataType,
                            typeof(decimal), typeof(decimal?), out probDetails);
                        break;
                    case "real":
                        CheckRequiredAssumedType(typeParts[0].Trim().ToLower(), ClrDataType,
                            typeof(Single), typeof(Single?), out probDetails);
                        break;
                    case "datetimeoffset":
                        CheckRequiredAssumedType(typeParts[0].Trim().ToLower(), ClrDataType,
                            typeof(DateTimeOffset), typeof(DateTimeOffset?), out probDetails);
                        break;
                    case "date":
                        CheckRequiredAssumedType(typeParts[0].Trim().ToLower(), ClrDataType,
                            typeof(DateTime), typeof(DateTime?), out probDetails);
                        break;
                    case "time":
                        CheckRequiredAssumedType(typeParts[0].Trim().ToLower(), ClrDataType,
                            typeof(TimeSpan), typeof(TimeSpan?), out probDetails);
                        break;
                    default:
        ...
        private void CompareSurveyToDatabase(List<EfTableAttributes> tables, EfToDbSynchHelperParams parms)
        {
            ...
            using (var conn = new SqlConnection(_context.Database.GetDbConnection().ConnectionString))
            {
                ...
                foreach (var tbl in tables)
                {
                    ...
                    {
                        _report.RegisterProblem(tbl, ReportProblem.DocumentNewTable);
                        ...
                        else
                        {
                                            ...
                                            && fk.SqlPrincipalEntityTypeFullName == null)
                                        {
                                            if (parms.ForeignKeysToIgnore == null
                                               || !parms.ForeignKeysToIgnore.Contains(fk.SqlConstraintName))
                                                _report.RegisterProblem(fk, ReportProblem.ForeignTableNotInModel);
                                        }
                                        else
                            ...
                            foreach (var foreignKey in tbl.ForeignKeys)
                            {
                                if (foreignKey.SqlOnly)
                                    ...
                                    _report.RegisterProblem(foreignKey, ReportProblem.ForeignKeyDoesntMatch);
                            }
                            ...
                            for (var i = 0; i < sqlOnlyKeys.Count; i++)
                            {
                                ...
                                for (var index = entityOnlyKeys.Count - 1; index >= 0; index--)
                                {
                                    ...
                                    _report.RegisterProblem(sqlKey, ReportProblem.ForeignKeyNameChanged);
                            ...
                            foreach (var sqlKey in sqlOnlyKeys.Where(k => !k.AccountedFor))
                                _report.RegisterProblem(sqlKey, ReportProblem.ForeignKeySqlOnly);

                            foreach (var entityKey in entityOnlyKeys.Where(entityKey => !parms.ForeignKeysToIgnore.Contains(entityKey.ConstraintName)))
                                _report.RegisterProblem(entityKey, ReportProblem.ForeignKeyEntityOnly);
                        }
                    }
                }
            }
            ...
            {
                _report.RegisterProblem(column, ReportProblem.ColumnNotInDatabase, null);
            }
        }
        ...
        private bool ProblemsExitInEfDefinitions(List<EfTableAttributes> tables)
        {
            ...
            try
            {
                foreach (var table in tables)
                {
                    ...
                    foreach (var column in table.Columns)
                    {
                        ...
                        ConfirmColumnPropertiesMatchSqlAnnotations(column);
                        //, column.SqlDataType,!column.Required, column.KeyColumn);
                    }
                }
            }
            ...
            return _report.Files.Any();
        }

        private void ConfirmColumnPropertiesMatchSqlAnnotations(EfColumnAttributes col)
        //,string sqlDataType, bool? sqlNullable, bool keyColumn)
        {
            ReportProblem error;
            ...
            _report.RegisterProblem(col, error, probDetails);
        }
        ...
        private void RegisterTableNotExists(EfTableAttributes tblObj)
        {
            _report.RegisterProblem(tblObj, ReportProblem.DbTableMissing);
        }

        private EfColumnAttributes RegisterMissingColumn(EfTableAttributes tblObj, string colName,
            string sqlDataType, bool nullable, bool keyColumn, bool computed)
        {
            _report.RegisterProblem(tblObj, colName, sqlDataType, nullable, keyColumn, FileSegment.EntityProperties,
            ...
            var ret = new EfColumnAttributes(_report)
            {
                Parent = tblObj,
                SqlName = colName,
                Required = !nullable,
        ...
        public class EfDiscrepancyReport
        {
            ...
            public EfDiscrepancyReport(string contextName)
            {
                _contextName = contextName;
            }
                ...
                ReportProblem problem, string problemDetails)
            {
                ...
                switch (problem)
                {
                    ...
                    case ReportProblem.ContextNotUnicodePropertyMissing:
                        file = GetFile(_contextName, _contextName);
            ...
            /// </summary>
            /// <param name="fk">The Foreign Key that's got the problem</param>
            ...
            internal void RegisterProblem(EfForeignKeyAttributes fk, ReportProblem problem)
            {
                switch (problem)
                {
                    ...
                    case ReportProblem.ForeignKeyNameChanged:
                        GetFile(_contextName, _contextName)
            ...
            internal void RegisterProblem(EfTableAttributes tblObj, ReportProblem problem, string problemDetails = null)
            {
                if (problem == ReportProblem.ForeignKeyMissingReverseDeclaration)
                {
                    GetFile(tblObj.SqlTableName, tblObj.EntityName)
                        .GetFileSegment(FileSegment.EntityForeignKeysInverse)
                        .CorrectionFragments.Add(
                            new EfDiscrepancyCodeCorrectionFragment(tblObj, problem, problemDetails, _contextName));
                }
                else if (problem == ReportProblem.DocumentNewTable)
                {
                    GetFile(_contextName, _contextName)
                        .GetFileSegment(FileSegment.ContextPropertySection)
                        .CorrectionFragments.Add(
                            new EfDiscrepancyCodeCorrectionFragment(tblObj, problem, null, _contextName));
        ...
        public class EfDiscrepancyCodeCorrectionFragment
        {
            ...
            public EfDiscrepancyCodeCorrectionFragment(EfTableAttributes tblObj, ReportProblem problem,
                string problemDetails, string contextName)
            {
                switch (problem)
                {
                    case ReportProblem.ForeignKeyMissingReverseDeclaration:
                        var details = problemDetails.Split(Environment.NewLine);
                        var detailDict = details.Select(detail => detail.Split('\t')).ToDictionary(detailParts => detailParts[0], detailParts => detailParts[1]);
                        var commentLine = "Missing the InverseProperty for a declared foreign key";
                        CorrectionCode =
                            (string.IsNullOrEmpty(commentLine)
                                ? ""
                                : $"// {commentLine}" + Environment.NewLine) +
                                  //$"// For Foreign Key: {SqlConstraintName ?? ConstraintName}" + Environment.NewLine +
                                  $"// For Foreign Key: {detailDict["constraintName"]}" + Environment.NewLine +
                        $"[InverseProperty(\"{tblObj.EntityName}\")]" + Environment.NewLine +
                        //$"public virtual ICollection<{ParentTable.EntityName}> {SqlPrincipalEntityNavigationProperty ?? PrincipalEntityNavigationProperty} {{ get; set; }}";
                        $"public virtual ICollection<{detailDict["declaringEntityNavigationProperty"]}> {detailDict["declaringEntityNavigationProperty"]}s {{ get; set; }}";
                        break;
                    case ReportProblem.DbTableMissing:
                        CorrectionCode =
                            ...
                            Environment.NewLine +
                            $"This will also cause you to have to update the {contextName} to remove the corresponding DB Set" +
                            Environment.NewLine +
        ...
        public enum ReportProblem
        {
            ...
            ForeignKeyMissingReverseDeclaration
        }
        ...
        private static string ClrDataTypeToString(Type dataType)
        {
            switch (dataType.ToString())
            {
                ...
                case "System.Decimal":
                    return "decimal";
                default:
                    throw new NotImplementedException($"ClrDataTypeToString() switch doesn't cover case: '{dataType}'");
            }
        }
