...
namespace Emar.Api.Controllers
{
    ...
    public class ImageController : ControllerBase
    {
        ...
        [HttpGet("patients/{patientId}/indicators/{imageName}/url", Name = nameof(GetPatientIndicatorImageURL))]
        [ProducesResponseType(200)] // (OK) - the resource is sent in the response
        [ProducesResponseType(400)] // (bad request) - indicates a bad request (e.g. wrong parameter)
        [ProducesResponseType(404)] // (not found) - the resource does not exits
        [HttpCacheExpiration(CacheLocation = CacheLocation.Public)]
        [HttpCacheValidation(MustRevalidate = true)]
        public IActionResult GetPatientIndicatorImageURL(
            [FromHeader(Name = "Accept")] string mediaType,
            [FromRoute(Name = "patientId")] long? patientId,
            [FromRoute(Name = "imageName")] string imageName,
            [FromQuery] bool detailsOnError = false
            )
        {
            ...
            string error;
            var result = GetPatientIndicatorImageURL(patientId.ToString(), GetPatientBy.Id, imageName, out error, detailsOnError);
            ...
            return Ok(result);
        } //end function GetPatientIndicatorImageURL

        private string GetPatientIndicatorImageURL(string number, GetPatientBy getPatientBy, string imageName, out string error, bool detailsOnError = false)
        {
            error = "";
            ...
            //Since we're only returning the URL here, we don't need to any validation on the file.
            //It should exist in the folder since it was selected by a user in PCED.
            //FileInfo fInfo = new FileInfo(filepath);
