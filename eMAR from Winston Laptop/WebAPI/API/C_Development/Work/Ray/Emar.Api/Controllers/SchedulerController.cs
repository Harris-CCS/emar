...
namespace Emar.Api.Controllers
{
    ...
    public class SchedulerController : ControllerBase
    {
        ...
        //Make a version of this that takes the brand name in from the request header rather than the route.
        //Winston Murdock, 08/12/2022. PC-27474
        /// <summary>
        /// Retrieve scheduler options by brand name while looking at the formulary settings to filter the list of medications.
        /// </summary>
        /// <param name="mediaType">Media type from Accept header.</param>
        ...
        /// <param name="brandName">The name of a medication.</param>
        /// <returns></returns>
        [HttpGet("site/{siteId}", Name = nameof(GetSchedulerOptionsSearchStringInheader))]
        [ProducesResponseType(typeof(SchedulerOptionsDto), 200)] // (OK) - the resource is sent in the response
        [ProducesResponseType(400)] // (bad request) - indicates a bad request (e.g. wrong parameter)
        [ProducesResponseType(404)] // (not found) - the resource does not exits
        //[ProducesResponseType(406)] // (not acceptable) - the server does not support the required representation
        public ActionResult<SchedulerOptionsDto> GetSchedulerOptionsSearchStringInheader(
            [FromHeader(Name = "Accept")] string mediaType,
            [FromRoute(Name = "siteId")] int? siteId,
            ...
            )
        {
            ...
            {
                return BadRequest("Brand name is MissingFieldException.");
            } //end if
        ...
        /// <param name="brandName">The name of a medication.</param>
        /// <returns></returns>
        [HttpGet("site/{siteId}/all", Name = nameof(GetSchedulerOptionsAllSearchStringInheader))]
        [ProducesResponseType(typeof(SchedulerOptionsDto), 200)] // (OK) - the resource is sent in the response
        [ProducesResponseType(400)] // (bad request) - indicates a bad request (e.g. wrong parameter)
        [ProducesResponseType(404)] // (not found) - the resource does not exits
        //[ProducesResponseType(406)] // (not acceptable) - the server does not support the required representation
        public ActionResult<SchedulerOptionsDto> GetSchedulerOptionsAllSearchStringInheader(
            [FromHeader(Name = "Accept")] string mediaType,
            [FromRoute(Name = "siteId")] int? siteId,
            [FromHeader(Name = "brandName")] string brandName
            )
