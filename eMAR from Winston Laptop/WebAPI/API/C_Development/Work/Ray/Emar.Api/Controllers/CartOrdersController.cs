...
namespace Emar.Api.Controllers
{
    ...
    public class CartOrdersController : ControllerBase
    {
        ...
        public CartOrdersController(ICartOrderService cartOrderService, IOrderService orderService, IPatientRepository patientRepository/*,
        ...
        public ActionResult<CartOrderDto> AddCartOrder(
            ...
            [FromHeader(Name = "isGroupitem")] bool? isGroupItem = false,
            [FromHeader(Name = "pathwayToLoad")] string? pathwayToLoad = ""
            )
        {
            if (!MediaTypes.IsValidMediaType(mediaType))
            {
            ...
            if (cartOrderBody.Duration != null && cartOrderBody.DurationUnitId != null)
            {
                var siteId = _patientRepository.GetSiteIdForPatient(patientId);
                FrequencyScheduleDto frequencySchedule = _orderService.GetFrequencies(siteId).First(x => x.Id == cartOrderBody.FrequencyId);
                if (frequencySchedule != null && frequencySchedule.FrequencyType.Name.ToLowerInvariant().Equals("continuous"))
                {
                    DurationUnitDto durationUnit = _orderService.GetDurationUnits().First(x => x.Id == cartOrderBody.DurationUnitId);
                    if (durationUnit != null)
                    {
                        cartOrderBody.EndDatetime =
                            cartOrderBody.BeginDatetime.AddMinutes((double)(cartOrderBody.Duration * durationUnit.DurationInMinutes));
                    }
                }
            }
            ...
            //If this is a group item (i.e. isGroupItem has a value and that value is true...
            //then tell Med Svc to load this specific pathway on the groups tab.
            //Winston Murdock, 10/03/2022.  PC-27538
            if (isGroupItem.HasValue ? isGroupItem.Value : false)
            {
                order.TabToLoad = "groups";
                order.PathwayName = DecodeString(pathwayToLoad);
            } //end if
