...
namespace Emar.Core.OutboundChart.Service
{
    public class OcsEmarOutboundService : IOcsEmarOutboundService
    {
        ...
        public string SendChartLinesAsync(OcsChartParameters ocsChartParams)
        {
            ...
            if (!order.Id.ToLowerInvariant().Equals("ft"))
            {
                ...
                if (!isComboMed)
                {
                    ...
                    component.PackagingId = medDetails.PackagingId;
                    ...
                    component.Service = _emarOutboundDataRepository.GetServiceByNdc(medDetails.PackagingId);
            ...
            //// Put new method CreateMeds here ////
            //// Substitute block above for CreateMeds call like in SendChartTemplateMarkup ////
        ...
        public string SendChartTemplateMarkup(List<OcsPromptParameters> orderedPromptList, int siteId, long patientOrderId, int userId, ActionEnum action, long adminId, bool newOrderAdmin)
        {
                ...
                switch (orderedPrompt.promptType)
                {
                    ...
                    case PromptType.threeStateButton:
                        ...
                        else
                        {
                            ...
                            // need promptMarkup when for vitals followup?
            ...
            {
                addTrxEntryForFormulary(patientOrderId, patId, patIbex, externalSiteId, externalUserId, enteredTime, siteId);
            }
            ...
            // BEGIN SECTION
            if (action == ActionEnum.PharmVerification)
            {
                updatePatientPharmacyVerification(patId, patientOrderId, patIbex, externalSiteId);
            }

            // update the vital data in the patients table if flag set - can only be action of FollowUp
            if (updateVitals)
            {
                addPatientVitals(vitalObj, patIbex, externalSiteId, externalUserId, (byte)siteId);
            }

            // update the patient medication service indicator if an appropriate action type
            // Need to add Complete, Repeat, Modify?
            if (action != ActionEnum.PharmVerification && action != ActionEnum.MissedDose
             && action != ActionEnum.Reschedule        && action != ActionEnum.FollowUp
             && activePatient)
            {
            ...
            {
                var site = new Site(externalSiteId);
                var patient = new Patient();
                patient.Ibex = patIbex;
                var patientDataForIbex = _emarOutboundChartRepository.GetPatientDataForIbex(patId);
                patient = MapPatientDataForIbex(patient, patientDataForIbex);
                var patientOrderParams = _emarOutboundChartRepository.GetPatientOrderDataForMeds(patientOrderId);
                ...
                var meds = CreateMeds(site, patient, externalUserId, patientOrderParams, losecsDerived, losecsFromOrder != losecsDerived);

                CreateTrigger(site, patient, externalUserId, Medication.ActionConstants.MEDICATION_MAP[action.ToString()].ToLower(), meds);
            }
            ...
            foreach (EMR.Line line in emr.Lines)
            {
                ...
                if (line.NCT() == EMR.Constants.NCT_MED_SVC)
                {
                    ...
                    if (losecs.Contains(losecsFromOrder) || losecsFromOrder.Equals(line.TableXRef()))
                    {
                        ...
                        // if not a new order admin or if a pharm verif action, then use current data segment. Else start anew.
                        var lineData = (newOrderAdmin == false || pharmVerificationAction) ? newLine.data() : new List<EMR.Line.DataSegment>();
                        // pharmacy verification requires special chart manipulation - remove the "needed" text line
                        ...
                        if (pharmVerificationAction)
                        {
                            ...
                            {
                                var lineString = segment.ToString();
                                // remove first 3 markup chars - don't want to duplicate them later and negate the chart line
                                // only a concern for the first data segment or for all data segments?
                                if (lineString.StartsWith("^s="))
                        ...
                        var resendPharmVerif = false;
                        if (action == ActionEnum.Cancel || action == ActionEnum.MissedDose
                         || action == ActionEnum.Reschedule || action == ActionEnum.Hold
                         || discontinueOrDiscontinued)
                        {
                            // next check dispo type code
                            var dispoTypeCode = _emarOutboundChartRepository.GetDispoTypeCodeFromPatient(patId);
                            if (dispoTypeCode == "INP" || dispoTypeCode == "INPT" || dispoTypeCode == "OBS")
                            {
                                resendPharmVerif = true;
                            }
                        }
                        // determine if additional newlines are needed - hack until the newline handling is solved
                        var newOrderAdminNewLines = "";
                        // TODO - uncomment following two lines and test
//                        if (newOrderAdmin)
//                            newOrderAdminNewLines = "\n\n";
                        // add action line including newOrderAdminNewLines
                        var newSegment = new EMR.Line.DataSegment(
                            EMR.Line.DataSegment.Constants.TYPE_DROPDOWN,
                            string.Format("{0}\n{1} by: {2} {3}", newOrderAdminNewLines, actionTitle, ordererName, orderDatetime)
                        );
                        ...
                        if (!string.IsNullOrWhiteSpace(templateMarkup))
                        {
                            var newSegment2 = new EMR.Line.DataSegment(
                                EMR.Line.DataSegment.Constants.TYPE_DROPDOWN,
                                string.Format("\n&{0}{1}", templateMarkup, resendPharmVerif ? "\n" : "")
                            );
                        ...
                        if (resendPharmVerif) {
                            ...
                            var newSegment3 = new EMR.Line.DataSegment(
                                EMR.Line.DataSegment.Constants.TYPE_DROPDOWN,
                                string.Format("\n{0}: Entered By {1} {2}", pharmVerifText, ordererName, orderDatetime)
                            );
                            lineData.Add(newSegment3);
                        }
                        ...
                        // do not inactivate a PharmVerification type action - happens at order level
//                        if (pharmVerificationAction)
//                            break;
            ...
            if (updateVitals)
            {
                var vitalsLines = _emarOutboundChartRepository.ChartVitalsEntries(externalUserId, losecsFromOrder, enteredTime, vitalObj);
                if (vitalsLines != null)
...
//                        }
        ...
        public string updatePatientMedicationIndicator(string ibex, int siteId)
        {
            try
            {
                ...
                // if index was set then update the patient record
                if (!string.IsNullOrWhiteSpace(index)) {
        ...
        public string updatePatientPharmacyVerification(long patientId, long patientOrderId, string extPatientId, byte externalSiteId)
        {
            // find the patient record
            ...
            var pharmVerificationStatus = _emarOutboundChartRepository.GetPharmVerificationReqStatus(patientOrderId, patientId);
