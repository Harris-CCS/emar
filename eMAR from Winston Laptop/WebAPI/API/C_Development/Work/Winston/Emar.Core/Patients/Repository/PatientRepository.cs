...
namespace Emar.Core.Patients.Repository
{
    public class PatientRepository : IPatientRepository
    {
        ...
        private IEnumerable<Patient> GetPatients(Expression<Func<Patient, bool>> wherePredicate, bool includeOrders = true)
        {
            ...
            //Instead of getting all patients entries from external_ids
            //and then grabbing the external_id for each one,
            //make individual calls to get the external_id.
            //Winston Murdock, 11/28/2023.  PC-28220
            //var extIds = patients
            ...
            {
                //Go get the external_id for this patient from the DB rather than from the
                //list of all "patients" rows from the external_ids table.
                //p.ExternalId = extIds.FirstOrDefault(x => x.InternalId == p.Id);
                //Winston Murdock, 11/28/2023.  PC-28220
                p.ExternalId = _context.ExternalIds.Where
                                (
                                    x => x.InternalId == p.Id
                                    && x.Entity == "patients"
                                    && x.Vendor == "pulsecheck"
                ...
                //Grab the site.
                //I could have probably added it to the query above.  But it's already busy enough.
                //And this ensures that we get the info without potentially bogging down the query above.
                //Winston Murdock, PC-27715
                p.Site = _context.Sites.Where(s => s.Id == p.SiteId).FirstOrDefault();
            } //end foreach patient
