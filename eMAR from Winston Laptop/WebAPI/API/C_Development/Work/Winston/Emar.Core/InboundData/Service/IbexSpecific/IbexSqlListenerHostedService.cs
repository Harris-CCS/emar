...
namespace Emar.Core.InboundData.Service.IbexSpecific
{
    ...
    public class IbexSqlListenerHostedService : BackgroundService
    {
        ...
        public IbexSqlListenerHostedService(IServiceProvider services, ILogger<IbexSqlListenerHostedService> logger,
            SqlQueueNotificationChannel channel, IConfiguration configuration)
        {
            _services = services;
            _logger = logger;
            _channel = channel;
        ...
        private async Task DoWork(CancellationToken stoppingToken)
        {
            ...
            var ibexConnectionString = _configuration.GetConnectionString("IbexSqlConnection");
            ...
            using (var scope = _services.CreateScope())
            {
                ...
                var logger = scope.ServiceProvider
                    .GetRequiredService<ILogger<EmarUpdateQueueSqlTableDependencyReactiveProvider>>();

                await EmarUpdateQueueSqlTableDependencyReactiveProvider.DoWork(logger, _channel, stoppingToken, ibexConnectionString);
            }
        }
