...
namespace Emar.Core.Orders.Service
{
    public class OrderService : IOrderService
    {
        ...
        public CartOrderDto CopyQuickListItemToCart(in int quickListItemId, BaseLinkResource resource, int? duration = null, int? durationUnitId = null)
        {
            ...
            SetContinuousOrderEndTime(ref cartOrder);

            var newCartOrder = _cartOrderRepository.AddCartOrder(cartOrder);
            var drugDbVendor = _optionRepository.GetOption(siteId, OptionNames.DRUG_DB_VENDOR);
            var codeShareSites = _orderRepository.GetCodeShareSites(siteId).ToList();

            //Call a helper function that recalculates all interactions/reactions for all cart orders for this user/patient.
            //Prior to this, we were only recalculating them for the cart order we just added and not for all cart orders for this user/patient.
        ...
        private List<GroupListItemDto> AddInteractionsReactionsCheckForComboMeds(List<GroupListItemDto> comboMeds, BaseLinkResource resource)
        {
            ...
            foreach (GroupListItemDto comboMed in comboMeds)
            {
                ...
                foreach (var medDetail in comboMed.Medication.MedicationDetails)
                {
                    ...
                    //Not sure why, but we were passing in false for "check cart orders" when calling
                    //the interaction checking logic.  Thus, combo med group items were not showing
                    //interactions to cart orders but only placed orders.
        ...
        public SchedulerOptionsDto GetSchedulerSetupData(int siteId, EmarOrderType itemType, int itemId, int? duration = null, int? durationUnitId = null, DateTimeOffset? endDateTime = null)
        {
            ...
            {
                //Get now in the site's time zone.
                //Then use for the comparison to the stop time of the existing order/cart order.
                //Winston Murdock, 08/02/2024.  PC-28315
                ...
                if (itemType == EmarOrderType.PatientOrder)
                {
                    ...
                    //This is causing an error when it's earlier than siteNow.
                    //In that case, leave the endDateTime as null.
                    //Winston Murdock, 08/02/2024.  PC-28315
                    if (thisOrder.EndDateTime.HasValue)
                    {
                        if (thisOrder.EndDateTime >= siteNow)
                        {
                ...
                else if (itemType == EmarOrderType.PatientCartOrder)
                {
                    ...
                    //In that case, leave the endDateTime as null.
                    //Winston Murdock, 08/02/2024.  PC-28315
                    if (thisCartOrder.EndDatetime.HasValue)
                    {
                        if (thisCartOrder.EndDatetime >= siteNow)
                        {
        ...
        {
            //This function was copied from CartOrderService since adding a remebered list item straight
            //to the cart was only recalculating the interactions/reactions for that one cart order and
            //not for all of them.  When an existing cart order interacted with the new cart order, we
            //would only show one side of that order (new cart order to existing cart order).
            //We can't just add a reference to CartOrderService here since it has a reference to OrderService.
            //That would create a circular reference.
            //Winston Murdock.  02/09/2023.  PC-27720
            ...
            //1) Add a quick list item straight to cart.
            //2) Add a department preferred list item straight to cart.
            //3) Add a group item straight to cart.
            ...
            foreach (var cartOrder in cartOrders)
            {
                ...
                if (cartOrder.Medication.DrugId == "COMBO")
                {
                    ...
                    foreach (var medDetail in cartOrder.Medication.MedicationDetails)
                    {
                        ...
                        //Also remove any cart orders (and regular orders) that have the same medication id.
                        ...
                        //Remove any other orders with the same medication ID.
                        //This way we don't check a combo med against another instance of itself.
        ...
        // If this is a continuous order, force the end date time to start date time + duration.
        public void SetContinuousOrderEndTime(ref PatientCartOrder cartOrder)
        {
            if (cartOrder.Duration != null && cartOrder.DurationUnitId != null)
            {
                var siteId = _patientRepository.GetSiteIdForPatient(cartOrder.PatientId);
                int? frequencyId = cartOrder.FrequencyScheduleId;
                FrequencyScheduleDto frequencySchedule = GetFrequencies(siteId).First(x => x.Id == frequencyId);
                if (frequencySchedule != null && frequencySchedule.FrequencyType.Name.ToLowerInvariant().Equals("continuous"))
                {
                    int? durationUnitId = cartOrder.DurationUnitId;
                    DurationUnitDto durationUnit = GetDurationUnits().First(x => x.Id == durationUnitId);
                    if (durationUnit != null)
                    {
                        cartOrder.EndDatetime =
                            cartOrder.BeginDatetime.AddMinutes((double)(cartOrder.Duration * durationUnit.DurationInMinutes));
                    }
                }
            }
        }
