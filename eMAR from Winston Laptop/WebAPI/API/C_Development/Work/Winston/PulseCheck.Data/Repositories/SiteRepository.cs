...
namespace PulseCheck.Data.Repositories
{
    public class SiteRepository : BaseRepository, ISiteRepository
    {
        ...
        public async Task<List<Group>> GetMedPathwaysBySiteIdAsync(byte id)
        {
            var result = await _context.GetSiteGroups(id).ToListAsync();
            return result.Select(g => new Group
            {
                Name = g.Name,
                Num = g.Num,
                Type = g.Type,
        ...
        /// <param name="id">Site identifier</param>
        /// <returns>List of Group objects</returns>
        public async Task<List<Group>> GetOrderPathwaysBySiteIdAsync(byte id) {
