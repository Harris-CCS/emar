...
namespace Emar.Core.MedicationReactions
{
    ...
    public class DrugDb
    {
        ...
        private IEnumerable<Dictionary<string, object>> LoadAlgMedTable(long patientId, string drugDbVendor, bool confirmedOnly = false)
        {
            ...
            var algResult = DB.ConvertDataSetToListOfDictionariesObj(
                _patientRepository.GetAllergiesByPatientId(patientId, a => a.IsActive && (a.ActionStatus == "C" || a.ActionStatus == "U"))
                    .ToList()
                    .ToDataSet());
            ...
            var homeMedsResult =
                DB.ConvertDataSetToListOfDictionariesObj(
                    _homeMedicationRepository.GetPatientHomeMedications(a => a.PatientId == patientId && a.IsActive)
                        .ToList()
                        .ToDataSet());
