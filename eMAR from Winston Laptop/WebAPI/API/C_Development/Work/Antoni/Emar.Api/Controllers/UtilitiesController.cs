...
namespace Emar.Api.Controllers
{
    ...
    public class UtilitiesController : ControllerBase
    {
        ...
        )
        {
            try
            {
                object rpt = context.ToLowerInvariant() switch
                {
                    "emarcontext" => new EfToDbSynchHelper(_emarContext).CompareEfToDb(parm),
                    "ibexcontext" => new EfToDbSynchHelperIbex(_ibexContext).CompareEfToDb(parm),
                ...
                if (rpt.GetType() == typeof(EfToDbSynchHelper.EfDiscrepancyReport))
                    return Conflict(((EfToDbSynchHelper.EfDiscrepancyReport)rpt).CreateOutputText());
                if (rpt.GetType() == typeof(EfToDbSynchHelperIbex.EfDiscrepancyReport))
                    return Conflict(((EfToDbSynchHelperIbex.EfDiscrepancyReport)rpt).CreateOutputText());
                if (!debugOutput)
                    return Ok();
                if (context.ToLowerInvariant() == "emarcontext")
                    return Ok((IEnumerable<EfToDbSynchHelper.EfTableAttributes>)rpt);

                return Ok((IEnumerable<EfToDbSynchHelperIbex.EfTableAttributes>)rpt);
            }
            catch (Exception e)
            {
        ...
        public ActionResult<string> AddTablesToEfConfiguration([FromBody] EfToDbSynchHelperParams parm)
        {
            try
            {
                var rpt = new EfToDbSynchHelper(_emarContext).AddTables(parm);
            ...
            )
        {
            ...
            {
                return Problem(Utilities.ExtractExceptionMessages(ex), statusCode: (int)HttpStatusCode.InternalServerError);
            }
