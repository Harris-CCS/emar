...
namespace Emar.Core.Orders.Service
{
    public interface IOrderService
    {
        ...
        CartOrderDto CopyQuickListItemToCart(in int quickListItemId, BaseLinkResource resource);
        ...
        IEnumerable<MedicationInteractionReaction> CheckInteractionsReactions(in int userId, List<MedicationModel> medicationList, long patientId);

        // Scheduler Support methods
        SchedulerOptionsDto GetSchedulerSetupData(int siteId, string brandName);
        SchedulerOptionsDto GetSchedulerSetupData(int siteId, EmarOrderType itemType, int itemId);
        ...
        IEnumerable<FrequencyScheduleAdministrationDto> GetNewAdministrations(int siteId, int frequencyId, DateTimeOffset? start, DateTimeOffset? stop);
