            ...
            foreach (var medicationDetail in medicationItem.Medication.MedicationDetails)
            {
                ...
                if (drugTrigger.ContainsKey(dnum))
                {
                    ...
                    foreach (var sel in s)
                    {
                        ...
                        var d = key.Equals("A") ? rsel.GetValueOrDefault("drug_id_2", null) : null;
                        rsel["dnum"] = d;
                        rsel["drug"] = drug;

                        if (key.Equals("A"))
                        {
                            if (reactionsCheckResult.Allergies[dnum].ContainsKey(drug.ToString().Trim('\'')))
                            {
                                rsel["SourceTable"] = reactionsCheckResult.Allergies[dnum][drug.ToString().Trim('\'')]["SourceTable"];
                                rsel["SourceTableId"] = reactionsCheckResult.Allergies[dnum][drug.ToString().Trim('\'')]["SourceTableId"];
                                rsel["Severity"] = reactionsCheckResult.Allergies[dnum][drug.ToString().Trim('\'')]["Severity"];
                            }
