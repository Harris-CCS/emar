...
namespace Emar.Core.Helpers
{
    ...
    public static class Trigger
    {
            ...
            try
            {
                ...
                using (var con = new SqlConnection(connection))
                {
                    using (SqlCommand cmd = new SqlCommand(sql, con))
                    {
                            ...
                            cmd.Parameters.Add("@instance_id", SqlDbType.Int).Value = Constants.INSTANCE_ID_BY_INTERFACE[interfaceName];
                            cmd.Parameters.Add("@path", SqlDbType.VarChar).Value = Constants.MESSAGE_ID_PATH[interfaceName];
                            cmd.Parameters.Add("@site", SqlDbType.TinyInt).Value = siteId;
        ...
        private static string WriteTriggerDB(ISite site, string patientId, int userId, string triggerContent, string interfaceName, string sourceName, bool append = false)
        {
            ...
            if (patient.Count == 0 || string.IsNullOrWhiteSpace(patient["ibex"]))
            {
                return InvalidValueError("patient ibex");
            }
            // Validate site
            if (site == null || site.Id < 1)
            {
                return InvalidValueError("site id");
            }
            // Validate trigger xml
            if (string.IsNullOrWhiteSpace(triggerContent))
            {
                return InvalidValueError("trigger xml");
            }
        ...
        private static XElement CreateTriggerDbXml(dynamic obj)
        {
            ...
            var trigger = new XElement(
                ...
                new XAttribute("Site", obj.org.Id.ToString()),
            ...
            using (var con = new SqlConnection(connection))
            {
                using (SqlCommand cmd = new SqlCommand("pc_interface_trigger_create", con))
                {
                    try
                    {
                        ...
                        cmd.Parameters.Add("@tped_message_id", SqlDbType.Int).Value = int.Parse(obj.trigger_id);
                    ...
                    {
                        DTFL.Write(obj.org.Id, obj.userid, ex, "pc_interface_trigger_create", cmd.Parameters.Cast<SqlParameter>().ToArray());
