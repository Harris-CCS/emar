...
namespace Emar.Core.Patients.Service
{
    public class PatientService : IPatientService
    {
        ...
        public PagedList<PatientDto> GetPatients(PatientsResourceParameters resourceParameters, bool includeOrders)
        {
            ...
            return new PagedList<PatientDto>(patientList, patients.TotalCount, patients.CurrentPage, 100);
        }
        ...
        public PatientDto GetAdministrationsByPatientAndOrder(long patientId, long orderId, int userId)
        {
            //Call the repository to get the data.
            var patient = _patientRepository.GetAdministrationsByPatientAndOrder(patientId, orderId);
            
            //Get the patient's siteId.
            var siteId = patient.SiteId;

            //Get the drug vendor.
            var drugDbVendor = _optionRepository.GetOption(siteId, OptionNames.DRUG_DB_VENDOR);

            //Get the list of code share sites.
            var codeShareSites = _orderRepository.GetCodeShareSites(siteId).ToList();

            //This only matters for the available actions.
            //We'll pass in null which just means we won't setup any available actions.
            //We don't need them here, so this is fine.
            OrderActionMapperHelper actionHelper = null;
            //actionHelper = new OrderActionMapperHelper(_templateRepository, patient.SiteId, orderLinkBase, adminLinkBase);

            //Call the existing MapPatient to map the patient, orders, and administrations.
            //Yay code reuse!
            var patientDto = PatientMapper.MapPatient(patient, drugDbVendor, null, codeShareSites, userId);
