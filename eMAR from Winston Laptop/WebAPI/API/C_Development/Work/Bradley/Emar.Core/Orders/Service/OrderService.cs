    ...
    public class OrderService : IOrderService
    {
        ...
        public void UpdatePatientOrderInteractionsAndReactions(long patientId, List<int>? medicationIds = null, bool? deleteExisting = false)
        {
            ...
            foreach (var order in activeOrders)
            {
                ...
                UpdateOrderInteractionsAndReactions(siteId, patientId, order.Id, order.MedicationId, order.AddUserId, EmarOrderType.PatientOrder, items, newOrders, existingOrders, cartOrders, deleteExisting);
            }
            foreach (var order in cartOrders)
            {
                ...
                UpdateOrderInteractionsAndReactions(siteId, patientId, order.Id, order.MedicationId, order.UserId, EmarOrderType.PatientCartOrder, items, newOrders, existingOrders, cartOrders, deleteExisting);
            }
        }
            ...
            IEnumerable<PatientOrder>? existingOrders = null, IEnumerable<PatientCartOrder>? cartOrders = null, bool? deleteExisting = false)
        {
            ...
            var interactionsReactions = CheckInteractionsReactions(userId, items, patientId, orderType == EmarOrderType.PatientCartOrder, newOrders, existingOrders, cartOrders);

            //Since we're doing target interaction/reaction calculations when checking out a cart 9and adding to the cart).
        ...
        public SchedulerOptionsDto GetSchedulerSetupData(int siteId, EmarOrderType itemType, int itemId, int? duration = null, int? durationUnitId = null, DateTimeOffset? endDateTime = null)
        {
            ...
            //Winston Murdock, 02/15/2022.  PC-27021
            return OrderMapper.MapSchedulerSetupData(medications.FirstOrDefault()?.DisplayName, medications, antimicrobialRequiredIndicators, administrations, orderInstructions, codeShareSites, endDateTime);
        }
