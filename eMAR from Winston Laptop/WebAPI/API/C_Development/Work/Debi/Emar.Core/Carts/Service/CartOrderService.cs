...
namespace Emar.Core.Carts.Service
{
    public class CartOrderService : ICartOrderService
    {
        ...
        public CartOrderDto AddCartOrder(CartOrderIuDto cartOrderAddDto, BaseLinkResource resource)
        {
            ...
            _orderService.SetContinuousOrderEndTime(ref order);
        ...
        public bool UpdateCartOrder(CartOrderIuDto cartOrderUpdateDto)
        {
            ...
            //Since we need to recalculate the interactions and reactions
            //for all cart orders, save the return value in a variable.
            //Then return it below.
            bool bRet =  _cartOrderRepository.UpdateCartOrder(order);

            //Recalculate the interactions and reactions for all cart orders.
            //We shouldn't need to do this here.
            //But updating a cart order is losing the interactions.
            //Winston Murdock, 02/10/2023.  PC-27720
            RecalculateCartOrderInteractionsAndReactions(cartOrderUpdateDto.PatientId, siteId, cartOrderUpdateDto.UserId);
        ...
        {
            //Delete the cart order (and its children), then return.
            return _cartOrderRepository.DeleteCartOrder(cartOrderId);

            //I don't need to get the cart order or patient's site ID any more.
            //Once upon a time, i was recalculating the interactions/reactions here.
            //We ended up not needing to do that, and I removed that logic from this method.
            //But I left the two lines below that were getting the cart order and patient's site id.
            //Something about having these two lines here was causing issues related to deleting
            //the cart order when the cart order is a combo med and when the patient has allergies
            //to one, or more, medications inside the combo med.
            //
            //Database operation expected to affect 1 row(s) but actually affected 4 row(s).
            ...
            //
            //Credit to Jim Hoos for finding this error.
        ...
        {
            //Similar to the method above, just delete the cart rather than doing the stuff
            //we were doing back when I was trying to recalculate the interactions and reactions here.
            //Winston Murdock, PC-27804
        ...
        {
            var bret = _cartOrderRepository.CheckoutOrders(cartPreCheckoutResponseData, userId, patientId);

            //Now that we've checked out the cart orders, delete everything in the cart for this user/patient.
            //Only do this if we were successful in checking out the cart.
            //IF there was an issue somewhere in the checkout cart process, then don't delete the cart orders.
            //Winston Murdock, 02/27/2023.  PC-27804
            if (bret)
            {
                var bDeleteRet = DeleteCartOrders(userId, patientId);
            } //end if
