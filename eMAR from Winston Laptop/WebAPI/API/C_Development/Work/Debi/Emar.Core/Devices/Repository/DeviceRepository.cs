...
namespace Emar.Core.Devices.Repository
{
    public partial class DeviceRepository : IDeviceRepository
    {
        ...
        public DeviceRepository(EmarContext emarContext, IUserRepository userRepository)
        {
        ...
        public string SendChartAdminEntry(string extPatientId, byte extSiteId, int extUserId, string data, bool activePatient)
        {
            // create EMR object - note activePatient flag
            var EMR = new EMR(extSiteId, extPatientId, true, false, !activePatient);
    ...
    //        line.LinePart.part = "MEDICATION SERVICE PRINTED";
        ...
        public string GetInternalDeviceId(int siteId, string externalDeviceId)
        {
            // If no external ID is found, 0 will be returned.
            var query =
                (
                    from e in _context.ExternalIds
                    where e.ExternalId == siteId + "|" + externalDeviceId &&
                        e.Entity == "devices" &&
                        e.Vendor == "pulsecheck"
                    select e.InternalId.ToString()
                );
        ...
        public bool GetPatientStatus(long patId)
        {
        ...
        public string SavePrintFile(Dictionary<string, string> printFileResponses = null)
        {
            try
            {
                ...
                // must determine if UNC path before replacing backslashes
                var uncPath = false;
                if (sPath.StartsWith(@"\\"))
                    uncPath = true;
                ...
                //Handle that here.
                sPath = sPath.Replace("\\\\", "\\");
                // add the extra backslash back to the front of a UNC path
                if (uncPath)
                    sPath = @"\" + sPath;
                ...
                if (bWriteWorked)
                {
                    ...
                    //Now that we've printed the document, save the printer that they used as this user's last used device.
                    //Winston Murdock.  07/13/2021.  EMAR-1088

                    //Get the row in the user_settings table for this user and for last used device.
                    var userLastUsedDevice = _context.UserSettings.Where
                    (
                        us => us.UserId == printHistory.PrintUserId &&
                        us.SettingId == _userRepository.GetSettingId("LAST_USED_PRINTER")
                    ).FirstOrDefault();

                    //Now that we've got the UserSettings entity, set the value to the device id we just printed to.
                    userLastUsedDevice.SettingValue = printHistory.DeviceId.ToString();

                    //Save things back to the DB.
                    //This is both the insert into the print_history table
                    //and the user's last used device in the user_settings table.
                    _context.SaveChanges();
