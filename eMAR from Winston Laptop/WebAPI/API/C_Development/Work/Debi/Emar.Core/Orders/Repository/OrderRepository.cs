...
namespace Emar.Core.Orders.Repository
{
    public class OrderRepository : IOrderRepository
    {
        ...
        public List<OrderAdministration> GetAllPastDueAdministrationsForPatient(long patientId)
        {
            //Get all administrations that are in the past and have not been given yet.
            //Winston Murdock, 07/22/2022.  PC-27276

            //Calculate siteNow.
            var siteId = _context.Patients.Find(patientId).SiteId;
            var siteNow = _siteRepository.GetSiteTimeZone(siteId).NowWithTimeZoneOffset();

            //Get the list of order ids for this patient that are not pending or ongiong
            //and that are for this patient
            //and that don't have any missed doses.
            var orders = _context.PatientOrders.Where
                (po =>
                    po.PatientId == patientId &&
                    !po.OrderAdministrations.Any(oa => oa.MissedDose == true) &&
                    po.OrderStatus != OrderStatus.Cancelled.ToString() &&
                    po.OrderStatus != OrderStatus.Deleted.ToString() &&
                    po.OrderStatus != OrderStatus.Completed.ToString() &&
                    po.OrderStatus != OrderStatus.Discontinued.ToString() &&
                    po.OrderStatus != OrderStatus.PendingDiscontinue.ToString() &&
                    po.OrderStatus != OrderStatus.OnHold.ToString()
                ).Select(po => po.Id).ToList();

            //Now that we have all of the order ids for this patient, get all of the administrations
            //for those orders where the administration has not been given yet.
            var ret = _context.OrderAdministrations.Where
                (oa =>
                    orders.Contains(oa.PatientOrderId) &&
                    !oa.AdministrationDatetime.HasValue &&
                    oa.AdministrationScheduledDatetime <= siteNow
                ).ToList();

            //Return.
            return ret;
        } //end GetAllPastDueAdministrationsForPatient
        ...
        public List<string> GetGroupNames(int siteId, string? departmentCode)
        {
            //Return the list of groups for the specified site/department.
            //If no department is specified, then get all groups for the site.
            //If a department is specified, then only get groups for that department at this site.
            //Winston Murdock, 07/18/2022.  PC-27238
            List<string> items = new List<string>();

            //If we don't have a department code, then don't use it to filter.
            //Else, do use it to filter.
            if (string.IsNullOrWhiteSpace(departmentCode))
            {
                //Do not include department code in the filter.
                items = 
                (
                    from gli in _context.GroupListItems
                    where gli.SiteId == siteId
                    select gli.GroupName
                ).Distinct().ToList();
            }
            else
            {
                //Do include department code in the filter.
                items =
                (
                    from gli in _context.GroupListItems
                    where gli.SiteId == siteId
                    && gli.DepartmentCode == departmentCode
                    select gli.GroupName
                ).Distinct().ToList();
            } //end if

            //Don't sort here.  Let the service do the sorting.
            //var ret = items.OrderBy(gli => gli).ToList();

            //Return.
            return items;
        } //end GetGroupNames

        public List<GroupListItem> GetGroupItemsByGroupName(string departmentCode, BaseLinkResource resource, string groupNameForFilter)
        {
            //Get all of the items in the specified group for the specified site.
            //This is the same logic as the original call that got all group items
            //for a specified site but with added filtering on the name of the group.
            ...
            {
                //Do not include department code in the filter.
                whereLambda = s => s.SiteId == resource.SiteId && s.GroupName == groupNameForFilter;
            }
            else
            {
                //Do include department code in the filter.
                whereLambda = s => s.SiteId == resource.SiteId && s.GroupName == groupNameForFilter && s.DepartmentCode == departmentCode;
            }

            //Get the group orders that match the lambda we have created.
            var items = _context.GroupListItems
                .Where(whereLambda)
                .Include(i => i.Medication)
                    .ThenInclude(m => m.MedicationDetails)
        ...
        {
            //Go out to site_code_shares to find the site we're pulling each item (units, routes,
            //schedules, etc...) from (i.e. target site id) for the site we are logged into (i.e.
            //source site id).
            //Winston Murdockm 04/21/2021.  EMAR-811.

            //I completely rewrote this method.
            //1) Rather than getting each of the nine rows in separate DB hits, get them all at once.
            //2) Cache them in memory so that we only go get them from the DB one time and then pull
            //     them from memoty each time after that.
            //3) Return the list to the calling method.
            //
            //By changing how this guy works but keeping the parameters and return value the same,
            //I don't have to change anyone who calls this.
            //
            //This is an amalgmation of Brad's method (directly below this) which returns the target
            //site id (from) for the specified entity and the previous version of this method that
            //returns a list of all code share ids.
            //
            //Currently, we don't code share override reasons, antimicrobial indications, services,
            //formulary and vital signs.  They are in the DB, and this pulls their rows.  But nothing
            //is setup to code share them in the API yet.
            //Winston Murdock, 09/28/2022.  PC-27511

            //Get the data from the cache if it's in there.
            //If it's not in there, get it from the DB and add it to the cache
            //so that all future calls pull it from the cache.
            //This is a list of CodeSharedId entities.
            var codeShares = _cache.GetOrCreate($"{siteId}{CacheKeys.CodeSharesList}", entry =>
            {
                entry.SlidingExpiration = TimeSpan.FromMinutes(30);

                var codeShares = _context.SiteCodeShares
                        .Where(s => s.SourceSiteId == siteId)
                        .ToList();

                List<CodeSharedId> ret = new List<CodeSharedId>();
                foreach (var codeShare in codeShares)
                {
                    //Get the code share entity and, if it isn't null, add this one to the return list.
                    var cs = GetCodeShareEnum(codeShare.Entity);
                    if (cs != null)
                    {
                        //Entity is not null.
                        //Add this one to the return list, if it's not already in there.
                        
                        //Since we had an issue in Brad's method below with attempting to add
                        //an entry into the list twice (which was fixed by this conditional)
                        //add a similar conditional here as a better safe than sorry idea.
                        //Winston Murdock, 10/07/2022.  PC-27570
                        if (!ret.Any(a => a.Entity == (CodeShareEntity)cs))
                        {
                            ret.Add
                            (
                                new CodeSharedId
                                {
                                    //We pull from the Target site and to the Source site.
                                    SiteId = codeShare.SourceSiteId,
                                    Entity = (CodeShareEntity)cs,
                                    SharedSiteId = codeShare.TargetSiteId
                                }
                            );
                        } //end if
                    } //end if (code share entity is not null?)
                } //end foreach
            ...
            //Return the list of CodeSharedId entities.
            return codeShares;
        } //end GetCodeShareSites
        ...
        public int GetCodeShareSites(int sourceSiteId, CodeShareEntity entity)
        {
            var codeShares = _cache.GetOrCreate($"{sourceSiteId}{CacheKeys.CodeShares}", entry =>
            {
                ...
                foreach (var codeShare in codeShares)
                {
                    ...
                    if (cs != null)
                    {
                        //if this one has not already been added, add it.
                        //We were seeing an error related to this on 57c,
                        //which is what prompted me to add this check.
                        //Winston Murdock, 10/07/2022.  PC-27570
                        if (!ret.Any(a => a.Key == (CodeShareEntity)cs))
                        {
        ...
        private Expression<Func<Medication, bool>> GetSchedulerSetupDataWhereExpression(int siteId, Expression<Func<Medication, bool>> baseExpression, string drugId = null)
        {
            ...
            //If drugId is null or if it exists but isn't "COMBO", then proceed.
            //If it's not null and is "COMBO", then don't proceed.
            bool bProceed = true;
            if (drugId != null)
            {
                if (drugId.ToUpper() == "COMBO")
        ...
        public IEnumerable<Medication> GetSchedulerSetupData(int siteId, string brandName, bool bAll)
        {
            ...
            //If we get here on a nonexistent med (say we're testing in Postman and fat finger the medication name),
            //then med will be null.  And then we can't get drugId.
            //Define drugId here (as a string rather than a var).
            //And then set it below in a try/catch.
            //If we error, it will be null.
            //If we don't error, then it will be whatever it is from the DB.
            //Winston Murdock, 01/11/2023.
            //var drugId = med.DrugId;
            string drugId;

            //Try to get the medication's DrugId.
            //If we hit an error getting it, then set the local variable to null.
            try
            {
                drugId = med.DrugId;
            }
            ...
            //But, just in case we didn't find one, try to get a match on
            ...
            if (query.Count() > 0)
            {
                ...
                if (query[0] != null)
                {
                    ...
                    if (vendor == "F")
                    {
                        //Any medications that don't find a match in fdb_ndc_infos
                        //and that we need to remove from the list.
                        //Winston Murdock, 01/11/2023.
                        List<int> medsWithNoNdc = new List<int>();
                        ...
                        foreach (Medication medication in query)
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    //If the client has a med on their formulary with an NDC that doesn't
                                    //match the NDC in the fdb_ndc_info table, then this will have an empty FdbNdcInfos object.
                                    //In that case, we want to remove the medication with no NDC match from the result set.
                                    //Winston Murdock, 01/11/2023.
                                    medication.FdbNdcInfos =
                                    (
                                        //Should've named this FdbNdcInfos, but we didn't.
                                ...
                                } //end if (exact match or not?)

                                //If we don't have any medication Infos, then we need to remove
                                //this medication from the return list.
                                //You can't remove an item from the list inside a loop through the list.
                                //Grab the Id and add it to a list of Ids we want to remove.
                                //Then we'll remove each one outside of this loop.
                                //Winston Murdock, 01/11/2023.
                                if (!medication.FdbNdcInfos.Any())
                                {
                                    medsWithNoNdc.Add(medication.Id);
                                } //end if
                        ...
                        //Now that we've looped through all of the medications and gotten their FdbNdcInfo data,
                        //remove any medications that don't have any FdbNdcInfo data.
                        //Winston Murdock, 01/11/2023.
                        foreach (int medId in medsWithNoNdc)
                        {
                            //Get the medication object.
                            Medication medToRemove = query.FirstOrDefault(m => m.Id == medId);

                            //Then remove it.
                            query.Remove(medToRemove);

                            //Then loop to the next iteration.
                        } //end if
                    } //end if (vendor = "F"?)
        ...
        public List<FrequencyScheduleAdministration> GetSchedulerAdministrations(int siteId, EmarOrderType itemType, int itemId, DateTimeOffset start, DateTimeOffset? stop, int? duration = null, int? durationUnitId = null)
        {
            ...
            //Winston Murdock, 08/20/2021.  EMAR-1162.
            //If stop time is earlier than start time (i.e. now in the site's time zone),
            //we get an exception here.
            //We handle that earlier on by nulling out the stop time in this case.
            //There is no code change here.  But the code change in OrderService will prevent
            //this call from hitting an exception.
            //Winston Murdock, 09/02/2024.  PC-28315
