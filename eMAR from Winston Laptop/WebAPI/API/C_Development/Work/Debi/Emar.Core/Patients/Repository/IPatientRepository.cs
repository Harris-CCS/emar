...
namespace Emar.Core.Patients.Repository
{
    public interface IPatientRepository
    {
        ...
        long GetInternalPatientId(string externalId);
        ...
        IEnumerable<FdbAllergyName> GetAllergyFdbAllergyNamesByIdentifier(string pcHiclSeqno, Expression<Func<FdbAllergyName, bool>> wherePredicate = null);
        int? GetPatientsAttendingDoctorIdByRole(long patientId, string role);

        Patient GetAdministrationsByPatientAndOrder(long patientId, long orderId);

        #region IDS Methods

        void FilePatient(Patient patientFromHost, out bool interactRecalcNeeded, string queueRecordId);
        void FilePatientIndicators(Patient patientFromHost);
