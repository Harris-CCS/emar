    ...
    public class OcsEmarOutboundService : IOcsEmarOutboundService
    {
        ...
        public string SendChartTemplateMarkup(List<OcsPromptParameters> orderedPromptList, int siteId, long patientOrderId, int userId, ActionEnum action, long adminId, bool newOrderAdmin)
        {
            try
            {
                ...
                //pharmacy verification), set enteredTime = siteNow.
                enteredTime = siteNow.ToString("yyyyMMddHHmmss");
                ...
                //Leaving it for now.
                var currentDatetime = (new Time()).LongDateTime(siteNow.ToString("yyyyMMddHHmmss"));
                        ...
                        if (losecs.Contains(losecsFromOrder) || losecsFromOrder.Equals(line.TableXRef()))
                        {
                            ...
                            if (cancelOrDelete)
                            {
                                ...
                                if (signed == 0)
                                {
                                    ...
                                    newLine.LineHeader.inactive_time = siteNow.ToString("yyyyMMddHHmmss");
                            ...
                            //use siteNow if we don't have a value for enteredTime.
                            //newLine.LineHeader.user_time = enteredTime != null ? enteredTime : new Time().Timestamp();
                            newLine.LineHeader.user_time = enteredTime != null ? enteredTime : siteNow.ToString("yyyyMMddHHmmss");
