...
namespace Emar.Core.Orders.Repository
{
    public class OrderRepository : IOrderRepository
    {
        ...
        public OrderRepository(EmarContext emarContext, ISiteRepository siteRepository, IPropertyMappingService propertyMappingService, EmarMemoryCache cache, IOptionRepository optionRepository)
        {
            ...
            if (orders.Count() > 0)
            {
                ...
                if (orders[0] != null)
                {
                    ...
                    foreach (PatientOrder order in orders)
                    {
                                ...
                                foreach (OrderEvent orderEvent in order.OrderEvents)
                                {
                                    ...
                                    orderEvent.User =
                                        (
                                            ...
                                            where row.Id == orderEvent.AddUserId
                                            select row
                                        ).FirstOrDefault();
                                } //end foreach
                            } //end if
                        ...
                        var siteId = _context.Patients.Find(order.PatientId).SiteId;
                        ...
                        if (_optionRepository.GetOption(siteId, OptionNames.DRUG_DB_VENDOR).ToUpper() == "F")
                        {
                            ...
                            if (order.Medication.MedicationDetails.Count() > 0)
                            {
                                ...
                                {
                                    md.FdbBrandName = _context.FdbBrandName.Where(x => x.MedidString == md.DrugId).FirstOrDefault();
                                } //end foreach
                            } //end if
                        } //end if
                        ...
                        order.IsHighRisk = false;
                        if (_context.MedicationsHighRisk.Any(mhr => mhr.MedicationId == order.MedicationId))
                        {
                            //This order's medication exists in the medications_high_risk table.
                            //This is a high risk medication.
                            order.IsHighRisk = true;
                        } //end if
                        ...
                        order.IsPedatric = false;
                        
                        //Either the of these two lines works.
                        //but Find seems to be a little bit faster, so I'm going with that.
                        //var dateOfBirth = (DateTime)_context.Patients.Where(p => p.Id == order.PatientId).FirstOrDefault().DateOfBirth;
                        ...
                        if (dateOfBirth.HasValue)
                        {
                            ...
                            {
                                order.IsPedatric = true;
                            } //end if
                        }
                        else
                        {
                            //No date of birth.
                            //Assume pediatric just to be safe.
                            order.IsPedatric = true;
                        } //end if
