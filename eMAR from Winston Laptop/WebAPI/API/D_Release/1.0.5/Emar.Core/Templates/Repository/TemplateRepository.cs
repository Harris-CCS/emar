    ...
    public class TemplateRepository : ITemplateRepository
    {
            ...
            Dictionary<string, string> templateResponses = null)
        {
            ...
            if (CheckActionIsInIni(action, "administration"))
            {
                ...
                {
                    //If there's not already a notification for this order, then create one.
                    //If there's already a notification, then we don't need to make a new one for this order.
                    if (!_context.PharmacyNotificationOrders.Any(x => x.PatientOrderId == admin.PatientOrderId))
                    {
                        //Create the notification.
                        PharmacyNotification notification = new PharmacyNotification
                        {
                    ...
                    else
                    {
                        ...
                        //Get the pharmacy notification entity.
                        //Winston Murdock, 11/12/2021.  PC-26781
                        PharmacyNotification existingNotification = _context.PharmacyNotifications.Where(pn => pn.PharmacyNotificationOrders.Any(pno => pno.PatientOrderId == admin.PatientOrderId)).FirstOrDefault();
