...
namespace Emar.Core.Patients.Repository
{
    public class PatientRepository : IPatientRepository
    {
        ...
        public PatientRepository(EmarContext emarContext, ISiteRepository siteRepository,
            IUserRepository userRepository, IPropertyMappingService propertyMappingService,
            ILogger<PatientRepository> logger, IOptionRepository optionRepository)
        {
        ...
        public void FilePatient(Patient patientFromHost)
        {
            if (patientFromHost.Id > 0)
            {
                ...
                //var trackedEntry = _context.ChangeTracker.Entries<Patient>()
                //    .First(u => u.Entity.Id == patientFromHost.Id);
                ...
                UpdateAllergies(patientFromHost, existingPatient);
                UpdateMedications(patientFromHost, existingPatient);
        ...
        private void UpdateAllergies(Patient patientFromHost, Patient existingPatient)
        {
            // Delete children
            foreach (var existingChild in existingPatient.PatientAllergies.ToList().Where(existingChild =>
                // TODO: Winston Consult with Romel on how to determine if a patient_allergy exists, and update the 
                //       joins below between the existing/fromHost lists in the loops below
                // Finding if an allergy exists is more complex than the MedicationId or Name, but due to time constraints...
                // Feedback from Romel (through Jim):   this is the sequence:
                //   1. if free text med, match on name.
                //   2. If not free text, match on allergy_id, class, and category (in that order).
                patientFromHost.PatientAllergies.All(c => c.MedicationId != existingChild.MedicationId) &&
                patientFromHost.PatientAllergies.All(c => c.Name != existingChild.Name)))
            {
            ...
            foreach (var childModel in patientFromHost.PatientAllergies)
            {
                var existingChild = existingPatient.PatientAllergies
                    .SingleOrDefault(c => (c.MedicationId ?? -98) == (childModel.MedicationId ?? -99) ||
                                          (c.MedicationId == null && c.Name == childModel.Name));
        ...
        private void UpdateMedications(Patient patientFromHost, Patient existingPatient)
        {
            // Delete children
            foreach (var existingChild in existingPatient.PatientHomeMedications.ToList().Where(existingChild =>
                patientFromHost.PatientHomeMedications.All(c => c.MedicationId != existingChild.MedicationId) &&
                patientFromHost.PatientHomeMedications.All(c => c.Name != existingChild.Name)))
            {
                _context.Remove(existingChild);
            }
