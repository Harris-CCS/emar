...
namespace Emar.Core.PharmacyNotifications
{
    public class PharmacyNotificationService : IPharmacyNotificationService, IHostedService, IDisposable
    {
        ...
        private async void CheckQueue(object state)
        {
            ...
            using (var scope = _service.CreateScope())
            {
                ...
                foreach (PharmacyNotification notification in notifications)
                {
                    ...
                    if (notification.Type == "Order")
                    {
                        ...
                        if (pharmacyNotifications != null && pharmacyNotifications.Count > 0)
                        {
                            orders =
                                ...
                                .Include(f => f.FrequencySchedule).ToList();
                        }

                        // In the case of multiple orders in one notification, modify the template content by finding the grouped 
                        // order/med-specific lines and duplicating them as many times as necessary to have a field for each order.
                        if (orders.Count > 1)
                        {
                            ...
                            foreach (string line in lines)
                            {
                                ...
                                foreach (string field in lineFields)
                                {
                                    ...
                                    if (fieldType == "Order" || fieldType.StartsWith("Med"))
                                    {
                    ...
                    {
                        // TODO: Originally we were going to have admin-level notifications, but that idea was scrapped.
                        // If it comes back, we'd need logic here to grab the objects we need below.
                    }
                    ...
                    foreach(string field in subjectFields)
                    {
                        ...
                        if (dbObject != null)
                        {
                            templateValue = FormatPropertyValue(dbObject, fieldValue);
                        }
                    ...
                    foreach (PatientOrder order in orders)
                    {
                        ...
                        OrderAdministration administration = null;
        ...
        private string FormatPropertyValue(Object dbObject, string dbValueName)
        {
            ...
            {
                DateTime df = ((DateTimeOffset)result).DateTime;
                return df.ToShortDateString() + " " + df.ToLongTimeString();
            }
