...
namespace Emar.Core.Templates.Repository
{
    public class TemplateRepository : ITemplateRepository
    {
            ...
            long orderId, long? adminId = null)
        {
            ...
            // if a pharmacy verification action, then not a new order administration
            // else call the GetOrderAdminStatus() method to determine the new order administration status
            var newOrderAdmin = action == ActionEnum.PharmVerification ? false : GetOrderAdminStatus(orderId, adminId);
        ...
        public bool GetOrderAdminStatus(long orderId, long? adminId)
        {
            // pharmacy verification action has no adminId but is prevented from being called
            // other actions w/o an adminId like cancel and delete are handled afterwards (that's the intent, anyhow)
            if (adminId == null)
                return true;

            var query = from o in _context.OrderEvents
                        where o.PatientOrderId == orderId
                        orderby o.OrderAdministrationId descending
                        select o.OrderAdministrationId;
