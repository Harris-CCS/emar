using System;
using System.Collections.Generic;
using System.Globalization;
using System.Linq;
using Emar.Core.Helpers;
using Emar.Core.HomeMedications.Repository;
using Emar.Core.Medications.Model;
using Emar.Core.Options.Model;
using Emar.Core.Options.Repository;
using Emar.Core.Patients.Repository;
using Emar.Data.Entities;

namespace Emar.Core.MedicationReactions
{
    /// <summary>
    /// Library to handle interaction with drug databases
    /// </summary>
    public class DrugDb
    {
        private readonly IPatientRepository _patientRepository;
        private readonly IHomeMedicationRepository _homeMedicationRepository;
        private readonly IOptionRepository _optionRepository;
        private readonly IDrugDbUtility _instance;

        /// <summary>
        /// Checklist drugs
        /// </summary>
        private Dictionary<string, Dictionary<string, string>> _checkListDrugs = new Dictionary<string, Dictionary<string, string>>();

        /// <summary>
        /// List of drug IDs for checking
        /// </summary>
        private List<string> _drugChecklist = new List<string>();

        /// <summary>
        /// Flag for whether the checklist needs to be built/processed
        /// </summary>
        private bool _doChecklist = true;

        /// <summary>
        /// Stores information about drug classifications
        /// </summary>
        private Dictionary<string, List<Dictionary<string, string>>> _classInfo = new Dictionary<string, List<Dictionary<string, string>>>();

        /// <summary>
        /// Stores classifications per drug
        /// </summary>
        private Dictionary<string, List<string>> _drugClass = new Dictionary<string, List<string>>();

        /// <summary>
        /// Stores classifications per category
        /// </summary>
        private Dictionary<string, List<string>> _catClass = new Dictionary<string, List<string>>();

        /// <summary>
        /// Stores... Multum info?
        /// </summary>
        private Dictionary<string, List<Dictionary<string, string>>> _multInfo = new Dictionary<string, List<Dictionary<string, string>>>();

        /// <summary>
        /// Stores component info
        /// </summary>
        private Dictionary<string, List<Dictionary<string, string>>> _componentInfo = new Dictionary<string, List<Dictionary<string, string>>>();

        /// <summary>
        /// Stores info about Medication Services
        /// </summary>
        private List<Dictionary<string, object>> _medSvcInfo = new List<Dictionary<string, object>>();
        //////private List<Dictionary<string, string>> _medSvcInfo = new List<Dictionary<string, string>>();

        /// <summary>
        /// Stores drug ID -> name links
        /// </summary>
        private Dictionary<string, string> _dnum2Name = new Dictionary<string, string>();

        /// <summary>
        /// Stores drug name -> ID links
        /// </summary>
        private Dictionary<string, string> _dname2Num = new Dictionary<string, string>();

        /// <summary>
        /// Stores drug ID -> SourceTable links
        /// </summary>
        private Dictionary<string, string> _dnum2SourceTable = new Dictionary<string, string>();

        /// <summary>
        /// Stores drug ID -> SourceTableId links
        /// </summary>
        private Dictionary<string, string> _dnum2SourceTableId = new Dictionary<string, string>();

        /// <summary>
        /// Stores drug ID -> SourceTableMedicationId links
        /// </summary>
        private Dictionary<string, string> _dnum2SourceTableMedicationId = new Dictionary<string, string>();

        /// <summary>
        /// Stores drug ID -> SourceTableMedication links
        /// </summary>
        private Dictionary<string, object> _dnum2SourceTableMedication = new Dictionary<string, object>();

        /// <summary>
        /// Stores allergy drug ID -> SourceTable links
        /// </summary>
        private Dictionary<string, string> _algDnum2SourceTable = new Dictionary<string, string>();

        /// <summary>
        /// Stores allergy drug ID -> SourceTableId links
        /// </summary>
        private Dictionary<string, string> _algDnum2SourceTableId = new Dictionary<string, string>();

        /// <summary>
        /// DrugDB constructor
        /// </summary>
        /// <param name="patientRepository"></param>
        /// <param name="homeMedicationRepository"></param>
        /// <param name="optionRepository"></param>
        /// <param name="drugDbVendor"></param>
        public DrugDb(
            IPatientRepository patientRepository,
            IHomeMedicationRepository homeMedicationRepository,
            IOptionRepository optionRepository,
            string drugDbVendor)
        {
            if (drugDbVendor == null || drugDbVendor.Equals("F"))
            {
                _instance = new DrugDbFdb();
            }
            else
            {
                throw new NotSupportedException("Unknown drug database selector (" + drugDbVendor + ")");
            }

            _patientRepository = patientRepository;
            _homeMedicationRepository = homeMedicationRepository;
            _optionRepository = optionRepository;
        }

        /// <summary>
        /// Get the underlying DrugDB instance
        /// </summary>
        /// <returns>IDrugDBUtility</returns>
        private IDrugDbUtility GetInstance()
        {
            return _instance;
        }

        /// <summary>
        /// Add known drug ID <-> name links
        /// </summary>
        /// <param name="dnum">Drug identifier</param>
        /// <param name="name">Drug name</param>
        /// <param name="sourceTable"></param>
        /// <param name="sourceTableId"></param>
        /// <param name="sourceTableMedication"></param>
        public void AddDnumToDictionaries(string dnum, string name, string sourceTable, string sourceTableId, MedicationDto sourceTableMedication)
        {
            _dnum2Name[dnum] = name;
            _dname2Num[name] = dnum;
            _dnum2SourceTable[dnum] = sourceTable;
            _dnum2SourceTableId[dnum] = sourceTableId;
            _dnum2SourceTableMedicationId[dnum] = sourceTableMedication.Id.ToString();
            _dnum2SourceTableMedication[sourceTableMedication.Id.ToString()] = sourceTableMedication;
        }

        /// <summary>
        /// Check drug reactions against a patient's information
        /// </summary>
        /// <param name="siteId">Site identifier</param>
        /// <param name="patientId">Patient identifier</param>
        /// <param name="checklist">Drugs to check</param>
        /// <param name="drugDbVendor">Drug DB vendor</param>
        /// <returns>ReactionsCheckResult object</returns>
        public ReactionsCheckResult CheckReactions(int siteId, long patientId, Dictionary<string, string> checklist, string drugDbVendor,
            IEnumerable<PatientAllergy>? patientAllergies = null, IEnumerable<PatientHomeMedication>? patientHomeMedications = null)
        {
            var result = new ReactionsCheckResult();
            //Had to add false for "confirmedOnly" rather than letting it default to false as I
            //need to pass in the allergies and home meds.
            //Winston Murdock, 09/27/2022.  PC-27110
            var algMedData = LoadAlgMedTable(patientId, drugDbVendor, false, patientAllergies, patientHomeMedications);
            var allergyInfo = new List<Dictionary<string, object>>();
            var homeMedsInfo = new List<Dictionary<string, object>>();
            var rTrigger = new Dictionary<string, List<Dictionary<string, object>>>();
            var oTrigger = new Dictionary<string, List<Dictionary<string, object>>>();

            foreach (var alg in algMedData)
            {
                var type = alg["type"];
                var drug = alg["InternalDrugId"].ToString();

                if (type.Equals("A"))
                {
                    allergyInfo.Add(alg);
                }
                else if (type.Equals("M"))
                {
                    if (!patientId.ToString().Equals(alg["PatientId"]) || !alg.GetValueOrDefault("ActionStatus").ToString().Equals("C"))
                    {
                        continue;
                    }

                    homeMedsInfo.Add(alg);

                    var name = alg.ContainsKey("Name") && !string.IsNullOrWhiteSpace(alg["Name"].ToString()) ? alg["Name"].ToString() : alg["BrandName"].ToString();

                    _dnum2SourceTable[drug] = alg.GetValueOrDefault("SourceTable", "").ToString();
                    _dnum2SourceTableId[drug] = alg.GetValueOrDefault("SourceTableId", "").ToString();
                    _dnum2SourceTableMedicationId[drug] = alg.GetValueOrDefault("SourceTableMedicationId", "").ToString();

                    if (alg.ContainsKey("SourceTableMedicationId"))
                    {
                        _dnum2SourceTableMedication[drug] = alg.GetValueOrDefault("SourceTableMedication", null);
                    }

                    _dnum2Name[drug] = name + (alg.ContainsKey("AlternateName") && !string.IsNullOrWhiteSpace(alg["AlternateName"].ToString()) ? " (" + alg["AlternateName"] + ")" : "");
                    _dname2Num[name] = drug;
                }
            }

            var checkInfo = new List<Dictionary<string, object>>();
            var lookups = new Dictionary<string, List<Dictionary<string, object>>>();

            foreach (var alg in allergyInfo)
            {
                var drugVal = alg.ContainsKey("AllergyDrugId") && !string.IsNullOrWhiteSpace(alg["AllergyDrugId"].ToString())
                    ? alg["AllergyDrugId"]
                    : alg.ContainsKey("InternalDrugId")
                        ? alg["InternalDrugId"]
                        : "";

                if (alg.ContainsKey("ParentDrugId") && !string.IsNullOrWhiteSpace(alg["ParentDrugId"].ToString()))
                {
                    checkInfo.Add(new Dictionary<string, object>
                    {
                        { "ParentDrugName", alg["ParentDrugName"]},
                        { "ParentDrugId", alg["ParentDrugId"] },
                        { "Name", alg["Name"] },
                        { "Severity", alg["Severity"] },
                        { "AllergyDrugId", drugVal },
                        { "SourceTable", alg["SourceTable"] },
                        { "SourceTableId", alg["SourceTableId"] },
                        { "SourceTableMedicationId", alg.GetValueOrDefault("SourceTableMedicationId", "") },
                        { "SourceTableMedication", alg.GetValueOrDefault("SourceTableMedication", "") }
                    });
                }
                else if ((alg.ContainsKey("AllergyDrugId") && !string.IsNullOrWhiteSpace(alg["AllergyDrugId"].ToString()) && !alg["AllergyDrugId"].Equals("0")) ||
                         (alg.ContainsKey("InternalDrugId") && !string.IsNullOrWhiteSpace(alg["InternalDrugId"].ToString()) && !alg["InternalDrugId"].Equals("0")))
                {
                    if (!lookups.ContainsKey(drugVal.ToString()))
                    {
                        lookups.Add(drugVal.ToString(), new List<Dictionary<string, object>>());
                    }

                    lookups[drugVal.ToString()].Add(alg);
                }
                else
                {
                    checkInfo.Add(new Dictionary<string, object>
                    {
                        { "Name", alg["Name"] },
                        { "Class", alg["Class"] },
                        { "Category", alg["Category"] },
                        { "SourceTable", alg["SourceTable"] },
                        { "SourceTableId", alg["SourceTableId"] },
                        { "SourceTableMedicationId", alg.GetValueOrDefault("SourceTableMedicationId", "") },
                        { "SourceTableMedication", alg.GetValueOrDefault("SourceTableMedication", "") }
                    });
                }
            }

            var codeDup = new HashSet<string>();

            if (lookups.Keys.Count > 0)
            {
                var components = new List<string>(lookups.Keys);    // allergy_drug_id or internal_drug_id

                foreach (var dInfo in GetInstance().GetComponentInfo(components))
                {
                    var key = dInfo["drug"] + "|" + dInfo["cdrug"];

                    if (codeDup.Contains(key))
                    {
                        continue;
                    }

                    codeDup.Add(key);
                    var lookupKey = "drug";

                    if ((!lookups.ContainsKey(dInfo["drug"]) || lookups[dInfo["drug"]].Count == 0) && lookups.ContainsKey(dInfo["c_root"]))
                    {
                        lookupKey = "c_root";
                    }

                    checkInfo.AddRange(from parentInfo in lookups[dInfo[lookupKey]]
                                       let id = parentInfo.ContainsKey("AllergyDrugId") && !string.IsNullOrWhiteSpace(parentInfo["AllergyDrugId"].ToString())
                                           ? parentInfo["AllergyDrugId"]
                                           : parentInfo.ContainsKey("InternalDrugId")
                                               ? parentInfo["InternalDrugId"]
                                               : ""
                                       select new Dictionary<string, object>
                        {
                            {"ParentDrugName", parentInfo["Name"]},
                            {"ParentDrugId", id},
                            {"Name", dInfo["name"]},
                            {"Severity", parentInfo["Severity"]},
                            {"InternalDrugId", dInfo["cdrug"]},
                            {"SourceTable", parentInfo["SourceTable"]},
                            {"SourceTableId", parentInfo["SourceTableId"]},
                            {"SourceTableMedicationId", parentInfo["SourceTableMedicationId"]},
                            {"SourceTableMedication", parentInfo.GetValueOrDefault("SourceTableMedicationId", null)}
                        });
                }
            }

            var algReact = new Dictionary<string, Dictionary<string, Dictionary<string, object>>>();

            if (checklist != null && checklist.Count > 0)
            {
                _checkListDrugs.Clear();
                var allergyGroup = new Dictionary<string, Dictionary<string, Dictionary<string, Dictionary<string, Dictionary<string, object>>>>>();

                foreach (var fld in checkInfo)
                {
                    var id = fld.ContainsKey("AllergyDrugId") && !string.IsNullOrWhiteSpace(fld["AllergyDrugId"]?.ToString())
                        ? fld["AllergyDrugId"]
                        : fld.ContainsKey("InternalDrugId") && !string.IsNullOrWhiteSpace(fld["InternalDrugId"]?.ToString())
                            ? fld["InternalDrugId"]
                            : null;

                    if (!fld.ContainsKey("AlternateName"))
                    {
                        fld["AlternateName"] = "";
                    }

                    var tAlgReact = new Dictionary<string, object>();
                    DrugDChecklist(ref tAlgReact, fld.GetValueOrDefault("Class", 0).ToString(), fld.GetValueOrDefault("Category", 0).ToString(), id?.ToString(), ref checklist);

                    foreach (var sel in tAlgReact.Keys)
                    {
                        if (!checklist.ContainsKey(sel))
                        {
                            continue;
                        }

                        var topName = 
                            fld.ContainsKey("ParentDrugName") && !string.IsNullOrWhiteSpace(fld["ParentDrugName"]?.ToString()) ?
                                fld["ParentDrugName"] 
                                : fld.ContainsKey("Name") && !string.IsNullOrWhiteSpace(fld["Name"]?.ToString()) ? fld["Name"] 
                                : "";

                        if (!allergyGroup.ContainsKey(sel))
                        {
                            allergyGroup.Add(sel, new Dictionary<string, Dictionary<string, Dictionary<string, Dictionary<string, object>>>>());
                        }

                        if (!allergyGroup[sel].ContainsKey(topName.ToString()))
                        {
                            allergyGroup[sel].Add(topName.ToString(), new Dictionary<string, Dictionary<string, Dictionary<string, object>>>());
                        }

                        if (!allergyGroup[sel][topName.ToString()].ContainsKey(fld["AlternateName"].ToString()))
                        {
                            allergyGroup[sel][topName.ToString()].Add(fld["AlternateName"].ToString(), new Dictionary<string, Dictionary<string, object>>());
                        }

                        var drug = fld.ContainsKey("ParentDrugId") && !string.IsNullOrWhiteSpace(fld["ParentDrugId"].ToString())
                            ? fld["ParentDrugId"]
                            : fld.ContainsKey("AllergyDrugId") && !string.IsNullOrWhiteSpace(fld["AllergyDrugId"].ToString())
                                ? fld["AllergyDrugId"]
                                : fld.GetValueOrDefault("InternalDrugId", "");

                        var name = fld.ContainsKey("ParentDrugName") && !string.IsNullOrWhiteSpace(fld["ParentDrugName"].ToString())
                            ? fld["ParentDrugName"]
                            : fld.GetValueOrDefault("Name", "");

                        allergyGroup[sel][topName.ToString()][fld["AlternateName"].ToString()][fld["Name"].ToString()] = new Dictionary<string, object>
                        {
                            { "Class", (fld.ContainsKey("Class") && !String.IsNullOrWhiteSpace(fld["Class"].ToString())) ? fld["Class"] : "0" },
                            { "Category",   (fld.ContainsKey("Category") && !String.IsNullOrWhiteSpace(fld["Category"].ToString())) ? fld["Category"] : "0" },
                            { "InternalDrugId",  drug },
                            { "Name",  name },
                            { "Severity", (fld.ContainsKey("Severity") && !String.IsNullOrWhiteSpace(fld["Severity"].ToString())) ? fld["Severity"] : "0" },
                            { "SourceTable", fld["SourceTable"] },
                            { "SourceTableId", fld["SourceTableId"] },
                            { "SourceTableMedicationId", fld.GetValueOrDefault("SourceTableMedicationId", "") },
                            { "SourceTableMedication", fld.GetValueOrDefault("SourceTableMedication", "") }
                        };
                    }
                }

                foreach (var sel in allergyGroup.Keys)
                {
                    foreach (var name in allergyGroup[sel].Keys)
                    {
                        foreach (var altName in allergyGroup[sel][name].Keys)
                        {
                            var algName = name;

                            if (!string.IsNullOrWhiteSpace(altName))
                            {
                                algName += " (" + altName + ")";
                            }

                            var components = new List<string>(allergyGroup[sel][name][altName].Keys);
                            components.Sort();

                            if (components.Count > 1 || (components.Count == 1 && !name.ToLowerInvariant().Equals(components[0].ToLowerInvariant())))
                            {
                                algName += " [" + string.Join("/", components) + "]";
                            }

                            if (!algReact.ContainsKey(sel))
                            {
                                algReact.Add(sel, new Dictionary<string, Dictionary<string, object>>());
                            }

                            algName = algName.Trim();

                            if (!algReact[sel].ContainsKey(algName))
                            {
                                algReact[sel][algName] = new Dictionary<string, object>();
                            }

                            foreach (string comp in components)
                            {
                                algReact[sel][algName].Add(comp, allergyGroup[sel][name][altName][comp]);
                            }
                        }
                    }
                }
            }

            // Allergy checking is complete.
            // The remainder of the code is for Drug Interaction checking.

            var keyList = new List<string>(checklist.Keys);
            keyList.AddRange(_dnum2Name.Keys);
            keyList = keyList.Distinct().Where(x => !x.Equals("ft") && !string.IsNullOrWhiteSpace(x)).ToList();

            // This code identifies which medications interact with other medications
            // Get the data for components of multi-component medications.
            var component = new Dictionary<string, Dictionary<string, string>>();
            var parent = new Dictionary<string, List<string>>();

            if (keyList.Count > 0)
            {
                foreach (var info in GetInstance().GetComponentInfo(keyList))
                {
                    var drug = info["drug"];
                    var cdrug = info["cdrug"];
                    var name = info["name"];

                    if (!component.ContainsKey(drug))
                    {
                        component.Add(drug, new Dictionary<string, string>());
                    }

                    component[drug][cdrug] = name;

                    if (!parent.ContainsKey(cdrug))
                    {
                        parent.Add(cdrug, new List<string>());
                    }

                    parent[cdrug].Add(drug);
                }

                if (component.Keys.Count > 0)
                {
                    keyList.AddRange(component.Values.SelectMany(d => d.Keys));
                }
            }

            // Gather the interactions
            var interXref = new Dictionary<string, Dictionary<string, Dictionary<int, Dictionary<string, object>>>>();
            keyList = keyList.Distinct().Where(x => !x.Equals("ft")).ToList();

            if (keyList.Count > 0)
            {
                var rxAlert = Convert.ToInt32(_optionRepository.GetOption(siteId, OptionNames.RXALERT));
                var siteRxAlert = Constants.INTERACTION_RXALERT[rxAlert];

                foreach (var rtrigger in GetInstance().GetDrugInteractions(keyList))
                {
                    // If the interaction severity is a recognized one, but not among the levels selected for display 
                    // in the rxAlert site setting, then skip the trigger creation (that is, don't skip it if it is 
                    // not a recognized level in case something else is going on).
                    var severity = Convert.ToInt32(rtrigger["severity_id"]);

                    if ((Constants.INTERACTION_RXALERT[0].Contains(severity)) && !siteRxAlert.Contains(severity))
                    {
                        continue;
                    }

                    var d1 = rtrigger["drug_id_1"];
                    var d2 = rtrigger["drug_id_2"];

                    if (!interXref.ContainsKey(d1.ToString()))
                    {
                        interXref.Add(d1.ToString(), new Dictionary<string, Dictionary<int, Dictionary<string, object>>>());
                    }

                    if (!interXref[d1.ToString()].ContainsKey(d2.ToString()))
                    {
                        interXref[d1.ToString()].Add(d2.ToString(), new Dictionary<int, Dictionary<string, object>>());
                    }

                    interXref[d1.ToString()][d2.ToString()][severity] = rtrigger;
                }
            }

            // Find parent drugs that are current/ordered where the child component interaction is flagged, but 
            // the parent is not. Create a parent entry that contains the needed info.
            var dnum1List = interXref.Keys.ToArray();

            foreach (var dnum1 in dnum1List)
            {
                var dnum2List = interXref[dnum1].Keys.ToArray();

                foreach (var dnum2 in dnum2List)
                {
                    var sevList = interXref[dnum1][dnum2].Keys.ToArray();

                    foreach (var sev in sevList)
                    {
                        // Create entries for the parents
                        var parent1List = parent.ContainsKey(dnum1) ? parent[dnum1] : new List<string>();

                        foreach (var p in parent1List)
                        {
                            var info = new Dictionary<string, object>(interXref[dnum1][dnum2][sev]);
                            info["drug_id_1"] = p;

                            if (!interXref.ContainsKey(p))
                            {
                                interXref.Add(p, new Dictionary<string, Dictionary<int, Dictionary<string, object>>>());
                            }

                            if (!interXref[p].ContainsKey(dnum2))
                            {
                                interXref[p].Add(dnum2, new Dictionary<int, Dictionary<string, object>>());
                            }

                            if (!interXref[p][dnum2].ContainsKey(sev))
                            {
                                interXref[p][dnum2].Add(sev, new Dictionary<string, object>());
                            }

                            interXref[p][dnum2][sev] = info;
                        }

                        var parent2List = parent.ContainsKey(dnum2) ? parent[dnum2] : new List<string>();

                        foreach (var p in parent2List)
                        {
                            var info = new Dictionary<string, object>(interXref[dnum1][dnum2][sev]);
                            info["drug_id_2"] = p;

                            if (!interXref.ContainsKey(dnum1))
                            {
                                interXref.Add(dnum1, new Dictionary<string, Dictionary<int, Dictionary<string, object>>>());
                            }

                            if (!interXref[dnum1].ContainsKey(p))
                            {
                                interXref[dnum1].Add(p, new Dictionary<int, Dictionary<string, object>>());
                            }

                            if (!interXref[dnum1][p].ContainsKey(sev))
                            {
                                interXref[dnum1][p].Add(sev, new Dictionary<string, object>());
                            }

                            interXref[dnum1][p][sev] = info;
                        }
                    }
                }
            }

            dnum1List = interXref.Keys.ToArray();

            foreach (var dnum1 in dnum1List)
            {
                var dnum2List = interXref[dnum1].Keys.ToArray();

                foreach (var dnum2 in dnum2List)
                {
                    if (dnum1.Equals(dnum2))
                    {
                        continue;
                    }
                    var sevList = interXref[dnum1][dnum2].Keys.ToArray();

                    foreach (var sev in sevList)
                    {
                        var rtrigger = interXref[dnum1][dnum2][sev];
                        rtrigger["dname1"] = _dnum2Name.ContainsKey(dnum1) && !string.IsNullOrWhiteSpace(_dnum2Name[dnum1]) ? _dnum2Name[dnum1] : checklist.ContainsKey(dnum1) && !string.IsNullOrWhiteSpace(checklist[dnum1]) ? checklist[dnum1] : null;
                        rtrigger["dnum2"] = dnum2;
                        rtrigger["dname2"] = _dnum2Name.ContainsKey(dnum2) && !string.IsNullOrWhiteSpace(_dnum2Name[dnum2]) ? _dnum2Name[dnum2] : checklist.ContainsKey(dnum2) && !string.IsNullOrWhiteSpace(checklist[dnum2]) ? checklist[dnum2] : null;
                        rtrigger["SourceTable2"] = _dnum2SourceTable.ContainsKey(dnum2) && !string.IsNullOrWhiteSpace(_dnum2SourceTable[dnum2]) ? _dnum2SourceTable[dnum2] : null;
                        rtrigger["SourceTableId2"] = _dnum2SourceTableId.ContainsKey(dnum2) && !string.IsNullOrWhiteSpace(_dnum2SourceTableId[dnum2]) ? _dnum2SourceTableId[dnum2] : null;
                        rtrigger["SourceTableMedicationId2"] = _dnum2SourceTableMedicationId.ContainsKey(dnum2) && !string.IsNullOrWhiteSpace(_dnum2SourceTableMedicationId[dnum2]) ? _dnum2SourceTableMedicationId[dnum2] : null;

                        if (!string.IsNullOrEmpty(rtrigger["SourceTableMedicationId2"]?.ToString()))
                        {
                            rtrigger["SourceTableMedication2"] = _dnum2SourceTableMedication.GetValueOrDefault(rtrigger["SourceTableMedicationId2"]?.ToString(), null);
                        }
                        else
                        {
                            rtrigger["SourceTableMedication2"] = null;
                        }

                        rtrigger["sevtxt"] = Constants.SEVERITY_TEXT[sev];

                        // Store the information for interactions with current/ordered drugs
                        if (_dnum2Name.ContainsKey(dnum2))
                        {
                            // The parent drug is already flagged. The drug is flagged, but multi-component
                            // drugs need to show which component(s) interact.
                            var n = _dnum2Name.ContainsKey(dnum2) ? _dnum2Name[dnum2] : null;

                            if (n == null || interXref[dnum1].ContainsKey(n) && interXref[dnum1][n].ContainsKey(sev))
                            {
                                continue;
                            }

                            if (component.ContainsKey(dnum2))
                            {
                                // Check the components of a multi-component drug for interactions.
                                var componentList = new List<string>();
                                var componentKeyList = component[dnum2].Keys.ToArray();

                                foreach (var comp in componentKeyList)
                                {
                                    if (interXref[dnum1].ContainsKey(comp) && interXref[dnum1][comp].ContainsKey(sev))
                                    {
                                        componentList.Add(component[dnum2][comp]);
                                    }
                                }

                                if (componentList.Count > 0)
                                {
                                    rtrigger["dname2"] = _dnum2Name[dnum2] + " " + String.Join("", componentList.Select(x => "[" + x + "]").ToList());
                                }
                            }

                            if (!rTrigger.ContainsKey(dnum1))
                            {
                                rTrigger.Add(dnum1, new List<Dictionary<string, object>>());
                            }

                            rTrigger[dnum1].Add(rtrigger);
                        }

                        // Store the information for Combo medications (custom IV bags, etc.)
                        if (checklist.ContainsKey(dnum2))
                        {
                            if (!oTrigger.ContainsKey(dnum1))
                            {
                                oTrigger.Add(dnum1, new List<Dictionary<string, object>>());
                            }

                            oTrigger[dnum1].Add(rtrigger);
                        }
                    }
                }
            }

            result.AllergyInfo = allergyInfo;
            result.HomeMedsInfo = homeMedsInfo;
            result.MedSvcInfo = _medSvcInfo;

            result.Dnum2Name = _dnum2Name;
            result.Dname2Num = _dname2Num;
            result.Dnum2SourceTable = _dnum2SourceTable;
            result.Dnum2SourceTableId = _dnum2SourceTableId;
            result.Dnum2SourceTableMedicationId = _dnum2SourceTableMedicationId;
            result.AlgDnum2SourceTable = _algDnum2SourceTable;
            result.AlgDnum2SourceTableId = _algDnum2SourceTableId;

            result.Allergies = algReact;
            result.Interactions = rTrigger;
            result.ComboInteractions = oTrigger;

            return result;
        }

        /// <summary>
        /// Allergy reaction lookup
        /// </summary>
        /// <param name="rAlgReact">Reference to a Dictionary where the allergy/reaction info will be stored</param>
        /// <param name="cls">Drug classification to check</param>
        /// <param name="cat">Drug category to check</param>
        /// <param name="drugId">Drug ID to use to identify a class to check</param>
        /// <param name="checklist">Reference to a dictionary of drugs to check</param>
        /// <returns>Dictionary of reaction checking results</returns>
        private void DrugDChecklist(ref Dictionary<string, object> rAlgReact, string cls, string cat, string drugId, ref Dictionary<string, string> checklist)
        {
            if (_doChecklist)
            {
                _doChecklist = false;

                if (checklist != null && checklist.Keys.Count > 0)
                {
                    // Filter out free text entries that do not contain drug IDs.
                    _drugChecklist = new List<string>(checklist.Keys).FindAll(o => !string.IsNullOrWhiteSpace(o) && !o.Equals("0"));

                    if (_drugChecklist.Count == 0)
                    {
                        return;
                    }
                }

                // Get the components of multi-component medications and add them to the list.
                // This is needed to assure getting the components for checking against class/cat selections.
                foreach (var info in GetInstance().GetComponentInfo(_drugChecklist))
                {
                    if (!_checkListDrugs.ContainsKey(info["cdrug"]))
                    {
                        _checkListDrugs.Add(info["cdrug"], new Dictionary<string, string>());
                    }

                    _checkListDrugs[info["cdrug"]][info["drug"]] = info["name"];
                }
            }

            List<string> classList = new List<string>();
            // If a 'drug' or 'cat' is passed in; Multum will have one class, 'fdb' can be 1 or more classes.
            // If there is no 'drug', a 'class' or 'cat' must exist.


            if (!string.IsNullOrWhiteSpace(drugId))
            {
                if (!_drugClass.ContainsKey(drugId))
                {
                    _drugClass.Add(drugId, GetInstance().GetAllergyClassByDrug(drugId));
                }

                classList.AddRange(_drugClass[drugId]);
            }
            else if (!string.IsNullOrWhiteSpace(cat) && Convert.ToInt32(cat) > 0)
            {
                if (!_catClass.ContainsKey(cat))
                {
                    _catClass.Add(cat, GetInstance().GetAllergyClassByCategory(cat));
                }

                classList.AddRange(_catClass[cat]);
            }
            else
            {
                classList.Add(cls);
            }

            // Gather all the allergy info for any of the classes
            if (classList.Count > 0)
            {
                // Only get classes that haven't already been retrieved
                var getClasses = new List<string>();

                foreach (var c in classList)
                {
                    if (!_classInfo.ContainsKey(c))
                    {
                        getClasses.Add(c);
                    }
                }

                if (getClasses.Count > 0)
                {
                    foreach (var info in GetInstance().GetAllergies(classList, _checkListDrugs))
                    {
                        if (!_classInfo.ContainsKey(info["class"]))
                        {
                            _classInfo.Add(info["class"], new List<Dictionary<string, string>>());
                        }

                        _classInfo[info["class"]].Add(info);
                    }

                    foreach (var info in classList.Where(c => _classInfo.ContainsKey(c)).SelectMany(c => _classInfo[c]))
                    {
                        rAlgReact[info["drug"]] = info["name"];
                    }
                }

                // Marks the components of the Known Allergy drug entry as an Allergy
                // If we ever start displaying the text to identify Potential Intolerance,
                // the information will be stored in a new hash for intolerances when
                // the info is not already in the allergy reaction hash
                var components = new Dictionary<string, string>();

                if (!string.IsNullOrWhiteSpace(drugId))
                {
                    if (!_multInfo.ContainsKey(drugId))
                    {
                        foreach (var info in GetInstance().GetComponentInfo(new List<string> { drugId }))
                        {
                            if (!_multInfo.ContainsKey(info["cdrug"]))
                            {
                                _multInfo.Add(info["cdrug"], new List<Dictionary<string, string>>());
                            }

                            _multInfo[info["cdrug"]].Add(info);
                        }
                    }

                    foreach (var d in _multInfo[drugId].Where(d => !components.ContainsKey(d["cdrug"])))
                    {
                        components.Add(d["cdrug"], d["cdrug"]);
                    }
                }

                // Gather drugs based on intolerance to the current drug
                if (components.Keys.Count > 0)
                {
                    var compList = components.Keys.Where(c => !_componentInfo.ContainsKey(c)).ToList();

                    if (compList.Count > 0)
                    {
                        foreach (var info in GetInstance().GetAllergyIntolerances(compList, _drugChecklist))
                        {
                            if (!_componentInfo.ContainsKey(info["cdrug"]))
                            {
                                _componentInfo.Add(info["cdrug"], new List<Dictionary<string, string>>());
                            }

                            _componentInfo[info["cdrug"]].Add(info);
                        }
                    }

                    foreach (var i in components.Keys.Where(k => _componentInfo.ContainsKey(k)).SelectMany(k => _componentInfo[k]))
                    {
                        rAlgReact.Remove(i["drug"]);
                        rAlgReact.Add(i["drug"], i["brand"]);
                    }
                }
            }
        }

        /// <summary>
        /// Load patient allergies and home medications from the database
        /// </summary>
        /// <param name="drugDbVendor">Drug database vendor identifier</param>
        /// <param name="patientId">Patient identifier</param>
        /// <param name="confirmedOnly">Flag for whether the results should only include confirmed entries</param>
        /// <returns>List of Dictionary objects representing alg/med entries</returns>
        private IEnumerable<Dictionary<string, object>> LoadAlgMedTable(long patientId, string drugDbVendor, bool confirmedOnly = false,
            IEnumerable<PatientAllergy>? patientAllergies = null, IEnumerable<PatientHomeMedication>? patientHomeMedications = null)
        {
            //Added allergies and current meds as parameters.  If they are empty/null, then the patient just doesn't have any.
            //Winston Murdock, 09/27/2022.  PC-27110

            var algMedData = new List<Dictionary<string, object>>();

            //Everywhere that calls this should be passing the list of allergies into this method.
            //If this list comes in empty or null, then the patient just doesn't have any allergies.
            //if (patientAllergies == null)
            //{
            //    //List is null, get them from the DB.
            //    patientAllergies = _patientRepository.GetAllergiesByPatientId(patientId, a => a.IsActive && (a.ActionStatus == "C" || a.ActionStatus == "U"));
            //}
            //else if (patientAllergies.Count() < 1)
            //{
            //    //List has zero items.
            //    patientAllergies = _patientRepository.GetAllergiesByPatientId(patientId, a => a.IsActive && (a.ActionStatus == "C" || a.ActionStatus == "U"));
            //} //end if

            List<Dictionary<string, object>> algResult;
            algResult = DB.ConvertDataSetToListOfDictionariesObj(
                patientAllergies
                    .ToList()
                    .ToDataSet());

            foreach (var alg in algResult)
            {
                if (drugDbVendor != null &&
                    drugDbVendor == DrugDbVendors.FDB &&
                    (
                        (!string.IsNullOrWhiteSpace(alg["AllergyDrugId"]?.ToString()) && !alg["AllergyDrugId"].ToString().Equals("0")) ||
                        (!string.IsNullOrWhiteSpace(alg["ParentDrugId"]?.ToString()) && !alg["ParentDrugId"].ToString().Equals("0"))
                    ) && 
                    !(_patientRepository.GetAllergyFdbAllergyNamesByIdentifier(alg["AllergyDrugId"]?.ToString()).Any() ||
                      _patientRepository.GetAllergyFdbAllergyNamesByIdentifier(alg["ParentDrugId"]?.ToString()).Any()
                    ))
                {
                    continue;
                }

                alg["AlternateName"] = alg["AlternateName"]?.ToString()?.Trim();
                alg["ParentDrugName"] = alg["ParentDrugName"]?.ToString()?.Trim();
                alg["ParentDrugId"] = alg["ParentDrugId"]?.ToString()?.Trim();
                alg["type"] = "A";

                alg["SourceTable"] = SourceTables.PatientAllergies;
                alg["SourceTableId"] = alg["Id"];
                alg["SourceTableMedicationId"] = alg["MedicationId"];
                alg["OrderType"] = EmarOrderType.PatientAllergy.ToString();

                alg.Add("data_source", "PC");

                if (alg["ActionStatus"].Equals("C") && confirmedOnly || !confirmedOnly)
                {
                    algMedData.Add(alg);
                }
            }

            //Everywhere that calls this should be passing the list of current medications into this method.
            //If this list comes in empty or null, then the patient just doesn't have any current medications.
            //if (patientHomeMedications == null)
            //{
            //    //List is null, get them from the DB.
            //    patientHomeMedications = _homeMedicationRepository.GetPatientHomeMedications(a => a.PatientId == patientId && a.IsActive);
            //}
            //else if (patientAllergies.Count() < 1)
            //{
            //    //List has zero items.
            //    patientHomeMedications = _homeMedicationRepository.GetPatientHomeMedications(a => a.PatientId == patientId && a.IsActive);
            //} //end if

            List<Dictionary<string, object>> homeMedsResult;
            homeMedsResult =
                DB.ConvertDataSetToListOfDictionariesObj(
                    patientHomeMedications
                        .ToList()
                        .ToDataSet());

            foreach (var alg in homeMedsResult
                .Where(alg => drugDbVendor == null
                              || drugDbVendor != DrugDbVendors.FDB
                              || _homeMedicationRepository.GetPatientHomeMedicationFdbBrandNameByPcRoutedGenId(alg["InternalDrugId"].ToString()) != null))
            {
                alg["AlternateName"] = alg["AlternateName"]?.ToString()?.Trim();
                alg["ParentDrugName"] = alg["ParentDrugName"]?.ToString()?.Trim();
                alg["ParentDrugId"] = alg["ParentDrugId"]?.ToString()?.Trim();
                alg["type"] = "M";

                alg["SourceTable"] = SourceTables.PatientHomeMedications;
                alg["SourceTableId"] = alg["Id"];
                alg["SourceTableMedicationId"] = alg["MedicationId"];
                alg["OrderType"] = EmarOrderType.HomeMedication.ToString();

                if (alg["ActionStatus"].Equals("R")
                    && (alg["InformationSource"].Equals("PC")
                        || !alg["InformationSource"].Equals("PC")
                        && Convert.ToInt32(alg["ChangeUserId"]) > 0
                        && Convert.ToInt32(alg["AddUserId"]) == 0))
                {
                    alg["ActionStatus"] = "C";
                }

                if (!alg["ActionStatus"].Equals("C"))
                {
                    continue;
                }

                alg.Add("data_source", alg["ActionStatus"].Equals("U") ? "HIE" : "PC");

                if (alg["ActionStatus"].Equals("C") && confirmedOnly || !confirmedOnly)
                {
                    algMedData.Add(alg);
                }
            }

            var c = new CultureInfo(CultureInfo.CurrentCulture.Name);

            foreach (var alg in algMedData)
            {
                alg.Add("sorting_date",
                    alg.ContainsKey("ChangeDatetime") && !String.IsNullOrWhiteSpace(alg["ChangeDatetime"].ToString()) ? alg["ChangeDatetime"] :
                    alg.ContainsKey("AddDatetime") && !String.IsNullOrWhiteSpace(alg["AddDatetime"].ToString()) ? alg["AddDatetime"] :
                    "");

                if ((!alg.ContainsKey("AllergyDrugId") || String.IsNullOrWhiteSpace(alg["AllergyDrugId"].ToString())) &&
                    (!alg.ContainsKey("InternalDrugId") || String.IsNullOrWhiteSpace(alg["InternalDrugId"].ToString())) &&
                    (!alg.ContainsKey("Category") || String.IsNullOrWhiteSpace(alg["Category"].ToString()) || Convert.ToInt32(alg["Category"]) == 0) &&
                    (!alg.ContainsKey("Class") || String.IsNullOrWhiteSpace(alg["Class"].ToString()) || Convert.ToInt32(alg["Class"]) == 0))
                {
                    alg["InternalDrugId"] = "ft";
                }

                var n = (alg["InternalDrugId"].Equals("ft")
                    ? c.TextInfo.ToTitleCase((alg.ContainsKey("Name") ? alg["Name"] : alg["BrandName"])?.ToString()?.ToLowerInvariant() ?? string.Empty)
                    : alg.ContainsKey("Name") ? alg["Name"] : alg["BrandName"]);

                var f = c.TextInfo.ToTitleCase((alg.ContainsKey("Comment") ? alg["Comment"] : "")?.ToString()?.ToLowerInvariant() ?? string.Empty);
                alg.Add("paltxt", n + (!String.IsNullOrWhiteSpace(f) ? " - " + f : ""));
            }

            return algMedData;
        }

        /// <summary>
        /// Object to store results from reaction checking in a format that is only slightly easier to deal with than a regular Dictionary.
        /// </summary>
        public class ReactionsCheckResult
        {
            /// <summary>
            /// Allergy information
            /// </summary>
            public List<Dictionary<string, object>> AllergyInfo { get; set; }

            /// <summary>
            /// Freetext Allergy information
            /// </summary>
            public List<Dictionary<string, object>> FtAllergyInfo { get; set; }

            /// <summary>
            /// Current Medications information
            /// </summary>
            public List<Dictionary<string, object>> HomeMedsInfo { get; set; }

            /// <summary>
            /// Freetext Current Medications information
            /// </summary>
            public List<Dictionary<string, object>> FtHomeMedsInfo { get; set; }

            /// <summary>
            /// Medication Services information
            /// </summary>
            public List<Dictionary<string, object>> MedSvcInfo { get; set; }

            /// <summary>
            /// Maps dnums to drug names
            /// </summary>
            public Dictionary<string, string> Dnum2Name { get; set; }

            /// <summary>
            /// Maps drug names to dnums
            /// </summary>
            public Dictionary<string, string> Dname2Num { get; set; }

            /// <summary>
            /// Maps dnums to source table
            /// </summary>
            public Dictionary<string, string> Dnum2SourceTable { get; set; }

            /// <summary>
            /// Maps dnums to source table id
            /// </summary>
            public Dictionary<string, string> Dnum2SourceTableId { get; set; }

            /// <summary>
            /// Maps dnums to source table medication id
            /// </summary>
            public Dictionary<string, string> Dnum2SourceTableMedicationId { get; set; }

            /// <summary>
            /// Maps medication id to source medication
            /// </summary>
            public Dictionary<string, object> Dnum2SourceTableMedication { get; set; }

            /// <summary>
            /// Maps allergy dnums to source table
            /// </summary>
            public Dictionary<string, string> AlgDnum2SourceTable { get; set; }

            /// <summary>
            /// Maps allergy dnums to source table id
            /// </summary>
            public Dictionary<string, string> AlgDnum2SourceTableId { get; set; }

            /// <summary>
            /// Allergy information
            /// </summary>
            public Dictionary<string, Dictionary<string, Dictionary<string, object>>> Allergies { get; set; }

            /// <summary>
            /// Reaction triggers
            /// </summary>
            public Dictionary<string, List<Dictionary<string, object>>> Interactions { get; set; }

            /// <summary>
            /// Override triggers
            /// </summary>
            public Dictionary<string, List<Dictionary<string, object>>> ComboInteractions { get; set; }

            /// <summary>
            /// Default constructor
            /// </summary>
            public ReactionsCheckResult()
            {
                AllergyInfo = new List<Dictionary<string, object>>();
                FtAllergyInfo = new List<Dictionary<string, object>>();
                HomeMedsInfo = new List<Dictionary<string, object>>();
                FtHomeMedsInfo = new List<Dictionary<string, object>>();
                MedSvcInfo = new List<Dictionary<string, object>>();
                Dnum2Name = new Dictionary<string, string>();
                Dname2Num = new Dictionary<string, string>();
                Dnum2SourceTable = new Dictionary<string, string>();
                Dnum2SourceTableId = new Dictionary<string, string>();
                Dnum2SourceTableMedicationId = new Dictionary<string, string>();
                Dnum2SourceTableMedication = new Dictionary<string, object>();
                Allergies = new Dictionary<string, Dictionary<string, Dictionary<string, object>>>();
                Interactions = new Dictionary<string, List<Dictionary<string, object>>>();
                ComboInteractions = new Dictionary<string, List<Dictionary<string, object>>>();
            }
        }

        /// <summary>
        /// Constants used in drug databases
        /// </summary>
        private class Constants
        {
            // Interaction severity text
            // Items [1,2,3,4] are for Multum
            // Items [5,6,7,8] are for FDB (First Data Bank)
            public static readonly List<string> SEVERITY_TEXT = new List<string> {
                "",
                "MINOR",
                "MODERATE",
                "SEVERE",
                "ALLERGY",
                "UNDETERMINED",
                "MODERATE",
                "SEVERE",
                "CONTRAINDICATED",
            };

            // Translation between severities and display level (rxAlert value in admin->sites)
            public static readonly Dictionary<int, List<int>> INTERACTION_RXALERT = new Dictionary<int, List<int>> {
                { 0, new List<int> { 1, 2, 3, 4, 5, 6, 7, 8 } },
                { 1, new List<int> { 2, 3} },
                { 2, new List<int> { 3 } },
                { 5, new List<int> { 6, 7, 8 } },
                { 6, new List<int> { 7, 8 } }
            };
        }
    }
}