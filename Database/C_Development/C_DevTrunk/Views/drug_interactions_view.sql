...
as
    select [mi].[id]
         ...
         , coalesce([oi_1].[patient_order_id], [oi_1].[patient_cart_order_id]) as                                      [order_id_1]
         , case
               when [oi_1].[patient_order_id] is not null
                   then 'patient_orders'
               when [oi_1].[patient_cart_order_id] is not null
                   then 'patient_cart_orders'
           end as                                                                                                      [order_table_1]
         , coalesce([phm1].[name], [pcom1].[brand_name], [pom1].[brand_name]) as                                       [order_name_1]
         , coalesce([oi_2].[patient_order_id], [oi_2].[patient_cart_order_id], [oi_2].[patient_home_medication_id]) as [order_id_2]
         , case
               ...
               when [oi_2].[patient_home_medication_id] is not null
                   then 'patient_home_medication'
           end as                                                                                                      [order_table_2]
         , coalesce([phm2].[name], [pcom2].[brand_name], [pom2].[brand_name]) as                                       [order_name_2]
