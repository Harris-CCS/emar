create procedure [dbo].[export_ibex_patient_allergies]
as
    begin
        ...
        select cast([source].[site] as varchar(5)) + '|' + [source].[ibex] as [patient_id]
             ...
             , ltrim(rtrim([source].[status])) as                             [action_status]
...
execute [sys].[sp_addextendedproperty] 
  ...
  , @value = N'Procedure used to export ibex patient_allergies in emar format'
  , @level0type = N'SCHEMA'
  , @level0name = N'dbo'
  , @level1type = N'PROCEDURE'
  , @level1name = N'export_ibex_patient_allergies';
go