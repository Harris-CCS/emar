...
export class CalendarFormComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.events = EVENTS.map((event:AdministrationEvent) => {
            if (event.eventDate.includes('today')) {
                ...
                return {
                    id: event.id,
                    eventDate: ('0' + date.month.toString()).slice(-2) + '/' + ('0' + date.day.toString()).slice(-2) + '/' + date.year.toString(),
        ...
        this.setEvent.subscribe((dateTime: string) => {
            ...
            if (dateTime !== this.selectedEvent.eventDate + ' ' + this.selectedEvent.eventTime) {
    ...
    group(events: AdministrationEvent[]): CalendarDay[][] {
        ...
        for (let ii = 0; ii < events.length; ++ii) {
            ...
            if (iWeek < MAXWEEKS) { // do not display events outside the displayed weeks
                weeks[iWeek][iDay].events.push({
                    time: event.eventTime,
                    id: event.id,
                    idPrev: (ii == 0)? -1: events[ii - 1].id,
                    idNext: (ii == events.length -1)? -1: events[ii + 1].id,
                    disabled: !moment().isBefore(moment(this.local2ISO(event.eventDate+ ' '+ event.eventTime)))
                });
            }
        }
        console.log(weeks);
    ...
    onSelectTime(event: CalendarEvent) {
        const elt: AdministrationEvent = this.events.find(({ id }) => id === event.id);
        if (typeof elt !== "undefined") {
            let minDateTime: string = '';
            let maxDateTime: string = '';
            if (event.idPrev >= 0) {
                const eltPrev: AdministrationEvent = this.events.find(({ id }) => id === event.idPrev);
                minDateTime = eltPrev.eventDate + ' ' + eltPrev.eventTime;
            } else {
                minDateTime = this.composerMedForm.controls['frequency'].value.startTime;
            }
            if (event.idNext >= 0) {
                const eltNext: AdministrationEvent = this.events.find(({ id }) => id === event.idNext);
