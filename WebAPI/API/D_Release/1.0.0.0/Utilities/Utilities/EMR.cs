...
namespace PulseCheck.Utilities
{
    ...
    public class EMR : IEMRUtility
    {
        ...
        /// TODO NOTE: Currently this only supports charts for patients in the pat table.
        /// </summary>
        /// <returns></returns>
        private bool LoadEMR()
        {
            ...
            using (var con = new SqlConnection(connection))
            {
                using (SqlCommand cmd = new SqlCommand("[dbo].pc_retrieve_lines_from_active_chart", con))
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            var EMRLine = new Line(i)
                            {
                                ...
                                LineHeader = new Line.Header()
                                {
                                    audio = reader[Constants.Audio]?.ToString() ?? "",
            ...
            using (var con = new SqlConnection(connection))
            {
                ...
                foreach(object o in chartLines)
                {
                    ...
                    var sqlParameters = new SqlParameter[]
                    {
                            ...
                            new SqlParameter("@audio", SqlDbType.VarBinary) { Value = DB.NullParam(clonedLine.Audio()) },
        ...
        public class Line : ICloneable
        {
            ...
            public class Header
            {
                ...
                public string get(string identifier)
                {
                    switch (identifier)
                    {
                        ...
                        case Constants.User: return user >= 0 ? user.ToString() : "";
                        ...
                        case Constants.InactiveUser: return inactive_user >= 0 ? inactive_user.ToString() : "";
            ...
            public class DataSegment
            {
                ...
                private void ParseDataMarkup(string p)
                {
                    ...
                    var name = pieces[0];
                    var value = pieces.Length > 1 ? Escape.ChartUnescape(pieces[1]) : "";
                    ...
                    for(int i = 0; i < VALUE_SEGMENTS.Count; i++)
                    {
                        ...
                        var segmentValue = (valuePieces.Length > i && valuePieces[i] != null) ? valuePieces[i] : "";
