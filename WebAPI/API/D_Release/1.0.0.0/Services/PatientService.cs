...
namespace Services
{
    public class PatientService : IPatientService
    {
        ...
        static MedicationService _medSvc;
        ...
        public PatientService(IPatientRepository patientRepository, ISiteRepository siteRepository, IMedicationRepository medicationRepository, IUserRepository userRepository)
        {
            ...
            _medSvc = new MedicationService(medicationRepository, userRepository);
        }
                ...
                if (newLine != null)
                {
                    ...
                    if (status)
                    {
                        ...
                        if (Site.GetOrgOption("MED_INF").Equals("Y"))
                            MedicationService.TriggerFile(Site, patientId, user.Id, "ACKNOWLEDGED", med.Losecs);
        ...
        public async Task<string> PostMedOrders(ISite site, string patientId, User user, string type, int orderingPhysicianId, string notes, List<string> serviceOptions, string authType, List<OrderMedication> orders)
        {
            ...
            else if (type.Equals("ql"))
            {
                foreach(var order in orders)
                {
                    ...
                    var med = ((new MedicationService(_medicationRepository, _userRepository)).GetExtraQuickListData(drugDb, user, site, medData))[0];
            ...
            MedicationService.AddInteractionsAndReactionsToMedications(site, meds, patientId, _medicationRepository);
            ...
            using (var con = new SqlConnection(connection))
            {
                ...
                try
                {
                    ...
                    {
                        PulseCheck.Utilities.Chart.OnChartWrite(site, patientId, user.Id);
                        transaction.Commit();
                        ordersPlaced = true;
                    }
                    else
            ...
            if (ordersPlaced)
            {
                ...
                foreach(var med in meds)
                {
                    MedicationService.TriggerFile(site, patientId, user.Id, "ENTERED", med.Losecs);
        ...
        public async Task<string> PlaceOrder(byte siteId, string patientId, User user, List<Order> orders)
        {
            ...
            using (var con = new SqlConnection(connection))
            {
                ...
                var genderCode =
                    gender == DomainModel.Options.Gender.Male ? "M" :
                    gender == DomainModel.Options.Gender.Female ? "F" :
                    gender == DomainModel.Options.Gender.Unknown ? "U" : "O";
                ...
                foreach (var order in orders)
                {
                    ...
                    var values = new Dictionary<string, object>
                                {
                                    { Transaction.Constants.Name, settings["name"] },
                    ...
                    for (int i = 0; i < order.Quantity; i++)
                    {
                        ...
                        try
                        {
                            ...
                            var sqlParams = new List<SqlParameter> {
                                    ...
                                    new SqlParameter("@name", SqlDbType.VarChar) { Value = settings["name"] },
                                    ...
                                    if (convertedQuery.Validate())
                                    {
                                        ...
                                        var queryResult = new DB.Insert
                                        {
                                            ...
                                            Parameters = new SqlParameter[] {
                                                ...
                                                new SqlParameter("@ibex", SqlDbType.Char) { Value = patientId },
                                                new SqlParameter("@losecs", SqlDbType.Int) { Value = order.Losecs },
                                                new SqlParameter("@status", SqlDbType.Char) { Value = Constants.ACTIVE_STATUS.ToString() },
                    ...
                    {
                        if (p.Role.Id.Equals(Constants.Id_Doctor) && p.User != null && p.User.Id > 0)
                        {
        ...
        public async Task<List<Query>> GetServiceQueries(User user, string patientId, string serviceCode)
        {
            ...
            foreach (var svc in queryDefaults.Keys)
            {
                ...
                {
                    if (!orderDefaults.ContainsKey(k) || !orderDefaults[k]["gender_defaults"].Equals("Y"))
                        continue;

                    var type = orderDefaults[k].ContainsKey("type") ? orderDefaults[k]["type"] : "";
                    if (type.Equals("code") || type.Equals("other"))
                    {
                        var restrictedInfo = new Dictionary<string, string>
                        {
                            { "mnemonic", k },
                            { "svccode", svc },
                            { "gender", genderCode.Equals("U") ? "O" : genderCode}
                        };
            ...
            foreach (var field in keys)
            {
                ...
                else if (type.Equals(Query.Constants.TYPE_DROPDOWN))
                {
                    ...
                    convertedQuery.Codes.AddRange(queryCodes.Select(c => new Code { Display = c.Value["name"], Value = c.Key }));
        ...
        public async Task<List<OrderResult>> GetPatientResults(byte siteId, string patientId, User user, string mostCurrentOnly = "")
        {
            ...
            if (patient != null)
            {
                ...
                foreach (MinimalProvider p in patient.Providers)
                {
                    if (p.Role.Id.Equals(Constants.Id_Doctor))
                    {
            ...
            foreach(var result in results)
            {
                ...
                foreach (var comp in result.Components)
                {
                    ...
                    if (result.Source != null && result.Source.Equals("RAD")) {
                        if (!enterRad) {
                ...
                foreach (var result in resultsData)
                {
                    ...
                    if (segment.Equals("MSH"))
                    {
                        ...
                        if (results.Count > 0 || lineNum > 1)
                        {
                            ...
                            if (string.IsNullOrWhiteSpace(resultData.Status) || string.IsNullOrWhiteSpace(config.GetEntry("status_flags", resultData.Status)))
                    ...
                    else if (segment.Equals("OBX"))
                    {
                        ...
                        if (result["source"].Equals("RAD") || resultData.Source.Equals("RAD"))
                        {
                    ...
                    else
                    {
                        if (segment.Equals("OBR"))
                        {
                            ...
                            if (resultData.Components != null && resultData.Components.Count > 0)
                            {
                                ...
                                if (string.IsNullOrWhiteSpace(rClone.Status) || string.IsNullOrWhiteSpace(config.GetEntry("status_flags", rClone.Status)))
                                    results.Add(rClone);
        ...
        private void CreateTrigger(ISite site, Patient patient, int userId, string action, List<Medication> meds)
        {
            ...
            if (patAgeUnit == DomainModel.Options.AgeUnit.Day || patAgeUnit == DomainModel.Options.AgeUnit.Week)
            {
                patAge = 0;
            } else if (patAgeUnit == DomainModel.Options.AgeUnit.Month)
            {
                patAge /= 12;
            }
            ...
            foreach (var triggerId in triggerSettings.Keys)
            {
                ...
                if ((opt.Equals(Trigger.Constants.TRIGGER_MED_SVC_BOTH) || opt.Equals(triggerId)) && ((patAge == 0 && patAgeUnit == DomainModel.Options.AgeUnit.Day) || (minAge <= patAge && maxAge >= patAge)))
                {
                    var frmCSSite = MedicationService.GetFormularyShareSite(site.Id);
        ...
        private bool CheckActionForTrigger(string action, Dictionary<string, string> triggerSettings)
        {
            ...
            {
                opt = MedicationService.Constants.OPTS_MAPPING.ContainsKey(action) ? MedicationService.Constants.OPTS_MAPPING[action] : "";
            }
