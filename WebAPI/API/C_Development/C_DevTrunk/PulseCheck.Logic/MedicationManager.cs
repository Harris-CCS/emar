            ...
            else
            {
                ...
                foreach (var comp in med.Components)
                {
                    ...
                    foreach (var drugInteraction in comp.Reactions)
                    {
                        ...
                        {
                            // Make sure we're working on the override for the correct drug
                            if (!drugInteraction["dnum"].Equals(irOverride.Dnum))
                                continue;

                            OverrideRationale rationale = null;
                            if (irOverride.Dnum.Equals(comp.ActiveId))
                            {
                    ...
                    foreach (var allergyReaction in comp.Interactions)
                    {
                        ...
                        {
                            // Make sure we're working on the override for the correct drug
                            if (!allergyReaction["drug_id_2"].Equals(irOverride.Dnum))
                                continue;
