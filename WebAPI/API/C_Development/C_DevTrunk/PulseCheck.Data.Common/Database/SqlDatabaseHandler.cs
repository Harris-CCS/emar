...
namespace PulseCheck.Data.Common.Database
{
    public class SqlDatabaseHandler : ISqlDatabaseHandler
    {
        ...
        /// <returns>Boolean indicating if entry was deleted</returns>
        ...
        /// <returns>Boolean indicating if entry was deleted</returns>
        public bool Delete<T>(IDbConnection connection, T value, IDbTransaction transaction = null)
            where T : class, IData
        {
            if (connection == null)
                throw new ArgumentNullException(nameof(connection));

            return connection.Delete(value, transaction);
        }
