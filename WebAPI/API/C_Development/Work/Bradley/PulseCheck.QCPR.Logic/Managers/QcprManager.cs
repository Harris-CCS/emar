...
namespace PulseCheck.QCPR.Logic.Managers
{
    public class QcprManager : Manager, IQcprManager
    {
        ...
        public IEnumerable<GetProceduresResponse> GetProceduresByName(byte siteId, string procedureName)
        {
            return AutoMapper.Mapper.Map<IEnumerable<GetProceduresResponse>>(_importRepository.GetProcedureByName(siteId, procedureName));
        }

        public IEnumerable<GetProductsResponse> GetProductsByName(byte siteId, string productName)
        {
            return AutoMapper.Mapper.Map<IEnumerable<GetProductsResponse>>(_importRepository.GetProductByName(siteId, productName));
        }

        public IEnumerable<GetProductsResponse> GetProductById(long id)
        {
            return AutoMapper.Mapper.Map<IEnumerable<GetProductsResponse>>(_importRepository.GetProductById(id));
        }

        public IEnumerable<GetProductsResponse> GetProductsByProcedureId(long procedureId)
        {
            return AutoMapper.Mapper.Map<IEnumerable<GetProductsResponse>>(_importRepository.GetProductsByProcedureId(procedureId));
        }
