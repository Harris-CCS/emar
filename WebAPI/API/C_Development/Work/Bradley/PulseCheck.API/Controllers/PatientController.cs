...
namespace PulseCheck.API.Controllers
{
    ...
    public class PatientController : ControllerBase
    {
        ...
        /// </summary>
        /// <param name="patientManager">Patient service</param>
        /// <param name="siteHManager">Site service</param>
        /// <param name="userManager">User service</param>
        /// <param name="medicationManager">Medication service</param>
        public PatientController(IPatientManager patientManager, ISiteManager siteHManager, IUserManager userManager, IMedicationManager medicationManager)
        {
            _patientManager = patientManager;
            _siteManager = siteHManager;
        ...
        public async Task<IHttpActionResult> GetPatientV1(string patientId, string expand = "")
        {
            ...
            var result = await _patientManager.GetPatientByIdAsync(user.SiteId, patientId, user, expand);
            if (result == null)
            {
            ...
            pat.expand = Domain.Constants.Expando.GetPatientOptions();
            return Ok(pat);
        }
        ...
        public async Task<IHttpActionResult> GetPatientAllergiesV1(string patientId)
        {
            ...
            var result = await _patientManager.GetPatientAllergies(user.SiteId, patientId, user);
            if (result == null)
            {
                return new WLRResponse(ErrorCodes.NOT_AUTHORIZED, Request);
            }
        ...
        public async Task<IHttpActionResult> GetPatientCurrentMedicationsV1(string patientId)
        {
            ...
            var result = await _patientManager.GetPatientCurrentMedications(user.SiteId, patientId, user);
            if (result == null)
            {
                return new WLRResponse(ErrorCodes.NOT_AUTHORIZED, Request);
            }
        ...
        [HttpGet]
        public async Task<List<Group>> SearchOrdersV1(string patientId, [FromUri]string name)
        {
            var userId = _authUtil.GetAuthenticatedUserId(User);
            var user = await _userManager.GetUserByIdAsync(userId);
            if (user.HasAtLeastReadPermission(Permission.ORDERS))
            {
                var services = await _siteManager.SearchOrdersBySiteIdAsync(user.SiteId, name, userId);
                var groupedServices = await _patientManager.CreateOrderServices(user, patientId, services);
        ...
        public async Task<IHttpActionResult> PostPatientOrdersV1(string patientId, [FromBody]List<Order> orders)
        {
            ...
            var result = await _patientManager.PlaceOrder(user.SiteId, patientId, user, orders);
            if (result == null)
                return new WLRResponse(ErrorCodes.NOT_AUTHORIZED, Request);
        ...
        public async Task<IHttpActionResult> GetServiceQueriesV1(string patientId, string serviceCode)
        {
            ...
            var result = await _patientManager.GetServiceQueries(user, patientId, serviceCode);
            if (result == null)
            {
                return new WLRResponse(ErrorCodes.NOT_AUTHORIZED, Request);
            }
        ...
        public async Task<IHttpActionResult> GetPatientOrdersV1(string patientId, [FromUri]bool includeQueries=false)
        {
            ...
            {
                var result = await _patientManager.GetPatientOrders(user.SiteId, patientId, includeQueries);
                return Ok(result);
            }
        ...
        public async Task<IHttpActionResult> GetMostUsedMedsV1(string patientId)
        {
            ...
            {
                var site = await _siteManager.GetSiteByIdAsync(user.SiteId);
                var qlData = await _medicationManager.GetMedMostUsedList(user, site, patientId);
                return Ok(qlData);
            }
        ...
        public async Task<IHttpActionResult> GetQuickListMedsV1(string patientId)
        {
            ...
            {
                var site = await _siteManager.GetSiteByIdAsync(user.SiteId);
                var qlData = await _medicationManager.GetMedQuickList(user, site, patientId);
                return Ok(qlData);
            }
        ...
        public async Task<IHttpActionResult> GetBrandMedsV1(string patientId, string brand)
        {
            ...
            {
                var site = await _siteManager.GetSiteByIdAsync(user.SiteId);
                var qlData = await _medicationManager.GetBrandMeds(user, site, patientId, brand);
                return Ok(qlData);
            }
        ...
        public async Task<IHttpActionResult> SignChartV1([FromBody]PatientSign signInfo)
        {
            ...
            var result = await _patientManager.SignChart(user.SiteId, signInfo.Ibex, user);
            if (result != null)
        ...
        public async Task<IHttpActionResult> GetPatientMedicationGroupV1(string patientId, int groupNum)
        {
            ...
            {
                var site = await _siteManager.GetSiteByIdAsync(user.SiteId);
                var groups = await _siteManager.GetMedPathwaysBySiteIdAsync(site.Id).ConfigureAwait(false);
                ...
                var result = await _medicationManager.GetMedicationGroup(user, site, group, patientId);
                if (result == null)
                {
                    return new WLRResponse(ErrorCodes.NOT_AUTHORIZED, Request);
                }
        ...
        public async Task<IHttpActionResult> OrderAcknowledgeV1(string patientId, [FromBody]PatientMedOrder MedOrder)
        {
            int userId = _authUtil.GetAuthenticatedUserId(User);
            User user = await _userManager.GetUserByIdAsync(userId);
            var result = await _patientManager.AcknowledgeMedOrder(user.SiteId, patientId, user, MedOrder.Id);
            if (result == null)
            {
                return new WLRResponse(ErrorCodes.NOT_AUTHORIZED, Request);
            }
                ...
                var site = await _siteManager.GetSiteByIdAsync(user.SiteId);
                var updatedMed = await _patientManager.GetMedOrder(site, patientId, user, MedOrder.Id);
        ...
        public async Task<IHttpActionResult> PostAuthFailureV1(string patientId, [FromBody]MedAuthFailure authFailure)
        {
            ...
            var medObj = await _medicationManager.GetMedicationByLosecsAsync(user.SiteId, patientId, authFailure.Losecs);
            if (medObj != null)
            {
                ...
                {
                    var patient = await _patientManager.GetPatientByIdAsync(user.SiteId, patientId, user);
        ...
        public async Task<IHttpActionResult> GetMedOrderV1(string patientId, int orderId)
        {
            int userId = _authUtil.GetAuthenticatedUserId(User);
            User user = await _userManager.GetUserByIdAsync(userId);
            Site site = await _siteManager.GetSiteByIdAsync(user.SiteId);
            var result = await _patientManager.GetMedOrder(site, patientId, user, orderId);
            if (result == null)
            {
                return new WLRResponse(ErrorCodes.NOT_AUTHORIZED, Request);
            }
        ...
        public async Task<IHttpActionResult> GetMedOrdersV1(string patientId)
        {
            ...
            User user = await _userManager.GetUserByIdAsync(userId);
            Site site = await _siteManager.GetSiteByIdAsync(user.SiteId);
            var result = await _patientManager.GetMedOrders(site, patientId, user);
            if (result == null)
                return new WLRResponse(ErrorCodes.NOT_AUTHORIZED, Request);
        ...
        public async Task<IHttpActionResult> PostMedOrdersV1(string patientId, [FromBody]MedSvcOrder order)
        {
            ...
            User user = await _userManager.GetUserByIdAsync(userId);
            Site site = await _siteManager.GetSiteByIdAsync(user.SiteId);
            var result = await _patientManager.PostMedOrders(site, patientId, user, order.Type, order.OrderingPhysician, order.Notes, order.ServiceOptions, order.AuthType, order.MedOrders);
            if (result == null)
            {
                return new WLRResponse(ErrorCodes.NOT_AUTHORIZED, Request);
            }
        ...
        public async Task<IHttpActionResult> GetPatientLateResultsV1(string patientId)
        {
            int userId = _authUtil.GetAuthenticatedUserId(User);
            User user = await _userManager.GetUserByIdAsync(userId);
            var result = await _patientManager.GetPatientLateResults(user.SiteId, patientId, user);
            if (result == null)
            {
                return new WLRResponse(ErrorCodes.NOT_AUTHORIZED, Request);
            }
        ...
        public async Task<IHttpActionResult> GetPatientResultsV1(string patientId, string mostCurrentOnly = "")
        {
            int userId = _authUtil.GetAuthenticatedUserId(User);
            User user = await _userManager.GetUserByIdAsync(userId);
            var result = await _patientManager.GetPatientResults(user.SiteId, patientId, user, mostCurrentOnly);
            if (result == null)
            {
                return new WLRResponse(ErrorCodes.NOT_AUTHORIZED, Request);
            }
        ...
        public async Task<IHttpActionResult> PostPatientResultsV1(string patientId, [FromBody]List<int> resultLines) {
            int userId = _authUtil.GetAuthenticatedUserId(User);
            User user = await _userManager.GetUserByIdAsync(userId);
            var result = await _patientManager.PostPatientResults(user.SiteId, patientId, user, resultLines);
            return Ok(result);
        }
        ...
        public async Task<IHttpActionResult> GetPatientPathwayV1(string patientId, int pathwayNum)
        {
            ...
            var result = await _patientManager.GetPatientPathway(user.SiteId, patientId, pathwayNum, user);
        ...
        public async Task<IHttpActionResult> PostPatientAreaAssignmentV1(PatientLocation loc)
        {
            ...
            if (res == null)
            {
                var newPat = await _patientManager.GetPatientByIdAsync(siteId, loc.Ibex, user);
                return Ok(newPat);
            }
            return res;
        }
        ...
        private async Task<IHttpActionResult> PatientLocationAssignmentV1(PatientLocation loc, byte siteId)
        {
            int userId = _authUtil.GetAuthenticatedUserId(User);
            User user = await _userManager.GetUserByIdAsync(userId);
            if (!user.HasWritePermission(Permission.TRANSFER))
            {
                return new WLRResponse(ErrorCodes.NOT_AUTHORIZED, Request);
            }
            ...
            var oldPatient = await _patientManager.GetPatientByIdAsync(siteId, loc.Ibex, user);
        ...
        public async Task<IHttpActionResult> PostCommentV1([FromBody]PatientComments commentsInfo)
        {
            ...
            var comments = await _siteManager.GetCommentsBySiteIdAsync(user.SiteId);
            var patient = await _patientManager.GetPatientByIdAsync(user.SiteId, ibex, user);

            foreach(PatientComment p in commentsInfo.Comments) {
                var result = _patientManager.PostComment(user.SiteId, ibex, userId, p.Id, p.Name, p.Remove);
                if (result != 1)
                {
                    if (result < 0)
                    {
        ...
        public async Task<IHttpActionResult> PostSignUpV1(SignupInfo info)
        {
            ...
            {
                { Domain.Constants.Id_Resident, "RESIDENT" },
                { Domain.Constants.Id_Doctor, "ATTENDING" },
                { Domain.Constants.Id_DoctorExtender, "DOCTOR EXTENDER" }
            };
            ...
            var currentPat = await _patientManager.GetPatientByIdAsync(user.SiteId, info.Ibex, user);
            var oldProvider = 0;
            ...
            // User is already signed up for this role and they aren't marking the time they saw the patient. We're done.
            if (oldProvider == user.Id && info.TimeSeen == null)
            ...
            var onlySettingTimeSeen = oldProvider == user.Id && info.TimeSeen != null;

            if (oldProvider != user.Id)
            {
                if (role.Equals(Domain.Constants.Id_Doctor))
                {
                ...
                else if (role.Equals(Domain.Constants.Id_Resident))
                {
                    if (oldProvider == 0)
                    {
                ...
                else if (role.Equals(Domain.Constants.Id_DoctorExtender))
                {
                    if (oldProvider == 0)
                    {
                ...
                else if (role.Equals(Domain.Constants.Id_PrimaryNurse))
                {
                    if (oldProvider == 0)
                    {
                ...
                else if (role.Equals(Domain.Constants.Id_Extender))
                {
                    if (oldProvider == 0)
                    {
                ...
                else if (role.Equals(Domain.Constants.Id_Scribe))
                {

                }
                else if (role.Equals(Domain.Constants.Id_CareCoordinator))
                {
                    if (oldProvider == 0)
                ...
                if (oldProvider > 0)
                {
                    var oldUser = await _userManager.GetUserByIdAsync(oldProvider);
            ...
            using (var con = new SqlConnection(connection))
            {
                ...
                if (!onlySettingTimeSeen)
                {
                    ...
                    newPat = await _patientManager.GetPatientByIdAsync(user.SiteId, info.Ibex, user);
                ...
                {
                    // Patient information hasn't changed, so we can use the existing one
                    newPat = currentPat;
                }
                ...
                if (user.IsPhysician() && info.TimeSeen != null)
                {
                    ...
                    switch (role)
                    {
                        case Domain.Constants.Id_Doctor:
                            trxService = 50;
                            break;
                        case Domain.Constants.Id_Resident:
                            trxService = 51;
                            break;
                        case Domain.Constants.Id_DoctorExtender:
                            trxService = 52;
                            break;
                        default:
                            break;
                    ...
                    if (trxService > 0)
                    {
                        ...
                        if (currentEntry == null || info.TimeSeen < Time.DateTimeFromString(currentEntry["thrudate"].ToString()))
                        {
                            Dictionary<string, object> Values = new Dictionary<string, object>
                            {
                                ...
                                { Transaction.Constants.Minutes, _t.DiffMinutes(trxDate, trxThruDate) },
                            };
                    ...
                    var signupInfo = await _siteManager.GetSignupInfo(user.SiteId, user);
