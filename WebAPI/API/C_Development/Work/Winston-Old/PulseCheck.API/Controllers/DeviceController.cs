...
namespace PulseCheck.API.Controllers
{
    ...
    public class DeviceController : ControllerBase
    {
        ...
        /// </summary>
        public DeviceController(IDeviceRepository deviceRepo, IDeviceManager deviceManager, IAuthenticationManager authManager, UserAccountManager userAccountManager)
        {
        ...
        public async Task<IHttpActionResult> PostActivationV1([FromBody]AuthorizedDevice dInfo)
        {
            ...
            var existingDevice = await _deviceManager.GetDeviceByIdAsync(mobileDevice.DeviceId);
            ...
            if (!string.IsNullOrWhiteSpace(dInfo.AuthorizationCode))
            {
                if (dInfo.AuthorizationCode == ConfigurationManager.AppSettings["DeviceMasterPasscode"])
                    ...
                    hasValidAuthorizationCode = await _deviceManager.CheckDeviceAuthorization(dInfo.AuthorizationCode);
            }

            if (!hasValidAuthorizationCode)
                return BadRequest("Invalid activation code");

            _deviceManager.AddDevice(mobileDevice);
        ...
        public async Task<IHttpActionResult> PutInfoV1(string deviceId, [FromBody]MobileDevice device)
        {
            ...
            var deviceToUpdate = await _deviceManager.GetDeviceByIdAsync(deviceId);
            ...
            {
                var users = _userAccountManager.GetUserAccounts();
                UserAccount accountUsingDevice = null;
                ...
                if (accountUsingDevice != null && accountUsingDevice.Claims.Any())
                {
                    ...
                    _userAccountManager.RemovePulseCheckClaims(accountUsingDevice.ID);
                }
            }

            var result = await _deviceManager.Save(deviceToUpdate);
            if (result == 0)
        ...
        public async Task<IHttpActionResult> DeleteDeviceV1(string deviceId)
        {
            ...
            await _deviceManager.DeleteDevice(device.DeviceId);
