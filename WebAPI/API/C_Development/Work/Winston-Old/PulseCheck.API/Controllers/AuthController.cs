...
namespace PulseCheck.API.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public AuthController(IDeviceManager deviceManager, ISiteManager siteManager, IUserManager userManager, IUserMappingRepository userMapping, UserAccountManager userAccountManager)
        {
        ...
        public async Task<IHttpActionResult> Post([FromBody]AuthCredentials credentials)
        {
            ...
            {
                var device = await _deviceManager.GetDeviceByIdAsync(credentials.DeviceId);
            ...
            accountInfo.Account = _userAccountManager.GetByUsername(accountInfo.APIUsername);
            ...
            if (validLogin)
            {
                ...
                _userAccountManager.RemovePulseCheckClaims(acct.ID);
                
                if (accountInfo.DomainLogin)
                {
                    apiUsername = acct.Username;
                    var user = _userAccountManager.GetByUsername(apiUsername);
                    try
                    {
                        _userAccountManager.SetPassword(new Guid(user.ID.ToString()), apiPassword);
                    }
                    catch
                    {
                ...
                _userAccountManager.AddClaim(acct.ID, PulseCheckClaims.AccessToken, token.AccessToken);

                if (!string.IsNullOrWhiteSpace(credentials.DeviceId))  
                    _userAccountManager.AddClaim(acct.ID, PulseCheckClaims.DeviceId, credentials.DeviceId);
        ...
        public async Task<IHttpActionResult> PostSite([FromBody]SiteInfo siteInfo)
        {
            ...
            var accessibleSites = await _siteManager.GetSitesBySubjectIdAsync(subjectId);
            ...
            var userAccount = _userAccountManager.GetByID(id);
            foreach (var s in accessibleSites) {
                if (s.Id == requestedSite)
                {
                    ...
                    _userAccountManager.RemovePulseCheckClaims(id);
                    _userAccountManager.AddClaim(id, PulseCheckClaims.PulseCheckSiteId, s.Id.ToString());
                    _userAccountManager.AddClaim(id, PulseCheckClaims.PulseCheckUserId, userId.ToString());

                    var user = await _userManager.GetUserByIdAsync(userId);
                    var site = await _siteManager.GetSiteByIdAsync(s.Id);
                    site.UserInfo = user;
        ...
        {
            _userAccountManager.RemoveClaim(id, PulseCheckClaims.PulseCheckSiteId);
            _userAccountManager.RemoveClaim(id, PulseCheckClaims.PulseCheckUserId);
            _userAccountManager.RemoveClaim(id, PulseCheckClaims.DeviceId);
        }
        ...
        private bool ValidateCredentials(Utilities.AccountInfo accountInfo)
        {
            ...
            {
                validLogin = _userAccountManager.Authenticate(accountInfo.Username, accountInfo.Password);
            }
