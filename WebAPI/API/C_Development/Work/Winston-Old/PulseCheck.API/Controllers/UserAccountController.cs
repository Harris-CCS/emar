...
namespace PulseCheck.API.Controllers
{
    ...
    public class UserAccountController : ControllerBase
    {
        ...
        public UserAccountController(ISiteManager siteManager, IUserManager userManager, IUserRepository userRepo, IUserMappingRepository mappingRepo, UserAccountManager userAccountManager, IAuthenticationManager authManager, IDeviceManager deviceManager, IEmailManager emailManager)
        {
        ...
        [HttpPost]
        [AllowAnonymous]
        public async Task<IHttpActionResult> CreateNewAccount([FromBody]UserAccountConfiguration newAccount)
        {
            var user = await GetValidatedWebUser();
            var password = Utilities.Security.GeneratePassword();
            var account = _userAccountManager.CreateUserAccount(newAccount, password);

            var devicePasscode = _deviceManager.CreateAuthorizationCode();
            var token = await GeneratePasswordToken(account, password);
            // TODO: something if the e-mail wasn't sent
            var sentMail = await _emailManager.SendNewAccountEmail(account, token.AccessToken, devicePasscode);
        ...
        {
            var user = await GetValidatedWebUser();
            var account = _userAccountManager.GetByID(accountId);
            if (account == null)
        ...
        public async Task<IHttpActionResult> RemoveAccount(Guid accountId)
        {
            ...
            {
                _userAccountManager.DeleteAccount(accountId);
                return Ok();
            }
        ...
        [HttpPut]
        [AllowAnonymous]
        public async Task<IHttpActionResult> UpdateAccount(Guid accountId, [FromBody]UserAccount updatedAccount)
        {
            var user = await GetValidatedWebUser();
            var existingAccount = _userAccountManager.GetByID(accountId);
            if (existingAccount == null)
                return new ErrorResponse("Account not found", 404, Request);

            _userAccountManager.ChangeUserAccount(existingAccount, updatedAccount);
            return Ok();
        }
        ...
        public IHttpActionResult UpdatePassword([FromBody]string password)
        {
            ...
            var updateError = _userAccountManager.UpdatePassword(account, password);
            if (updateError != null)
        ...
        public async Task<IHttpActionResult> SendDeviceAuthorization(Guid accountId)
        {
            ...
            var devicePasscode = _deviceManager.CreateAuthorizationCode();
            var sentMail = await _emailManager.SendDeviceAuthorizationEmail(account, devicePasscode);
            if (sentMail)
                return Ok();
        ...
        public async Task<IHttpActionResult> ResetPassword(Guid accountId)
        {
            ...
            var password = Utilities.Security.GeneratePassword();
            _userAccountManager.SetPassword(account.ID, password);

            var token = await GeneratePasswordToken(account, password);
            var sentMail = await _emailManager.SendAccountPasswordResetEmail(account, token.AccessToken);
            if (sentMail)
                return Ok();
        ...
        {
            var user = await GetValidatedWebUser();
            var accounts = _userAccountManager.GetUserAccounts();
            return Ok(accounts);
        }

        private async Task<User> GetValidatedWebUser()
        {            
            var user = await _authManager.GetValidatedWebUser();
        ...
        private async Task<TokenResponse> GeneratePasswordToken(UserAccount account, string password)
        {
