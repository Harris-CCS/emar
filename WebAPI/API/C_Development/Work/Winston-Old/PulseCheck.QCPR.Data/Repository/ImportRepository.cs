    ...
    public class ImportRepository : SqlDatabaseRepository, IImportRepository
    {
        ...
        public void SaveImportData(Procedure[] procedures)
        {
            ...
            if(siteMappings == null)
                throw new InvalidOperationException("No site mappings were defined for the integrations");
         
            SqlDatabaseHandler.Execute("DELETE FROM dbo.QcprProcedure");
            SqlDatabaseHandler.Execute("DELETE FROM dbo.QcprProduct");
            ...
            {
                var map = siteMappings.First(x => x.FacilityName.ToLowerInvariant() == procedure.Facility.ToLowerInvariant());
                procedure.SiteId = map?.SiteId;
                procedure.Id = SqlDatabaseHandler.Insert(procedure);

                proceduresToCache.Add(procedure);

                foreach (Product product in procedure.Products)
                {
                    product.QcprProcedureId = procedure.Id;
                    product.Id = SqlDatabaseHandler.Insert(product);

                    productsToCache.Add(product);

                    foreach (Route route in product.Routes)
                    {
                        route.QcprProductId = product.Id;
        ...
        public IEnumerable<Procedure> GetProcedureByName(string procedureName)
        {
            if (string.IsNullOrEmpty(procedureName))
                throw new ArgumentNullException(nameof(procedureName));

            IEnumerable<Procedure> retVal = GetProceduresFromCache(procedureName);

            if (retVal != null)
                return retVal;

            retVal = GetProcedureFromTable(procedureName);
        ...
        private IEnumerable<Procedure> GetProcedureFromTable(string procedureName)
        {
            if (string.IsNullOrEmpty(procedureName))
                throw new ArgumentNullException(nameof(procedureName));
        ...
        private IEnumerable<Procedure> GetProceduresFromCache(string name)
        {
            ...
            return from p in procs where p.Name.ToLowerInvariant().Contains(name.ToLowerInvariant()) select p;
        }
        ...
        public IEnumerable<Product> GetProductByName(string productName)
        {
            ...
            IEnumerable<Product> retVal = GetProductsFromCache(productName);

            if (retVal != null)
                return retVal;

            retVal = GetProductFromTable(productName);
        ...
        public IEnumerable<Product> GetProductById(long productId)
        {
            ...
            if (retVal == null)
                throw new RowNotInTableException($"ProductName = {productId}");

            return retVal;
        }

        private IEnumerable<Product> GetProductFromTable(string productName)
        {
            if (string.IsNullOrEmpty(productName))
                throw new ArgumentNullException(nameof(productName));
            ...
            return SqlDatabaseHandler.ExecuteStoredProcedure<Product>("QcprProductGetByName", p);
        }
        ...
        private IEnumerable<Product> GetProductsFromCache(string name)
        {
            ...
            return from p in products where p.Name.ToLowerInvariant().Contains(name.ToLowerInvariant()) select p;
        }
        ...
        private IEnumerable<Product> GetProductsFromCache(long id)
        {
            ...
            return from p in products where p.Id == id select p;
        }
