        ...
        public async Task<bool> PostPatientResults(byte siteId, string patientId, User user, List<int> lineNums)
        {
            ...
            foreach(var postLine in lineNums)
            {
                foreach(var result in results)
                {
                    ...
                    // If there was only a single Component, use the postComponent result text instead of allContent
                    if (i == 1)
                        allContent = "";

                    //if (postComponent != null && !multi) - multi line Lab results have multi set, but we don't want to use allContent for them
                    if (postComponent != null && string.IsNullOrWhiteSpace(allContent))
                    {
                    ...
                    if (postComponent != null)
                    {
                        ...
                        postResults.Add(new OrderResult.ResultForChart
                        {
                            ...
                            LineCT = postLine,
                            IsAbnormal = postComponent.IsAbnormal || postComponent.IsCritical,
                            DateString = DTE,
