    ...
    public sealed class NLogger : ITraceWriter
    {
        ...
        private void Log(TraceRecord record)
        {
            ...
            if (record.Request != null)
            {
                ...
                {
                    var token = record.Request.Headers.GetValues("Authorization").FirstOrDefault();
                    message.Append("").Append("Token: " + token + Environment.NewLine);
                    IPrincipal principal = System.Threading.Thread.CurrentPrincipal;
                    if (principal != null)
                    {
                        try
                        {
                            var authUtil = new Authentication();
                            int userId = authUtil.GetAuthenticatedUserId(principal);
                            int siteId = authUtil.GetAuthenticatedUserSite(principal);

                            message.Append("").Append("User ID: " + userId + Environment.NewLine);
                            message.Append("").Append("User Site: " + siteId + Environment.NewLine);
                        } catch (Exception)
                        {
